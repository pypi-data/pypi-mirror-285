...
# pip install "acryl-datahub[datahub-rest,datahub-kafka,snowflake,my-custom-extra] @ https://something.vercel.com/artifacts/wheels/acryl_datahub-0.0.0.dev1-py3-none-any.whl" extra_package extra_package2
# pip install "acryl-datahub[datahub-rest,datahub-kafka,snowflake,my-custom-extra]==0.10.2" extra_package extra_package2
# pip install --upgrade "acryl-datahub[datahub-rest,datahub-kafka,snowflake,my-custom-extra]" extra_package extra_package2
set -x  # print the final pip install command
pip install ${PIP_EXTRA_ARGS:-} "${PACKAGE}${package_extras:+[$package_extras]}${package_suffix}" ${PIP_EXTRA_PACKAGES:-} ${PIP_GLOBAL_EXTRA_PACKAGES:-}

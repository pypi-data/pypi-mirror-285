...
from Compression.PyTorch.prune.Weight.compute_conv import ComputeConv


class Ln_conv_prune(PruneGraph):
    def __init__(self, orinet, configs):
        super(Ln_conv_prune,self).__init__(orinet, configs)
        # self.compress()
    ...
    def cacl_prune(self):
        self.layers_pruned_config = ComputeConv(self.original_net, pru_ratio=self.configs['prune_ration'], use_global_bn=self.configs['global']).layer_thrs
...
#
#     Dot_gen = Ln_conv_prune(model, config)

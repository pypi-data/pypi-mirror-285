...
def analyse_lr2(train_path:str):
...
def save2json(lr,test_path:str):
    ...
    with open('result.json', 'w', encoding='utf-8') as fw:
        fw.write(res_)
def fast_acc(lr,test_path):
    test = pd.read_csv(test_path)
    result_dict = json.load(open('bel.json'))
    xxxxxxxx = test.drop(['PatientID', 'target'], axis=1)
    xxxxxxxx_y = test['target']
    print(f1_score(xxxxxxxx_y, lr.predict(xxxxxxxx)))
    arr = test['PatientID'].array
    arr2 = test['BMI'].array
    arr3 = test['Smoker'].array
    ...
    for e in zip(arr,arr2,arr3, p_label):
        print(e)
        t_.append(result_dict[str(e[0])])
        if (int(e[2])+int(e[-1]))>=1:
            pre_.append(1)
        else:
            pre_.append(0)
    ...
    print('f--f1-',f1_score(t_,pre_))

if __name__=="__main__":
    fiter_,x_test=analyse_lr2('train.csv')
    save_pic(x_test)
    # save2json(fiter_,'test.csv')
    fast_acc(fiter_,'test.csv')

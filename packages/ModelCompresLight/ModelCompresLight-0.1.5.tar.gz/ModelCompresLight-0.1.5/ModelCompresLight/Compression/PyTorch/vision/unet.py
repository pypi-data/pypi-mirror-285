...
def dataload():
    ...
    image_datasets = {
        'Train': Loadataset(image_path, mask_path, transform=data_transforms['Train']),
        'Test': Loadataset(image_path, mask_path, transform=data_transforms['Test'])
    }
...
model = smp.Unet(
    encoder_name="resnet50",    # 使用的Encoder名称，例如resnet18
    encoder_weights=None, # 使用的预训练权重，这里使用ImageNet预训练权重
    # in_channels=3,              # 输入图像通道数，通常为3（RGB图像）
    classes=13,        # 分割的类别数
)
...
#     torch.save(model, os.path.join(bpath, '50epochs_weights.pt'))
    return model
def light_date(save_out,epochs):
    epochs = 50
    ...
    trained_model=train_model(model, criterion, dataloaders,optimizer, bpath=save_out, metrics=metrics, num_epochs=epochs)

    torch.save(trained_model, os.path.join(save_out, f'{epochs}_model.pt'))

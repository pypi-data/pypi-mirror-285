...
def getSongPos(mode: int = -1) -> 'float | int':
    ...
    * This will get the position in the song in song mode, or the position
      in the pattern, depending on the state of [transport.getLoopMode()](https://il-group.github.io/FL-Studio-API-Stubs/midi_controller_scripting/transport/position#transport.__position.getLoopMode).

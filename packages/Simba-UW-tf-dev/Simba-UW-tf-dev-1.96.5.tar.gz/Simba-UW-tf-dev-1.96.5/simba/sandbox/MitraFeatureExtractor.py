...
class MitraFeatureExtractor(ConfigReader):
    ...
    def run(self):
        for file_cnt, file_path in enumerate(self.outlier_corrected_paths):
            ...
            print(f'Featurizing video {video_name} ...')
            ...
            left_body_arr = df[[f'{LEFT_EAR}_x', f'{LEFT_EAR}_y', f'{NOSE}_x', f'{NOSE}_y', f'{LEFT_SIDE}_x', f'{LEFT_SIDE}_y', f'{TAIL_BASE}_x', f'{TAIL_BASE}_y', f'{CENTER}_x', f'{CENTER}_y']].values.astype(np.float32).reshape(len(df), 6, 2)
            right_body_arr = df[[f'{RIGHT_EAR}_x', f'{RIGHT_EAR}_y', f'{NOSE}_x', f'{NOSE}_y', f'{RIGHT_SIDE}_x', f'{RIGHT_SIDE}_y', f'{TAIL_BASE}_x', f'{TAIL_BASE}_y', f'{CENTER}_x', f'{CENTER}_y']].values.astype(np.float32).reshape(len(df), 6, 2)
            ...
            results['GEOMETRY_FRAME_BODY_PERIMETER_LENGTH'] = (jitted_hull(points=animal_body_arr, target='perimeter') / px_per_mm).astype(np.int32)
            results['GEOMETRY_FRAME_LOWER_BODY_PERIMETER_LENGTH'] = (jitted_hull(points=animal_lower_body_arr, target='perimeter') / px_per_mm).astype(np.int32)
            results['GEOMETRY_FRAME_UPPER_BODY_PERIMETER_LENGTH'] = (jitted_hull(points=animal_upper_body_arr, target='perimeter') / px_per_mm).astype(np.int32)
            ...
            results['GEOMETRY_FRAME_LEFT_BODY_PERIMETER_LENGTH'] = (jitted_hull(points=left_body_arr, target='perimeter') / px_per_mm).astype(np.int32)
            results['GEOMETRY_FRAME_LEFT_BODY_PERIMETER_LENGTH'] = (jitted_hull(points=left_body_arr, target='perimeter') / px_per_mm).astype(np.int32)
            ...
            for time, feature in product(TIME_WINDOWS, ['HULL_LENGTH', 'HULL_WIDTH', 'HULL_PERIMETER_LENGTH', 'BODY_PERIMETER_LENGTH', 'LOWER_BODY_PERIMETER_LENGTH', 'UPPER_BODY_PERIMETER_LENGTH', 'HEAD_PERIMETER_LENGTH', 'TAIL_LENGTH', 'EAR_DISTANCE']):
                results[f'GEOMETRY_MEAN_{feature}_{time}'] = results[f'GEOMETRY_FRAME_{feature}'].rolling(int(time * fps), min_periods=1).mean()
                results[f'GEOMETRY_VAR_{feature}_{time}'] = results[f'GEOMETRY_FRAME_{feature}'].rolling(int(time * fps), min_periods=1).var()
                results[f'GEOMETRY_SUM_{feature}_{time}'] = results[f'GEOMETRY_FRAME_{feature}'].rolling(int(time * fps), min_periods=1).sum()

            for feature in ['HULL_LENGTH', 'HULL_WIDTH', 'HULL_PERIMETER_LENGTH', 'BODY_PERIMETER_LENGTH', 'LOWER_BODY_PERIMETER_LENGTH', 'UPPER_BODY_PERIMETER_LENGTH', 'HEAD_PERIMETER_LENGTH', 'TAIL_LENGTH', 'EAR_DISTANCE']:
            ...
            upper_lower_body_size_correlations = pd.DataFrame(Statistics.sliding_spearman_rank_correlation(sample_1=results['GEOMETRY_FRAME_UPPER_BODY_PERIMETER_LENGTH'].values.astype(np.float32), sample_2=results['GEOMETRY_FRAME_LOWER_BODY_PERIMETER_LENGTH'].values.astype(np.float32), time_windows=TIME_WINDOWS, fps=fps), columns=['GEOMETRY_UPPER_LOWER_BODY_SIZE_SPEARMAN_250', 'GEOMETRY_UPPER_LOWER_BODY_SIZE_SPEARMAN_500', 'GEOMETRY_UPPER_LOWER_BODY_SIZE_SPEARMAN_1000', 'GEOMETRY_UPPER_LOWER_BODY_SIZE_SPEARMAN_2000'])
            hull_head_correlations = pd.DataFrame(Statistics.sliding_spearman_rank_correlation(sample_1=results['GEOMETRY_FRAME_HULL_PERIMETER_LENGTH'].values.astype(np.float32), sample_2=results['GEOMETRY_FRAME_HEAD_PERIMETER_LENGTH'].values.astype(np.float32), time_windows=TIME_WINDOWS, fps=fps), columns=['GEOMETRY_HULL_HEAD_SIZE_SPEARMAN_250', 'GEOMETRY_HULL_HEAD_BODY_SIZE_SPEARMAN_500', 'GEOMETRY_HULL_HEAD_SIZE_SPEARMAN_1000', 'GEOMETRY_HULL_HEAD_SIZE_SPEARMAN_2000'])
            hull_tail_length_correlations = pd.DataFrame(Statistics.sliding_spearman_rank_correlation(sample_1=results['GEOMETRY_FRAME_HULL_LENGTH'].values.astype(np.float32), sample_2=results['GEOMETRY_FRAME_TAIL_LENGTH'].values.astype(np.float32), time_windows=TIME_WINDOWS, fps=fps), columns=['GEOMETRY_HULL_TAIL_LENGTH_SPEARMAN_250', 'GEOMETRY_HULL_TAIL_LENGTH__SPEARMAN_500', 'GEOMETRY_HULL_TAIL_LENGTH_SPEARMAN_1000', 'GEOMETRY_HULL_TAIL_LENGTH_SPEARMAN_2000'])
            results = pd.concat([results, upper_lower_body_size_correlations, hull_head_correlations, hull_tail_length_correlations], axis=1)
            ...
            results['MOVEMENT_FRAME_SUMMED'] = results['MOVEMENT_FRAME_NOSE'] + results['MOVEMENT_FRAME_CENTER'] + results['MOVEMENT_FRAME_TAILBASE'] + results['MOVEMENT_FRAME_TAILTIP'] + results['MOVEMENT_FRAME_TAILTIP']
            ...
            for time, bp in product(TIME_WINDOWS, [NOSE, CENTER, 'TAILTIP']):
            ...
            results = pd.concat([results, p_df], axis=1).fillna(-1)
            write_df(df=results.astype(np.float32), file_type=self.file_type, save_path=save_path)

    ...
    for importer, name, _ in pkgutil.iter_modules():
        if name == module_name:
            try:
                ...
                module_inner = loader.load_module(name)
                module_path = os.path.dirname(module_inner.__file__)
                folder_name = os.path.basename(module_path)
                modules[folder_name] = module_inner
            except ImportError:
                pass  # 如果模块加载失败，则跳过
    return modules


inference_modules = find_modules(CM_MODEL_WRAPPER_NAME)
ModelWrapper = []
ModelWrappers = {}

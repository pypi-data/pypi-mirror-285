# Example without circular import using dependency injection
...
class MOPInferenceWrapper:
    def __init__(self, base_model_wrapper_init) -> None:
        self.model_wrapper = base_model_wrapper_init

    def init(self, model_root: str) -> None:
        self.model_wrapper.init(model_root)

    def run(self, item: Dict, triggered_by_mop) -> Dict:
        from add_core_mop.base_model_wrapper import MopInferenceInput  # import at use
...
class WrapModel(BaseModel):
    def predict(self, items: List[Any]) -> List[Any]:
        if inference_wrapper:
            ...
            raise Exception("Inference wrapper not initialized")
...
def mop_init(model_root, dynamic_batch_args: None):
    ...
    p = Path(model_root)
    is_models_folder = p.is_dir()

    if is_models_folder:
        for child in p.iterdir():
            if child.is_dir():
                model_name = child.stem
                wrapper = MOPInferenceWrapper(ModelWrapper())
                wrapper.init(str(child))
                inference_wrappers[model_name] = wrapper
                print(f"Initialized wrapper for model: {model_name}")
            inference_wrapper.init(str(child))
            inference_wrappers[child.stem] = inference_wrapper
    else:
...
Parameters:
    raw_data - raw input data to perform inference
...
def mop_run(raw_data: any, is_mop_triggered: bool = False, **kwargs) -> any:
    ...
    if isinstance(raw_data, dict) and 'holder_type' in raw_data:
        ...
        if holder_type in inference_wrappers:
            inference_wrapper = inference_wrappers[holder_type]
            raw_data = raw_data['data']
        else:
            raise Exception(f"No wrapper found for holder type '{holder_type}'")
    else:
        print("raw_data is not a dictionary or does not contain 'holder_type' key")

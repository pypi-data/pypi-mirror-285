...
def find_and_import_module(module_name, file_path):
...
# 遍历基础路径下的所有子目录
for subdir in next(os.walk(base_path))[1]:  # 获取所有子目录
    file_path = os.path.join(base_path, subdir, CM_MODEL_WRAPPER_NAME + '.py')
    if os.path.isfile(file_path):
        try:
            modules[subdir] = find_and_import_module(subdir + '.' + CM_MODEL_WRAPPER_NAME, file_path)

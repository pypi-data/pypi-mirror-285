...
def print_all_directories(current_here_directory):
    try:
        entries = os.listdir(current_here_directory)

        directories = [entry for entry in entries if os.path.isdir(os.path.join(current_here_directory, entry))]

        print(f"当前目录 ({current_here_directory}) 下的所有子目录：")
...
print_all_directories(current_directory)

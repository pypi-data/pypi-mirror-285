...
def check_model_directory(directory):
    ...
    print(f"目录 {directory} 下的所有子目录：")
    for d in directories:
        print(d)

    for d in directories:
        sub_directory = os.path.join(directory, d)
        sub_directories = [sd for sd in os.listdir(sub_directory) if os.path.isdir(os.path.join(sub_directory, sd))]
        print(f"目录 {sub_directory} 下的所有子目录：")

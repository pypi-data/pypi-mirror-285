...
from typing import Dict, List, Set, Tuple, Any, cast, Optional, Union, Callable
...
def _safe_remove_dashes(yaml_text: str) -> str:
    ...
    for i, line in enumerate(lines):
        ...
        if trimmed.startswith("---"):
            ...
            actual_line = re.sub(r"^(\s*?)---", r"\1   ", line, 1)
...
def _parse_role(
    directory: Path, parse_values: bool = False  # pylint: disable=unused-argument
...
def _parse_py(file: Path, parse_values: bool = False) -> List[Dict[str, Any]]:  # pylint: disable=unused-argument
    if not file.parent:
        return []
    plugin_type_dict = {
        "library": "module",
...
def parse_unknown_ansible_artifact(
    ...
    if str(path.absolute()) in exclude_paths:
        ...
        parsed_ansible_artifacts_plugins += _parse_py(path, parse_values)
    elif path.is_dir():
        if _is_role(path):
            parsed_role, yaml_error = _parse_role(path, parse_values)
...
def parse_ansible_artifacts(
    ...
    """
    paths_to_exclude = set(str(x.absolute()) for x in exclude_paths) if exclude_paths else set()
    parsed_ansible_artifacts_tasks = []
    parsed_ansible_artifacts_playbooks = []
    parsed_ansible_artifacts_blocks = []
    parsed_ansible_artifacts_dynamic_inventories = []

...
# TODO This Recipient Class, why not to create user_external_generic_cruid object and remove the inheritance?
class Recipient:
    def __init__(self, contact_id: int = None, person_id: int = None, user_id: int = None, profile_id: int = None,
        ...
        # TODO This Recipient Class, why not to create user_external_generic_cruid object?
    ...
    def get_usernames(self, system_id: int) -> list[str]:
        ...
        GC = GenericCRUD(default_schema_name="user_external")
        ...
        if self.__person_id:
            custom_condition += f" AND person_id = {self.__person_id}"

        if custom_condition == "system_id = 2":
            return []
        usernames = GC.select_multi_value_by_where(schema_name="user_external",

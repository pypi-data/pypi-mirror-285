...
class CartModelEx(CartModel):
    ...
    def get_cart_list(self, shop_id, begin_time, end_time):
        """
        :description: 获取时间段内购物车列表
        :last_editors: KangWenBin
        """
        sql = "SELECT a.user_code,a.buy_count,a.add_time FROM cart_tb a JOIN goods_tb b ON a.goods_id = b.id WHERE b.shop_id = %s and a.add_time >= %s and a.add_time < %s"
        params = (shop_id, begin_time, end_time)

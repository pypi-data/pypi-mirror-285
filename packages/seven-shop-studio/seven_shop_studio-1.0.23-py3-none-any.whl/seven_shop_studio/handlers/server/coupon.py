...
:LastEditTime: 2024-07-11 14:18:32
    ...
    def get_async(self, *args, **kwargs):
        ...
        if sel_status:
            if sel_status == "0": # 未发布
                ...
                condition += " and a.status = 1 and begin_time > %s "
                param_list.append(TimeHelper.get_now_timestamp())
            elif sel_status == "2": # 进行中
                condition += " and a.status = 1 and begin_time <= %s and end_time >= %s "
                param_list.append(TimeHelper.get_now_timestamp())
                param_list.append(TimeHelper.get_now_timestamp())
            elif sel_status == "3": # 已结束
                condition += " and a.status = 1 and end_time < %s "
                param_list.append(TimeHelper.get_now_timestamp())

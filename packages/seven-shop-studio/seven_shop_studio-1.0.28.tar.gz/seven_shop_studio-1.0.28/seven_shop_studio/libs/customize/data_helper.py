...
class DataHelper:
    ...
    def get_shop_data(self,shop_id,act_id,begin_time,end_time):
        ...
        # 购物车数据
        cart_list = CartModelEx().get_cart_list(shop_id,act_id,begin_time,end_time)
        if cart_list:
    ...
    def get_user_total_price(self, user_code, act_id, shop_id):
        ...
        :last_editors: KangWenBin
        """        
        user_price = round(OrderModel().get_dict(where="shop_id = %s and pay_time > 0 and act_id = %s and user_code = %s",params=[shop_id,act_id,user_code],field="ifnull(sum(real_pay_price),0) as total_price")["total_price"],2)
        return user_price

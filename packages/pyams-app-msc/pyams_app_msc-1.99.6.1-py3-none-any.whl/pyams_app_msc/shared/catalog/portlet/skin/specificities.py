...
from pyams_app_msc.feature.planning.interfaces import ISession, VERSION_INFO, VERSION_INFO_LABEL
...
class CatalogEntrySpecificitiesPortletRenderer(PortletRenderer):
    ...
    def get_version(self, session):
        if not session.version:
            return None
        translate = self.request.localizer.translate
        return translate(VERSION_INFO_LABEL.get(VERSION_INFO(session.version), _("(undefined)")))

...
from pyams_utils.registry import get_utility, get_pyramid_registry, query_utility
...
@implementer(IUserProfileCleanerTask)
...
def handle_registered_profile(event):
    ...
    task = UserProfileCleanerTask(principal_id)
    ...
    scheduler_info.start_date = tztime(datetime.utcnow()) + timedelta(days=configuration.activation_delay)

...
def create_timeline(max_time, dataframe, event_details):
    ...
    shortened_history = dataframe.loc[(dataframe["Product"] != 'oaf') & (dataframe["Product"] != 'dyfi') & (dataframe["Product"] != 'phase-data') & (dataframe["Elapsed (min)"] < max_time)] # not including dyfi or oaf products in timeline, not including anything done 4 days later

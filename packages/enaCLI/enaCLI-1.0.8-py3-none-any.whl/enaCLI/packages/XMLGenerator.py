...
class XML_generator:
    ...
    def generate_project_xml(result_directory,args):
        ...
        for i in range(len(manifestFile[head[0]])):
            if '#' not in str(manifestFile[head[0]][i]):
                ...
                if args.centerName :
                    project = etree.SubElement(project_set, 'PROJECT', alias=manifestFile['alias'][i], center_name=args.centerName)
                else:
        ...
        for i in range(len(manifest)):
            ...
            if not pd.isna(manifest['tax_id'][i]):
                ...
                if '#' not in str(manifest['tax_id'][i]) :
                    ...
                    if args.centerName :
                        sample = etree.SubElement(sample_set, 'SAMPLE', alias=str(manifest['sample_alias'][i]) , center_name = args.centerName)
                    else:
    ...
    def antibiogram_generate_set_xml(result_directory, args):
        ...
        if args.centerName :
            analysis = etree.SubElement(analysis_set, 'ANALYSIS', alias=args.alias , center_name = args.centerName )
        else: 

...
class Index:
        ...
        if files and len(files) > 0:
            ...
            try:
                ...
                response = requests.post(
                    f"{SERVER_ADDRESS}/v0/indexes/{self.index_name}/upload",
    ...
    def search(
        self, query_text: str, max_results: Optional[int] = None, metadata_filter: Optional[dict] = None, specificity: Optional[float] = None
    ) -> List[SearchResult]:
        """
        ...
        specificity: Optional[float] = None
            An optional measure of how specific the search results should be, defaults to 1.0
            Specificity is a number between 0 and 1 where 0.0 would return the least specific results i.e. the entire relevant document and 1.0 would return the most specific results i.e. only the relevant chunk.
        ...
        ```
        """
        url = f"{SERVER_ADDRESS}/v0/indexes/{self.index_name}/search"
        headers = {"x-api-key": self._api_key}
    ...
    def get_upload_status(self, document_keys: List[str]) -> List[UploadTrackingResult]:
        ...
        # send http request
        response = requests.post(
            f"{SERVER_ADDRESS}/v0/indexes/{self.index_name}/upload_status",
            headers={"x-api-key": self._api_key},
    ...
    def delete_documents(self, document_keys: List[str]) -> bool:
        ...
        try:
            response = requests.post(
                f"{SERVER_ADDRESS}/v0/indexes/{self.index_name}/documents/delete",
                headers={"x-api-key": self._api_key},
                json={"document_keys": document_keys},
            )
            if response.status_code == 200:
    ...
    def list_documents(self) -> List[str]:
        ...
        try:
            response = requests.get(
                f"{SERVER_ADDRESS}/v0/indexes/{self.index_name}/documents",
                headers={"x-api-key": self._api_key},
            )
            if response.status_code == 200:
    ...
    def delete_document_metadata(self, document_key: str) -> bool:
        ...
        try:
            response = requests.delete(
                f"{SERVER_ADDRESS}/v0/indexes/{self.index_name}/{document_key}/metadata",
                headers={"x-api-key": self._api_key}
            )

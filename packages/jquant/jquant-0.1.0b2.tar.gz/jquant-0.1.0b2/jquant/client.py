...
class PlatformClient:
    ...
    def _subscribe_request(
        self, method: str, req: message.Message
    ) -> pb.SubscribeRequest:
        any = anypb.Any()
        any.Pack(req)
    ...
    def subscribe_tick(
        ...
        self.subscribe("ticker", req, handler)
    ...
    def subscribe_kline(
        ...
        self.subscribe("kline", req, handler)
    ...
    def subscribe_order(
        ...
        self.subscribe("order", req, handler)
    ...
    def subscribe(
        ...
        response_iterator = self._stream.Subscribe(
            iter_from_queue(q), metadata=self.metadata
        )
        # Receive responses
    ...
    def get_kline(
        ...
        - start_timestamp (int): 开始时间戳(毫秒)
        - end_timestamp (int): 结束时间戳(毫秒)
        - offset (int): 偏移量(默认为0)
        - limit (int): 限制返回的数量(默认为100)
    ...
    def get_position(self, platform: str, **kwargs) -> pb.Position:
        """获取持仓
        Args:
            platform (str): 平台
            **kwargs: 其他参数

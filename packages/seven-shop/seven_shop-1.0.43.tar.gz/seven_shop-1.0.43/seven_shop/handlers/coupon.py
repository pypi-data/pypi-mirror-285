...
:LastEditTime: 2024-07-17 17:37:28
    ...
    def post_async(self):
        ...
        try:
            if coupon_conn.update_coupon_inventory(coupon_model["id"]):
                ...
                return self.reponse_json_error("抱歉~来晚啦，优惠券已领完")
        except:
...
class CouponReceiveHandler(SevenBaseHandler):
    ...
    def post_async(self, *args, **kwargs):
        ...
        # 获取可领取的商城优惠券
        coupon_list =  CouponModelEx().get_receive_coupon_list(user_code,act_id,shop_id)
        if not coupon_list:

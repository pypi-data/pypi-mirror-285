...
:LastEditTime: 2024-07-17 10:49:54
...
class UserCartManageHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        ret_list = []
        user_cart_list,has_next = CartModelEx().get_user_cart_page_list(user_code,act_id,shop_id,page_index,page_size)
        if user_cart_list:
            # 数据整理
    ...
    def delete_async(self):
        ...
        if CartModelEx().del_entity(where="user_code = %s and act_id = %s and id in %s",params=[user_code,act_id,cart_id_list]):
...
class CartCountHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        cart_count = CartModelEx().get_user_cart_count(user_code,act_id,shop_id)
        ret_data = {

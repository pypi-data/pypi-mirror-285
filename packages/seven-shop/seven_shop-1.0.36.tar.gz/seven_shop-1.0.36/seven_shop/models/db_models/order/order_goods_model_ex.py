...
:LastEditTime: 2024-07-16 10:23:30
...
class OrderGoodsModelEx(OrderGoodsModel):
    ...
    def get_order_goods_list(self,order_id_list):
        ...
        goods_list = [goods for goods in goods_list if item["refund_status"] != 4]
        if goods_list:
            # 排除已撤销的退款单
    ...
    def get_refund_goods_list(self,order_id,goods_list = []):
        ...
        # 排除退款撤销的商品
        goods_list = [goods for goods in goods_list if goods["refund_status"] != 4]
        # 状态大于0的设置为1
        if goods_list:

...
:LastEditTime: 2024-07-19 15:57:52
    ...
    def post_async(self):
        ...
        try:
            ...
            if self.is_continue_request(f"{user_code}_{act_id}_{shop_id}_order_create",3000):
                return self.reponse_json_error(desc="操作太频繁，请稍后再试")

            # 验证用户是否是黑名单
            user_info = UserHelper.get_user_info(act_id,open_id)
            if not user_info:
                return self.reponse_json_error("无法获取用户信息")
            if user_info["user_state"] == 1:
                # 黑名单无法下单
                return self.reponse_json_error("用户账号异常\r\n请咨询客服处理")

...
:LastEditTime: 2024-07-05 14:56:45
...
class GoodsDetailListHandler(SevenBaseHandler):
    ...
    def post_async(self):
        ...
        shop_id = self.request_params["shop_id"]
        goods_list = GoodsModelEx(context=self).get_dict_list(field="id,goods_short_name as goods_name,goods_remark,goods_main_images as goods_picture,price",where="status=1 and id in %s and shop_id = %s",params=(goods_id_list,shop_id),order_by="sort desc")
        if goods_list:
            for item in goods_list:
                try:
                    item["goods_picture"] = json.loads(item["goods_picture"])[0]

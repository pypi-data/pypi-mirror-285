...
:LastEditTime: 2024-07-18 09:38:52
...
class OrderaddressHandler(SevenBaseHandler):
    ...
    def post_async(self):
        ...
        # 修改订单地址
        if OrderModel().update_table("province_name=%s,city_name=%s,district_name=%s,consignee=%s,phone=%s,address_info=%s","order_id = %s",params=(address_info["province_name"],address_info["city_name"],address_info["district_name"],address_info["consignee"],address_info["phone"],address_info["address_info"],order_id)):
            try:
    ...
    def post_async(self):
        ...
        try:
            ...
            if ret.status_code == 200:
                ...
                if ret["status"] != "0":
                    return self.reponse_json_error("快递单号错误")
        except:
...
class RefundListHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        refund_order_list,is_next = OrderRefundModel().get_dict_page_list(where="user_code = %s and act_id = %s and shop_id = %s",params=[user_code,act_id,shop_id],page_index=page_index,page_size=page_size,field="order_id,refund_order_id,status,goods_refund_price,real_refund_price,postage",order_by="id desc",page_count_mode="next")
...
class LogisticsHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        if ret_json["status"] == "0":
            ...
            return self.reponse_json_error("未查询到物流信息")

...
:LastEditTime: 2024-07-17 11:04:33
...
class OrderHelper:
    ...
    @classmethod
    def order_check(cls,shop_id,buy_list,province_name,user_code,act_id,coupon_id):
        """
        ...
        if coupon_id:
            # 验证优惠券是否可用
            coupon_model = CouponRecordModel().get_dict(where="id = %s and status = 0 and user_code = %s and act_id = %s and begin_time <= %s and end_time > %s and shop_id = %s",params=[coupon_id,user_code,act_id,TimeHelper.get_now_timestamp(),TimeHelper.get_now_timestamp(),shop_id],field="coupon_info,use_price,goods_limit,goods_list")
            if not coupon_model:

...
:LastEditTime: 2024-07-17 10:57:59
...
class CouponHelper:
    ...
    def get_user_grant_coupon(self, user_code, open_id, shop_id, act_id):
        ...
        # 获取当前用户已获取的优惠券
        user_coupon_list = CouponRecordModel().get_dict_list(where="coupon_id in %s and user_code=%s and act_id = %s", params=[coupon_ids, user_code, act_id])
        ...
        if [x for x in coupon_list if 0 in x["grant_type"] or 3 in x["grant_type"]]:
            user_data = self.get_user_info(act_id,open_id)
        
        is_pay = None
        if [x for x in coupon_list if 1 in x["grant_type"] or 2 in x["grant_type"]]:
            order_model = OrderModel().get_dict(where="user_code=%s and act_id = %s and pay_time > 0 and pay_order_id!=''", params=[user_code,act_id])
            if order_model:
                is_pay = True
            else:
                is_pay = False  

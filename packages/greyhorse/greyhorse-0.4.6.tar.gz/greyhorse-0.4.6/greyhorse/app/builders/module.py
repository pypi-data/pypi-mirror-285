...
from ..schemas.module import ModuleConf, ModuleDesc, ControllerFactoryFn, ServiceFactoryFn
...
class ModuleBuilder:
    ...
    def create_module_pass(self, desc: ModuleDesc | None = None) -> Result[Module | None]:
        ...
        for submodule_desc in current_conf.submodules:
            ...
            instance._grant_providers(submodule)
            # noinspection PyProtectedMember
            instance._import_providers(submodule)
    ...
    def _create_controller(
        self, module: Module, conf: IfaceCtrlConf | ResCtrlConf,
        factory: ControllerFactoryFn,
    ) -> Result[Controller]:
        ...
        if isinstance(conf, IfaceCtrlConf):
            assert isinstance(instance, InterfaceController)
            module.add_iface_controller(conf, instance, name=conf.name)
        elif isinstance(conf, ResCtrlConf):
            assert isinstance(instance, ResourceController)
            module.add_res_controller(conf, instance, name=conf.name)

        logger.info(tr('app.builder.ctrl-success').format(name=conf.name or conf.key.__name__))
        return Result.from_ok(instance)
    ...
    def _create_service(
        self, module: Module, conf: IfaceSrvConf | ResSrvConf,
        ...
        if isinstance(conf, IfaceSrvConf):
            assert isinstance(instance, InterfaceService)
            module.add_iface_service(conf, instance, name=conf.name)
        elif isinstance(conf, ResSrvConf):
            assert isinstance(instance, ResourceService)
            module.add_res_service(conf, instance, name=conf.name)

        submodule_key = '.'.join(self._key_stack)
        self._app.register_service(f'{submodule_key}.{conf.name or conf.key.__name__}', instance)

        logger.info(tr('app.builder.service-success').format(name=conf.name or conf.key.__name__))
        return Result.from_ok(instance)
...
class ModuleTerminator:
    ...
    def destroy_module_pass(
        ...
        if parent:
            ...
            # noinspection PyProtectedMember
            parent._grant_providers(current, undo=True)
            # noinspection PyProtectedMember
            parent._import_providers(current, undo=True)

        for srv_conf in current_conf.services:
            self._app.unregister_service(f'{submodule_key}.{srv_conf.name or srv_conf.key.__name__}')

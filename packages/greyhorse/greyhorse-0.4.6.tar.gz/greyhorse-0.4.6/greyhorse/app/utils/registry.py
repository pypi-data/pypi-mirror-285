...
class MutableRegistry[K, V](Registry[K, V], ABC):
    @abstractmethod
...
class DictRegistry[K, V](MutableRegistry[K, V]):
    def __init__(self):
    ...
    def get(self, key: K, name: str | None = None) -> V | None:
        if key in self._registry:
            if name is None:
                if value := self._registry[key].get(''):
                    return value
...
class ScopedMutableRegistry[K, V](MutableRegistry[K, V]):
    def __init__(
        self, factory: Callable[[], MutableRegistry[K, V]],
        scope_func: Callable[[], str],
    ):
        self._factory = factory
        self._scope_func = scope_func
        self._registries: dict[str, MutableRegistry[K, V]] = defaultdict(self._factory)

    def _get_registry(self) -> MutableRegistry[K, V]:
        key = self._scope_func()
        return self._registries[key]

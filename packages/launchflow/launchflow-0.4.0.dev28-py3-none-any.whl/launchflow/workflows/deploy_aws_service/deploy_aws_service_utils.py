...
async def build_docker_image_on_code_build(
    ...
    await _upload_source_tarball_to_s3(
        ...
        artifact_bucket=aws_environment_config.artifact_bucket,
...
async def release_docker_image_to_ecs_fargate(
    ...
    task_definition_name = f"{service_manager.project_name}-{service_manager.environment_name}-{service_manager.service_name}"
    ...
    # Use EC2 client to describe network interfaces with filters. This matches the logic in the terraform module
    response = ec2_client.describe_network_interfaces(
        Filters=[
            {"Name": "tag:aws:ecs:serviceName", "Values": [service_name]},
            {"Name": "tag:aws:ecs:clusterName", "Values": [cluster_name]},
        ]
    )

    # Check if any network interfaces were found
    if "NetworkInterfaces" in response and len(response["NetworkInterfaces"]) > 0:
        # Get the public IP of the first network interface
        network_interface = response["NetworkInterfaces"][0]
        if (
            "Association" in network_interface
            and "PublicIp" in network_interface["Association"]
        ):
            public_ip = network_interface["Association"]["PublicIp"]
            # The public IP is used to construct the URL for the service.
            # TODO: Check if a load balancer is being used and use that URL instead
            return f"http://{public_ip}"
        else:
            raise RuntimeError("No public IP associated with the network interface.")

    else:
        raise RuntimeError("No network interfaces found with the specified tags.")
...
async def promote_aws_service_image(
    ...
    if not promote_local:
        ...
        raise NotImplementedError("Promoting local docker images is not yet supported for AWS services.")

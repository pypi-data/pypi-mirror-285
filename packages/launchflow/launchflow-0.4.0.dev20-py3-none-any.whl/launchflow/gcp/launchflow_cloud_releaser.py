...
@dataclasses.dataclass
class LaunchFlowReleaserInputs(TofuInputs):
    launchflow_service_account: str
    permissions: List[str]


class LaunchFlowCloudReleaser(Resource[LaunchFlowReleaserOutputs]):
    """A resource for creating a service account that can be used to deploy to Cloud Run.
    ...
    releaser = lf.gcp.LaunchFlowCloudReleaser("my-releaser")
    ```
    """
    ...
    async def connect_to_launchflow(self):
        ...
        rich.print(
            f"Releases will be pushed using the service account we created for you: `{outputs.service_account_email}`. You can learn more at: https://docs.launchflow.com/docs/user-guides/github-deployments\n"
        )

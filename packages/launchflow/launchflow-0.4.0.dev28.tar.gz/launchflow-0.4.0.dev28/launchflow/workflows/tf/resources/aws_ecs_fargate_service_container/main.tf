...
data "aws_network_interface" "first_interface" {
  count = length(data.aws_network_interfaces.interface_tags.ids) > 0 ? 1 : 0
  id    = data.aws_network_interfaces.interface_tags.ids[0]
}

output "public_ip" {
  value = data.aws_network_interface.first_interface[0].association[0].public_ip
}
output "aws_arn" {

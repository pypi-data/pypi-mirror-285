...
async def set(
    ...
    if resource.product == ResourceProduct.GCP_SECRET_MANAGER_SECRET:
        ...
        async def add_version_fn():
            env_info = await EnvironmentManager(
                project_name=launchflow.project,
                environment_name=environment,
                backend=config.launchflow_yaml.backend,
            ).load_environment()
            if env_info.aws_config is None:
                raise exceptions.AWSConfigNotFound(environment)
            secret = SecretsManagerSecret(name=resource_name)
            secret.add_version(secret_value, region=env_info.aws_config.region)

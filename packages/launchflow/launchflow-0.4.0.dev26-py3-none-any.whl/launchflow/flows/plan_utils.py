...
async def lock_plans(
    *plans: Plan,
    ...
    if local_plans:
        lock_tasks.extend(_build_lock_tasks_recursively(local_plans))
    if remote_plans:
        lock_tasks.append(_lock_remote_plans(remote_plans, environment_manager))

    lock_results: List[Union[Optional[Lock], Exception]] = []
    try:
        lock_results = await asyncio.gather(*lock_tasks, return_exceptions=True)
        exceptions_list = []

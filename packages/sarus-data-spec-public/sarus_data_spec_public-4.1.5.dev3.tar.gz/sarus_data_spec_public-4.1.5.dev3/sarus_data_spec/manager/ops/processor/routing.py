...
try:
    from sarus_data_spec.manager.ops.processor.standard.select_sql.select_sql import (
        SelectSQL,
...
try:
    from sarus_data_spec.manager.ops.processor.standard.select_sql.dp_select_sql import (
        DPSelectSQL,
    ...
    logger = logging.getLogger(__name__)
    logger.info("Transforms: DP SelectSQL not available.")

try:
    from sarus_data_spec.manager.ops.processor.standard.select_sql.pup_select_sql import (
        PUPSelectSQL,
        PUPSelectSQLStaticChecker,
    )
except ModuleNotFoundError:
    logger = logging.getLogger(__name__)
    logger.info("Transforms: PUP SelectSQL not available.")
...
def get_dataset_op(
    ...
    elif transform.spec() == "pup_select_sql":
        return PUPSelectSQL, PUPSelectSQLStaticChecker

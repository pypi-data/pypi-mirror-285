...
@click.option("--vault", type=str, required=True, help="ID of the Vault to use for the units. Should be SSH-KEY based")
@click.option("--vault-pub-key", type=str, required=True, help="Pub ssh key corresponding to the vault used.")
@click.option("--ignore-target-subnet", type=bool, is_flag=True, help="ID of the Vault for windows VMs")
def migrate(ws_portal, target_ws_portal, migrate_portal, source_cloudspace, target_cloudspace, source_vm_id, jwt, target_jwt, migrate_login, migrate_passwd, vault, vault_pub_key, customer_id, target_customer_id, ignore_target_subnet) -> None:
    ...
    target_vms = create_target_vms(migrator, source_vms)
...
def create_target_vms(migrator: "Migrator", source_vms):
        ...
        for target_vm in target_vms:
            if source_vm["name"] == target_vm["name"]:
                if target_vm["image_id"] not in (migrator.ubuntu_server_image_in_target["image_id"], migrator.windows_server_image_in_target["image_id"]) \
                    ...
                    or target_vm["storage"] != source_vm["storage"]:

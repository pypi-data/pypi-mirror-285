...
def _performApiKeyLogin(api: MbApi, region: Optional[str]):
  ...
  if '/' in workspaceName:
    errMsg = "Invalid MB_WORKSPACE_NAME. Workspace name must not contain slashes."
    import re
    tryNameMatch = re.search('/w/(([a-z0-9]-?)*[a-z0-9])/?', workspaceName)  # workspaceNameSchema
    if tryNameMatch:
      errMsg = f"{errMsg} Try setting MB_WORKSPACE_NAME to '{tryNameMatch[1]}'"
    raise UserFacingError(errMsg)

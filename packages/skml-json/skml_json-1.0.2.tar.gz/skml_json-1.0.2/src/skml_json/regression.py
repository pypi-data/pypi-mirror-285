...
from sklearn.ensemble import RandomForestRegressor, GradientBoostingRegressor, _gb
from sklearn.neural_network import MLPRegressor
from sklearn.tree._tree import Tree
from sklearn.svm import SVR
from sklearn import dummy
from skml_json import csr
import numpy as np
import scipy as sp
...
def serialize_gradient_boosting_regressor(model):
    ...
    if isinstance(model.loss_, _gb.LeastSquaresError):
        serialized_model['loss_'] = 'ls'
    elif isinstance(model.loss_, _gb.LeastAbsoluteError):
        serialized_model['loss_'] = 'lad'
    elif isinstance(model.loss_, _gb.HuberLossFunction):
        serialized_model['loss_'] = 'huber'
    elif isinstance(model.loss_, _gb.QuantileLossFunction):
...
def deserialize_gradient_boosting_regressor(model_dict):
    ...
    if model_dict['loss_'] == 'ls':
        model.loss_ = _gb.LeastSquaresError(1)
    elif model_dict['loss_'] == 'lad':
        model.loss_ = _gb.LeastAbsoluteError(1)
    elif model_dict['loss_'] == 'huber':
        model.loss_ = _gb.HuberLossFunction(1)
    elif model_dict['loss_'] == 'quantile':
        model.loss_ = _gb.QuantileLossFunction(1)

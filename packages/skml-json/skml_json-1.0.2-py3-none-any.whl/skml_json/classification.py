...
from sklearn.ensemble import RandomForestClassifier, GradientBoostingClassifier, _gb
from sklearn.naive_bayes import BernoulliNB, GaussianNB, MultinomialNB, ComplementNB
from sklearn.neural_network import MLPClassifier
from sklearn.preprocessing import LabelBinarizer
from skml_json import regression, csr
...
def serialize_gradient_boosting(model):
    ...
    if isinstance(model.loss_, _gb.BinomialDeviance):
        serialized_model['loss_'] = 'deviance'
    elif isinstance(model.loss_, _gb.ExponentialLoss):
        serialized_model['loss_'] = 'exponential'
    elif isinstance(model.loss_, _gb.MultinomialDeviance):
...
def deserialize_gradient_boosting(model_dict):
    ...
    if model_dict['loss_'] == 'deviance':
        model.loss_ = _gb.BinomialDeviance(model.n_classes_)
    elif model_dict['loss_'] == 'exponential':
        model.loss_ = _gb.ExponentialLoss(model.n_classes_)
    elif model_dict['loss_'] == 'multinomial':
        model.loss_ = _gb.MultinomialDeviance(model.n_classes_)

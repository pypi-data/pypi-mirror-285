def reannotate(configuration, slivar, output, chunksize=100000, keep_all=False, progress=False, verbose=False):
    import yaml
    import pandas as pd
    ...
    if progress:
        if slivar.endswith('.gz'):
            ...
            len_input = subprocess.run(['wc','-l',slivar], stdout=subprocess.PIPE, text=True)
    ...
    with pd.read_csv(slivar, sep='\t', dtype="string", chunksize=chunksize) as reader:
        for df in tqdm(reader, desc='chunks analyzed', unit='chunk', total=nb_chunks+1, disable=(not progress)):
            if verbose:
...
def main(args=None):
    ...
    reannotate(configuration=args.configuration, slivar=args.slivar, output=args.output, chunksize=args.chunksize, keep_all=args.keep_all, progress=args.progress, verbose=args.verbose)

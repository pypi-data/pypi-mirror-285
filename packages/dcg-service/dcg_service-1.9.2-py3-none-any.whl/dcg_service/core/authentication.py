...
def jwt_get_vendor_from_payload_handler(payload):
    """
    Override this function if permissions is formatted differently in payload
    """
    return payload.get('vendor', '') or None


ADMIN = 1


def authenticate_credentials(payload, header):
    """
    ...
    if not user:
        vendor = jwt_get_vendor_from_payload_handler(payload)
        if not vendor and header.get("application_code") != "WORDPRESS":
            raise AuthException(_('Invalid Authorization header. No user found.'))
        user = {
            "id": ADMIN,
            "user_type": payload.get("role", None),
        }
...
def authenticate(token, header=None):
    """
    Returns a two-tuple of `User` and token if a valid signature has been
    supplied using JWT-based authentication.  Otherwise, returns `None`.
    """
    if header is None:
        header = {"application_code": "DCG"}
    if not token:
        raise AuthException(
    ...
    user = authenticate_credentials(payload, header)
    return user

...
class Chrome(_Chrome):
    ...
    def __init__(self, **kwargs):
        ...
        extension_dir = pathlib.Path(
            user_data_dir, 'Extensions', extension_name
        )
    ...
    def build(self):
        ...
        if user_data_dir.exists():
            raise UserDataDirExistsError(
                f'The "{user_data_dir}" already exists.'
            )
        command = [
        ...
        if not cookies_file.exists():
            raise UserDataDirBuildError(
                'User Data dir did not appear within the timeout period.'
            )
        os.chmod(
            cookies_file, stat.S_IWUSR | stat.S_IRUSR
        )
        start = time.time()
        while time.time() - start < 3:
            if all([os.access(cookies_file, i)
                    for i in (os.W_OK, os.R_OK)]):
                break
            time.sleep(0.125)
        if all([os.access(cookies_file, i)
                for i in (os.W_OK, os.R_OK)]):

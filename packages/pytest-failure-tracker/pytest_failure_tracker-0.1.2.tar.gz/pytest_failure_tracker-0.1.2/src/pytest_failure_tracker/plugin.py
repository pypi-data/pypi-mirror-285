...
def pytest_configure(config):
    config.addinivalue_line("markers", "track_failures: mark test to have its failures tracked")

def pytest_addoption(parser):
    parser.addoption("--track-failures", action="store_true", help="Track test failures across runs")
...
def pytest_runtest_makereport(item, call):
    ...
    if item.config.getoption("track_failures"):
        ...
        if test_id not in item.session.results:
            item.session.results[test_id] = {"passes": 0, "failures": 0, "skips": 0, "last_failure": None}
...
def pytest_terminal_summary(terminalreporter, exitstatus, config):
    if config.getoption("track-failures"):

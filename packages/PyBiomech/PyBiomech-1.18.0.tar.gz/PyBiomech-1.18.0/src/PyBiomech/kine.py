...
def femurPoseISB(mkrs, s='R'):
    """Calculate roto-translation matrix from femur (ISB conventions) to
    laboratory reference frame.
    ...
    # Create versors
    Ofem = HJC.copy()
    Yfem = getVersor(HJC - 0.5 * (ME + LE))
    if s == 'R':
        Xfem = getVersor(np.cross(Yfem, LE - ME))
    else:
        Xfem = getVersor(np.cross(Yfem, ME - LE))
    Zfem = getVersor(np.cross(Xfem, Yfem))

    # Create rotation matrix from shank reference frame to laboratory reference frame
    R = np.array((Xfem.T, Yfem.T, Zfem.T))   # 3 x 3 x N
    R = np.transpose(R, (2,1,0))  # N x 3 x 3
    ...
    **kwargs
    ...
    if funInput == 'jointR':
        ...
        if kwargs:
            ...
            angles = np.apply_along_axis(R2anglesFun, 1, Rvect)

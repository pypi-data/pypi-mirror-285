...
def patellaOR(mkrs, s='R'):
    ...
    K = getVersor(PPCR - PDCR)
    if s == 'R':
        J = getVersor(np.cross(K, PLE - PMFLc))
    else:
        J = -getVersor(np.cross(K, PLE - PMFLc))
    I = getVersor(np.cross(J, K))
...
register = {}
register['femurOR'] = femurOR
register['tibiaOR'] = tibiaOR
register['patellaOR'] = patellaOR

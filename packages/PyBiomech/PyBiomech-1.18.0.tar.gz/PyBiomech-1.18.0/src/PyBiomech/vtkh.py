...
def createPolynomial(pts, k=3, retU=False, weights=None, xy='auto'):
    # can also add: https://docs.scipy.org/doc/scipy/reference/generated/scipy.odr.polynomial.html#scipy.odr.polynomial
    x, y = pts[:,0], pts[:,1]
    flipped = False
    if xy == 'auto':
       if np.argmax(x) != 0 or np.argmin(x) != (pts.shape[0]-1):
        # if np.argmax(x) != 0:
            x, y = y, x
            flipped = True
    elif xy == 'xy':

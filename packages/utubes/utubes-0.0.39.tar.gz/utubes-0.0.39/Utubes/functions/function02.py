...
class Extractors:
    ...
    async def extract04(anames, cnames, exeson):
        onames = await Extractors.extract06(cnames)
        oxoexe = await Extractors.extract07(cnames)
        exeoxo = exeson if exeson else Scripted.DATA06
        execxc = oxoexe.replace(".", "") if oxoexe else exeoxo
        return Scripted.DATA10.format(cnames, execxc) if cnames else Scripted.DATA07.format(onames, exeoxo)

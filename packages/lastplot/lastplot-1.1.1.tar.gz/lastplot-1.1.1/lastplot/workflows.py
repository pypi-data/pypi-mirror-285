...
def data_workflow(file_path, data_sheet, mice_sheet, output_path, control_name, experimental_name):
    """
    ...
    statistics = statistics_tests(df_clean=df_clean, control_name=control_name, experimental_name=experimental_name)
    df_final = z_scores(df_clean=df_clean, statistics=statistics)
    # df_final = lipid_selection(df_mid, invalid_df, control_name=control_name)

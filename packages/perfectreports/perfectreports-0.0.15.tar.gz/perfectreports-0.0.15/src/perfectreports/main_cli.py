    ...
    if (len(failed_df) > 0):
        ...
        if (len(latest)) > 0:
            ...
            remove_old_bugs(JIRA_BASE_URL, JIRA_API_TOKEN,
                            JIRA_USERNAME, PROJECT_KEY, BUG_LABEL, jira)
...
def remove_old_bugs(JIRA_BASE_URL, JIRA_API_TOKEN, JIRA_USERNAME, PROJECT_KEY, BUG_LABEL, jira):
    JQL_BUG_LABEL = 'project = ' + PROJECT_KEY + \
        ' AND type = Bug AND status = "To Do" AND labels = ' + BUG_LABEL
    total_issues = jira.get_jira_issues(
    ...
    if (len(total_issues)) > 0:
        for issue in total_issues:
            jira.delete_jira_bug(JIRA_BASE_URL, JIRA_API_TOKEN,

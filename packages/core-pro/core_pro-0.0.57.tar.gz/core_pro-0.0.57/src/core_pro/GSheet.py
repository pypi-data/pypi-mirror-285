...
from src.core_pro.config import GoogleAuthentication
...
class Sheet(GoogleAuthentication):
    ...
    def google_sheet_into_df(
        ...
        if use_polars:
            ...
            return pl.DataFrame(padded_data, schema=result['values'][0])
        else:
...
pos_id = 'L'
sheet_range = f'A:{pos_id}'
sheet = Sheet(sh)
sh_name = 'Form Responses 1'
range_update = f'{sh_name}!{sheet_range}'

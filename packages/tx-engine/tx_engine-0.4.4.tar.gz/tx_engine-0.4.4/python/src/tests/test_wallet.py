...
class WalletTest(unittest.TestCase):
    ...
    def test_sign_tx_with_2_inputs(self):
        f1 = ""
        ...
        fund1_tx = Tx.parse(bytes.fromhex(f1))
        fund2_tx = Tx.parse(bytes.fromhex(f2))
        ...
        #  fn new(prev_tx: [u8; 32], prev_index: u32, script: &[u8], sequence: u32) -> Self
        vins = [
            TxIn(prev_tx=fund1_tx.id(), prev_index=1),
            TxIn(prev_tx=fund2_tx.id(), prev_index=1)
        ]
        ...
        # fn sign_tx(&mut self, index: usize, input_pytx: PyTx, pytx: PyTx) -> PyResult<PyTx>
        new_tx = wallet.sign_tx(0, fund1_tx, tx)
        new_tx2 = wallet.sign_tx(1, fund2_tx, new_tx)

        self.assertNotEqual(new_tx2, new_tx)

        txsecond = Tx(version=1, tx_ins=vins, tx_outs=vouts, locktime=0)
        new_tx3 = wallet.sign_tx(0, fund1_tx, txsecond)
        new_tx4 = wallet.sign_tx(1, fund2_tx, new_tx3)

        self.assertEqual(new_tx2, new_tx4)

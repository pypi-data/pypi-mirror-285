...
class Pipeline:
    def __init__(self, name, QA_flag=None):
        ...
        dual_logger = DualLogger(self.log_stream)
        logging.basicConfig(level=logging.INFO, handlers=[dual_logger], format='%(asctime)s - %(message)s')

        if QA_flag is None:
            logging.info("NO QA_FLAG PASSED. DEFAULTING TO TRUE.")
            self.QA = True
    ...
    def print_initial_summary(self):
        ...
        logging.info(f"QA Mode: {'Enabled' if self.QA else 'Disabled'}")
        logging.info(f"Total Tasks: {len(self.tasks)}")
        logging.info(f"Total Stages: {len(set(task.stage for task in self.tasks))}")
        logging.info("Tasks by Stage:")
    ...
    def send_email(self, subject, body, recipients=None):
        ...
        log_attachment.set_payload(self.log_stream.getvalue().encode())
        encoders.encode_base64(log_attachment)
        log_attachment.add_header('Content-Disposition', f'attachment; filename="pipeline_log.txt"')
    ...
    def create_temp_table(self, query_job, temp_table_name):
        try:
            ...
            if destination is None:
                logging.info(f"No destination table created for job: {query_job.job_id}")
                return None
            ...
            if table.table_type != 'TEMPORARY':
                ...
                logging.info(f"Temporary table {temp_table_name} created.")
            else:
                logging.info(f"Skipping expiration update for anonymous table {destination}")
    ...
    def log_task_execution(self, task_name, data_size, elapsed_time):
        try:
            if "test" in task_name.lower():
                logging.info(f"Skipping logging for test task: {task_name}")
                return
            ...
            logging.info(f"Logged task execution for task: {task_name}")
        except Exception as e:
            logging.error(f"Failed to log task execution for task: {task_name}. Error: {str(e)}")
        ...
        try:
            ...
            for stage in range(1, max_stage + 1):
                ...
                if stage_tasks and stage not in stages_executed:
                    logging.info(f"Executing stage {stage} with {len(stage_tasks)} task(s).")
            ...
            if not self.errors and not self.halt_execution:
                ...
                if qa_tasks and Task.QA_STAGE not in stages_executed:
                    logging.info(f"Executing stage {Task.QA_STAGE} with {len(qa_tasks)} task(s).")
                    self.execute_stage(qa_tasks)
                    stages_executed.add(Task.QA_STAGE)
            else:
                logging.info("Skipping QA tasks due to errors in regular tasks.")
            ...
            if self.errors:
                logging.info("Errors encountered during pipeline execution:")
        ...
        try:
            ...
            if not task.query_definition:
                if not task.optional:
                    ...
                    logging.info(f'WARNING: {task.name} does not have a defined query. Skipping optional task...')
            ...
            logging.info(f"Estimated data size for task '{task.name}': {estimated_data_size/1000000000:.2f} Gb")

            estimated_run_time = self.estimate_run_time(estimated_data_size, task.name)
            if estimated_run_time:
                logging.info(f"Estimated run time for task '{task.name}': {estimated_run_time:.2f} seconds")
    ...
    def execute_qa_task(self, task):
        logging.info(f"Executing QA task '{task.name}'")
        try:
            ...
            if condition_result:
                logging.info(f"QA check passed for task '{task.name}'")
                self.update_status(task, 'Completed')
            else:
                logging.info(f"QA check failed for task '{task.name}'")
                if task.optional:
            ...
            if task.optional:
                ...
                self.update_status(task, f'Failed: {str(e)}')
            logging.error(f"QA task '{task.name}' failed with exception: {str(e)}")
    ...
    def estimate_run_time(self, estimated_data_size, task_name):
        ...
        if df_task.empty:
            logging.info(f"No historical data available to estimate run time for task '{task_name}'")
            return None

...
class Pipeline:
    def __init__(self, name, QA_flag=None):
        ...
        if QA_flag is None:
            self.dual_logger.log_and_print(("NO QA_FLAG PASSED. DEFAULTING TO TRUE."), 'info')
            self.QA = True
    ...
    def print_initial_summary(self):
        self.dual_logger.log_and_print(("\nInitial Pipeline Summary"), 'info')
        self.dual_logger.log_and_print(("========================"), 'info')
        self.dual_logger.log_and_print((f"Pipeline Name: {self.name}"), 'info')
        self.dual_logger.log_and_print((f"QA Mode: {'Enabled' if self.QA else 'Disabled'}"), 'info')
        self.dual_logger.log_and_print((f"Total Tasks: {len(self.tasks)}"), 'info')
        self.dual_logger.log_and_print((f"Total Stages: {len(set(task.stage for task in self.tasks))}"), 'info')
        self.dual_logger.log_and_print(("Tasks by Stage:"), 'info')
        ...
        for stage, tasks in sorted(stages.items()):
            self.dual_logger.log_and_print((f"  Stage {stage}:"), 'info')
            for task in tasks:
                self.dual_logger.log_and_print((f"    - {task.name}"), 'info')
    ...
    def send_email(self, subject, body, recipients=None):
        ...
        try:
            ...
            server.quit()
            self.dual_logger.log_and_print(("Email sent successfully."), 'info')
        except Exception as e:
            self.dual_logger.log_and_print(("Failed to send email:", str(e)), 'info')
    ...
    def execute_query(self, query, temp_table_name=None):
        try:
            ...
            self.dual_logger.log_and_print((f"\nQuery executed in {total_elapsed_time:.2f} seconds."), 'info')
            ...
            stop_event.set()
            elapsed_time_thread.join()
            self.dual_logger.log_and_print((f"TaskError: {e}"), 'error')
            raise e
        except Exception as e:
            stop_event.set()
            elapsed_time_thread.join()
            error_message = str(e).split('\n\n')[0].strip()
            self.dual_logger.log_and_print((f"Failed to execute query: {error_message}"), 'error')
    ...
    def create_temp_table(self, query_job, temp_table_name):
        try:
            ...
            if destination is None:
                self.dual_logger.log_and_print((f"No destination table created for job: {query_job.job_id}"), 'info')
                return None
            ...
            if table.table_type != 'TEMPORARY':
                ...
                self.dual_logger.log_and_print((f"Temporary table {temp_table_name} created."), 'info')
            else:
                self.dual_logger.log_and_print((f"Skipping expiration update for anonymous table {destination}"), 'info')
    ...
    def log_task_execution(self, task_name, data_size, elapsed_time):
        try:
            if "test" in task_name.lower():
                self.dual_logger.log_and_print((f"Skipping logging for test task: {task_name}"), 'info')
                return
            ...
            self.dual_logger.log_and_print((f"Logged task execution for task: {task_name}"), 'info')
        except Exception as e:
            self.dual_logger.log_and_print((f"Failed to log task execution for task: {task_name}. Error: {str(e)}"), 'error')
        ...
        try:
            ...
            for stage in range(1, max_stage + 1):
                ...
                if stage_tasks and stage not in stages_executed:
                    self.dual_logger.log_and_print((f"Executing stage {stage} with {len(stage_tasks)} task(s)."), 'info')
            ...
            if not self.errors and not self.halt_execution:
                ...
                if qa_tasks and Task.QA_STAGE not in stages_executed:
                    self.dual_logger.log_and_print((f"Executing stage {Task.QA_STAGE} with {len(qa_tasks)} task(s)."), 'info')
                    self.execute_stage(qa_tasks)
                    stages_executed.add(Task.QA_STAGE)
            else:
                self.dual_logger.log_and_print(("Skipping QA tasks due to errors in regular tasks."), 'info')

            self.dual_logger.log_and_print(("Pipeline execution completed."), 'info')
        except Exception as e:
            error_message = f"Pipeline Failure: {e}"
            self.errors.append(error_message)
            self.dual_logger.log_and_print(error_message, 'error'))
        finally:
            if self.errors:
                self.dual_logger.log_and_print(("Errors encountered during pipeline execution:"), 'info')
                for error in self.errors:
                    self.dual_logger.log_and_print((error), 'info')
        ...
        try:
            ...
            if not task.query_definition:
                if not task.optional:
                    ...
                    self.dual_logger.log_and_print((f'WARNING: {task.name} does not have a defined query. Skipping optional task...'), 'warning')
            ...
            try:
                ...
                self.dual_logger.log_and_print((f"Failed to translate query for task '{task.name}': {e}"), 'error')
                raise

            estimated_data_size = self.estimate_data_size(task.query)
            self.dual_logger.log_and_print((f"Estimated data size for task '{task.name}': {estimated_data_size/1000000000:.2f} Gb"), 'info')

            estimated_run_time = self.estimate_run_time(estimated_data_size, task.name)
            if estimated_run_time:
                self.dual_logger.log_and_print((f"Estimated run time for task '{task.name}': {estimated_run_time:.2f} seconds"), 'info')
            ...
            try:
                ...
                self.dual_logger.log_and_print((f"{task.name} Completed."), 'info')
            except Exception as e:
    ...
    def execute_qa_task(self, task):
        self.dual_logger.log_and_print((f"Executing QA task '{task.name}'"), 'info')
        try:
            ...
            if condition_result:
                self.dual_logger.log_and_print((f"QA check passed for task '{task.name}'"), 'info')
                self.update_status(task, 'Completed')
            else:
                self.dual_logger.log_and_print((f"QA check failed for task '{task.name}'"), 'info')
                if task.optional:
            ...
            if task.optional:
                ...
                self.update_status(task, f'Failed: {str(e)}')
            self.dual_logger.log_and_print((f"QA task '{task.name}' failed with exception: {str(e)}"), 'error')
    ...
    def estimate_run_time(self, estimated_data_size, task_name):
        ...
        if df_task.empty:
            self.dual_logger.log_and_print((f"No historical data available to estimate run time for task '{task_name}'"), 'info')
            return None
    ...
    def send_completion_email(self):
        ...
        try:
            ...
            self.dual_logger.log_and_print(("Completion email sent successfully."), 'info')
        except Exception as e:
            self.dual_logger.log_and_print((f"Failed to send completion email: {e}"), 'info')

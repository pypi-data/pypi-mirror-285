...
class Pipeline:
    ...
    def print_initial_summary(self):
        ...
        self.logger.log("========================")
        self.logger.log(f"Pipeline Name: {self.name}")
        self.logger.log(f"QA Mode: {'Enabled' if self.QA else 'Disabled'}")
        self.logger.log(f"Total Tasks: {len(self.tasks)}")
        self.logger.log(f"Total Stages: {len(set(task.stage for task in self.tasks))}")
        self.logger.log("Tasks by Stage:")
        ...
        for stage, tasks in sorted(stages.items()):
            self.logger.log(f"  Stage {stage}:")
            for task in tasks:
                self.logger.log(f"    - {task.name}")
    ...
    def execute_regular_task(self, task):
        try:
            ...
            self.errors.append(error_message)
            self.logger.log(error_message, 'error')
            self.logger.log(f"Task details: {task.__dict__}", 'debug')

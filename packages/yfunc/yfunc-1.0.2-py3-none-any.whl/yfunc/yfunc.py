...
class filepath():
    ...
    def db(self, only_read=False, auto_commit=True, auto_close=True, check_same_thread=True) -> 'DBFileHandler':
        return DBFileHandler(
            self.s, 
            only_read=only_read, 
...
class DBFileHandler:
    ...
    def __init__(self, path, only_read=False, auto_commit=True, auto_close=True, check_same_thread=True) -> None:
        import sqlite3
        self.path = path
        self.only_read = only_read
        self.auto_commit = auto_commit
        self.auto_close = auto_close
        self.uri = f"file:{path}?mode={'ro' if only_read else 'rw'}"
        self.con = sqlite3.connect(self.uri, check_same_thread=check_same_thread, uri=True)
...
class json():
    ...
    def to_object(self) -> Any:
        return json.module__json.loads(self.s)

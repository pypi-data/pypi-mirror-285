...
from seekh_data_scraper.openstax_data_scraping.scraper.content import scrape_content, save_content, get_all_chapter_info
...
def main():
    for sitemap_url in SCRAPING_MODULES['openstax']['site_url']:
        ...
        # Scrape chapter info
        chapter_info = get_all_chapter_info(sitemap_url)
        ...
        if not check_file_exists(book_dir, DEFAULT_CONTENT_OUTPUT):
            content_data, summary_urls, _ = scrape_content(url_groups['group_3'])
            save_content(content_data, chapter_info, content_output)
            ...
            _, summary_urls, _ = scrape_content(url_groups['group_3'], scrape=False)

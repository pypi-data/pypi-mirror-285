...
def get_all_chapter_info(url):
    ...
    chapter_info = {}
    summary_titles = soup.find_all('span', class_='styled__SummaryTitle-sc-18yti3s-0')
    
    for summary_title in summary_titles:
        chapter_number = summary_title.find('span', class_='os-number').text.strip()
        chapter_name = summary_title.find('span', class_='os-text').text.strip()
        chapter_info[chapter_number] = chapter_name
...
def scrape_content(urls, scrape=True):
    ...
    return all_data, summary_urls, chapter_info

def save_content(data, chapter_info, output_file):
    rows = []
    for item in data:
        if "data" not in item:
        ...
        chapter_number = main_topic.split('-')[0]
        chapter_name = chapter_info.get(chapter_number, "Unknown Chapter")

    ...
    def split_process(self, bin_size, label, uID):
        if self.geneFile.endswith(".gef"):
            with h5py.File(self.geneFile, "r") as gef_f:
                OffsetX = gef_f["geneExp"][f"bin{bin_size}"]["expression"].attrs[
                    "minX"
                ][0]
                OffsetY = gef_f["geneExp"][f"bin{bin_size}"]["expression"].attrs[
                    "minY"
                ][0]
                ...
                if uID_label_df is not None:
                    for key in uID_label_df.iloc:
                        ...
                        with open(
                            os.path.join(
                                self.gem_path,
                                f"{self.sampleid}.lasso.bin{bin_size}.{label}.gem",
                            ),
                            "w",
                        ) as fg:
                            ...
                            for i in range(0, totalNum, batch):
                                ...
                                self.gene_df = pd.DataFrame(
                                    gef_f["geneExp"][f"bin{bin_size}"]["expression"][
                                        i:endIdx
                                    ]
                                )
                                ...
                                if "exon" in gef_f["geneExp"][f"bin{bin_size}"]:
                                    self.has_exon_ = True
                                    self.gene_df["ExonCount"] = gef_f["geneExp"][
                                        f"bin{bin_size}"
                                    ]["exon"][i:endIdx]
                                    ...
                                    for row in self.gene_df.itertuples():
                                        if 1 == labels[int(row.y)][int(row.x)]:
                                            fg.write(
                                                f"{row.geneID}\t{row.x}\t{row.y}\t{row.MIDCount}\t{row.ExonCount}\n"
                                            )
                                else:
                                    for row in self.gene_df.itertuples():
                                        if 1 == labels[int(row.y)][int(row.x)]:
                                            fg.write(
                                                f"{row.geneID}\t{row.x}\t{row.y}\t{row.MIDCount}\n"
                                            )

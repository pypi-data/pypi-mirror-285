...
class Window:
    ...
    def register_load_handler(self, callback, single=False):
        """
        Registers a load handler callback that will be executed when the window is loaded.

        :param callback: The callback function to be executed when the window loads. It must task a single argument.
        :param single: If True, the callback will be executed only once. If False, the callback will be executed every time the window loads.
        :raises TypeError: If the 'single' parameter is not a boolean.
        """

        if not isinstance(single, bool):
            raise TypeError(f"Expected argument 'single' to be a bool object, instead got '{get_type_name(single)}'")
        
        (self.__load_handlers, self.__single_load_handlers)[single].append(callback)

    def load_handler(self, window):
        """
        Handles the loading of the window, executing registered load handler callbacks.

        :param window: The window instance that has been loaded.
        """
        
        self.__single_load_handlers.append(lambda window: window.show())

        def on_single_load():
            for callback in self.__single_load_handlers:
                callback(window)
            
            window.events.loaded -= on_single_load
        
        def on_load():
            for callback in self.__load_handlers:
                callback(window)

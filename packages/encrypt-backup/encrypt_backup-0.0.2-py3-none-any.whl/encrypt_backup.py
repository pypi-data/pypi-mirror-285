...
def generate_save_rsa_keys(private_key_path, public_key_path, public_exponent=65537, key_size=4096):
    ...
    save_rsa_keys(*generate_rsa_keys(public_exponent, key_size), private_key_path, public_key_path)
...
def compress_and_encrypt_folder(backup_file_path, encrypted_7zip_path, public_key_path, base_path=None):
    ...
    # 使用 AES 加密数据
    encrypted_data = aes_encrypt(aes_key, aes_iv, zip_data)
...
def decrypt_file(source_file_path, output_file_path, private_key_path):
    ...
    # 使用 AES 密钥和 IV 解密数据
    decrypted_data = aes_decrypt(aes_key, aes_iv, encrypted_data)
...
def generate_rsa_keys(public_exponent=65537, key_size=4096):

...
if __name__ == '__main__':
    ...
    # cont = retireve_batch_results(client, batch_id)
    # print(json.loads(cont.split('\n')[0]))
    # path = os.path.join(script_path, 'test.jsonl')
    # with open(path, 'w') as file:
    #     file.write(cont)
    # dic_out = read_jsonl(path)
    # dic_inp = retireve_batch_input(batch_id)
    # # print(dic_inp[0]['body']['messages'][-1]['content'])
    # print(dic_out[0]['response']['body']['choices'][0]['message']['content'])

...
class Optimizer:
    ...
    status : str
        Text description of optimizer status.
    ...
    def __init__(self):
        ...
        self.status = 'Optimization not started'
        self._err_msg = None
        self.history = None
    ...
    def optimize(self, resume=False, inject=None, seed=None):
        ...
        if not resume:
            ...
            self.status = 'Optimization running'
            self._err_msg = None
            self.history = None
            ...
            self._log('')
            self.status = 'Optimization running (resumed)'
    ...
    def _stopping_criteria(self):
        ...
        if self.max_iterations:
            if self.it >= self.max_iterations:
                self.status = f'Optimization terminated: maximum number of iterations reached ({self.max_iterations}).'
                stop = True

        # Stop if maximum number of evaluations surpassed
        if self.max_evaluations:
            if self.eval >= self.max_evaluations:
                self.status = f'Optimization terminated: maximum number of evaluations reached ({self.eval}).'
                stop = True

        # Stop if fitness threshold achieved
        if self.target_fitness:
            if self.best.f < self.target_fitness and np.all(self.best.C <= 0):
                self.status = f'Optimization terminated: target fitness achieved ({self.best.f:.6e}).'
                stop = True
        ...
        if self.max_stalled_iterations:
            ...
            if self._stalled_it >= self.max_stalled_iterations:
                self.status = f'Optimization terminated: maximum stalled iterations reached ({self._stalled_it:d}).'
                stop = True
        ...
        if self.max_stalled_evaluations:
            ...
            if self._stalled_eval >= self.max_stalled_evaluations:
                self.status = f'Optimization terminated: maximum stalled evaluations reached ({self._stalled_eval:d}).'
                stop = True

        # Stop if maximum elapsed time reached
        if self.max_elapsed_time:
            if self.elapsed_time >= self.max_elapsed_time:
                self.status = f'Optimization terminated: maximum elapsed time reached ({self.max_elapsed_time} s).'
                stop = True

        if stop:
            if self.constraints > 0:
                if self.best.is_feasible():
                    self.status += ' Feasible solution found.'
                else:
                    self.status += ' No feasible solution found.'
    ...
    def _finalize_iteration(self):
        ...
        if self._err_msg is not None:
            self.status = f'Optimization aborted with error: {self._err_msg}'
            self._log(self.status)
            assert False, \

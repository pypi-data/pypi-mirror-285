...
VERSION = "1.0.0"
VERSION_TEST = "1.0.25_Pankósmios"
...
def _Print(Message: str, Level: int):
    if(Level & PRINT_ERROR_CRITICAL & _PrintLevel):
...
class FT_Buffer:
    ...
    def from_int(Integer: int):
        ...
        if(isinstance(Integer, int)):
            ...
            _Print("FT_Buffer(int), not given an int!", PRINT_ERROR_MINOR)
        return NewBuffer
    def from_str(String: str):
        ...
        if(isinstance(String, str)):
            ...
            _Print("FT_Buffer(str), not given a str!", PRINT_ERROR_MINOR)
        return NewBuffer
    def from_bytearray(ByteArray: bytearray):
        ...
        if(isinstance(ByteArray, bytearray)):
            ...
            _Print("FT_Buffer(bytearray), not given a bytearray!", PRINT_ERROR_MINOR)
        return NewBuffer
    def from_bytes(Bytes: bytes):
        ...
        if(isinstance(Bytes, bytes)):
            ...
            _Print("FT_Buffer(bytes), not given bytes!", PRINT_ERROR_MINOR)
        return NewBuffer
...
if _Python64:
    _Print("DETECTED 64-BIT PYTHON ENVIRONMENT: LOADING 64-bit dynamic library file & setting 64-bit argtypes.", PRINT_INFO_START)
    if Platform == "linux":
    ...
    try: _DLL = ctypes.cdll.LoadLibrary(_DLL_Path) # Check if 64-bit dll exists in same directory as executable.
        ...
        exit()
    # Define argtypes of SOME functions from FTDI D3XX Programmer's Guide.
else:
    _Print("DETECTED 32-BIT PYTHON ENVIRONMENT: LOADING 32-bit dynamic library file & setting 32-bit argtypes.", PRINT_INFO_START)
    if Platform == "linux":
    ...
    try: _DLL = ctypes.cdll.LoadLibrary(_DLL_Path) # Check if 32-bit dll exists in same directory as executable.
        ...
        exit()
    # Define argtypes of SOME functions from FTDI D3XX Programmer's Guide.
_Print("Successfully loaded FTDI D3XX driver.", PRINT_INFO_START)
...
def FT_GetDeviceInfoList(DeviceCount: int) -> int | list[FT_Device]:
    ...
    if(Status != FT_OK):
        _Print(FT_STATUS_STR[Status] + " | ERROR: FAILED TO GET DEVICE INFO LIST.", PRINT_ERROR_MAJOR)
...
def FT_GetDeviceInfoDict(DeviceCount: int) -> int | dict[int, FT_Device]:
    ...
    for i in range(0, DeviceCount): # Get device info at each index from 0->(DeviceCount - 1).
        ...
        if Status != FT_OK:
            _Print(FT_STATUS_STR[Status] + " | WARNING: FAILED TO GET INFO FOR DEVICE " + str(i), PRINT_ERROR_MAJOR)
...
def FT_ListDevices(IndexCount: int, Flags: int) -> int | (int | str | list[str]):
    ...
    if(Flags & FT_LIST_NUMBER_ONLY):
        ...
        if(Status == FT_OK):
            ...
            ReturnValue = 0
            _Print(FT_STATUS_STR[Status] + " | FT_ListDevices(), ERROR: FAILED TO LIST DEVICE COUNT.", PRINT_ERROR_MAJOR)
        ...
        if(Status == FT_OK):
            ...
            _Print(FT_STATUS_STR[Status] + " | FT_ListDevices(), ERROR: FAILED TO GET DEVICE INFO.", PRINT_ERROR_MAJOR)
        ...
        if(Status == FT_OK):
            ...
            _Print(FT_STATUS_STR[Status] + " | FT_ListDevices(), ERROR: FAILED TO GET INFO FOR DEVICES.", PRINT_ERROR_MAJOR)
    else:
        _Print(FT_STATUS_STR[Status] + " | FT_ListDevices(), WARNING: NOT GIVEN VALID FLAGS.", PRINT_ERROR_MAJOR)
...
def FT_GetPipeInformation(Device: FT_Device, InterfaceIndex: int, PipeIndex: int) -> int | FT_Pipe:
    ...
    if FT_STATUS_STR[Status] != "FT_OK":
        _Print(FT_STATUS_STR[Status] + " | Failed to get pipe information!", PRINT_ERROR_MAJOR)
...
def FT_InitializeOverlapped(Device: FT_Device) -> int | FT_Overlapped:
    ...
    if FT_STATUS_STR[Status] != "FT_OK":
        _Print(FT_STATUS_STR[Status] + " | Failed to get overlap information!", PRINT_ERROR_MAJOR)
...
def FT_Create(Identifier, OpenFlag: int, Device: FT_Device) -> int:
    ...
    if(OpenFlag & FT_OPEN_BY_INDEX):
        ...
        if(isinstance(Identifier, str)):
            ...
            _Print("FT_Create(), did not get expected str for serial number!", PRINT_ERROR_MAJOR)
        ...
        if(isinstance(Identifier, str)):
            ...
            _Print("FT_Create(), did not get expected str for description!", PRINT_ERROR_MAJOR)
    else:
        _Print("FT_Create(), given invalid open flag!", PRINT_ERROR_MAJOR)
    return Status
...
def FT_SetSuspendTimeout(Device: FT_Device, Timeout: int) -> int:
    Status = FT_OTHER_ERROR
    if Platform != "windows":
        _Print("FT_SetSuspendTimeout() DOES NOT EXIST IN LINUX OR MACOS", PRINT_ERROR_CRITICAL)
    else:
...
def FT_GetSuspendTimeout(Device: FT_Device) -> int | int:
    Status = FT_OTHER_ERROR
    Timeout = Timeout = ctypes.c_ulong(0)
    if Platform != "windows":
        _Print("FT_GetSuspendTimeout() DOES NOT EXIST IN LINUX OR MACOS", PRINT_ERROR_CRITICAL)
    else:
...
def FT_GetPipeTimeout(Device: FT_Device, Pipe: FT_Pipe) -> int | int:
    if Platform != "windows":
        _Print("FT_GetPipeTimeout() DOES NOT EXIST IN LINUX OR MACOS", PRINT_ERROR_CRITICAL)
...
def FT_GetDeviceDescriptor(Device: FT_Device) -> int | FT_DeviceDescriptor:
    ...
    if(Status != FT_OK):
        _Print(FT_STATUS_STR[Status] + " | ERROR: FAILED TO GET DEVICE DESCRIPTOR.", PRINT_ERROR_MAJOR)
        Descriptor = FT_DeviceDescriptor()
...
def FT_GetConfigurationDescriptor(Device: FT_Device) -> int | FT_ConfigurationDescriptor:
    ...
    if(Status != FT_OK):
        _Print(FT_STATUS_STR[Status] + " | ERROR: FAILED TO GET CONFIGURATION DESCRIPTOR.", PRINT_ERROR_MAJOR)
        Descriptor = FT_ConfigurationDescriptor()
        return Status, Descriptor
...
def FT_GetInterfaceDescriptor(Device: FT_Device, InterfaceIndex) -> int | FT_InterfaceDescriptor:
    ...
    if FT_STATUS_STR[Status] != "FT_OK":
        _Print(FT_STATUS_STR[Status] + " | Failed to get interface information!", PRINT_ERROR_MAJOR)
...
def FT_GetStringDescriptor(Device: FT_Device, StringIndex: int) -> int | FT_StringDescriptor:
    ...
    if(Status != FT_OK):
        _Print(FT_STATUS_STR[Status] + " | ERROR: FAILED TO GET STRING DESCRIPTOR.", PRINT_ERROR_MAJOR)
        Descriptor = FT_StringDescriptor()
        return Status, Descriptor
...
def FT_GetDescriptor(Device: FT_Device, DescriptorType: int, Index: int) -> int | FT_DeviceDescriptor | FT_InterfaceDescriptor | FT_ConfigurationDescriptor | FT_StringDescriptor | int:
    ...
    match DescriptorType:
        case _GetDescriptorHelperClass.DEVICE:
            ...
            if(Status != FT_OK):
                _Print(FT_STATUS_STR[Status] + " | FT_GetDescriptor(), ERROR: FAILED TO GET DEVICE DESCRIPTOR.", PRINT_ERROR_MAJOR)
                Descriptor = FT_DeviceDescriptor()
            else:
                Descriptor = _GetDeviceDescriptorHelper(Descriptor)
        case _GetDescriptorHelperClass.CONFIGURATION:
            ...
            if(Status != FT_OK):
                _Print(FT_STATUS_STR[Status] + " | FT_GetDescriptor(), ERROR: FAILED TO GET CONFIGURATION DESCRIPTOR.", PRINT_ERROR_MAJOR)
                Descriptor = FT_ConfigurationDescriptor()
            else:
                Descriptor = _GetConfigurationDescriptorHelper(Descriptor)
        case _GetDescriptorHelperClass.STRING:
            ...
            if(Status != FT_OK):
                _Print(FT_STATUS_STR[Status] + " | FT_GetDescriptor(), ERROR: FAILED TO GET STRING DESCRIPTOR.", PRINT_ERROR_MAJOR)
                Descriptor = FT_StringDescriptor()
            else:
                Descriptor = _GetStringDescriptorHelper(Descriptor)
        case _GetDescriptorHelperClass.INTERFACE:
            ...
            #if(Status != FT_OK):
            #    _Print(FT_STATUS_STR[Status] + " | FT_GetDescriptor(), ERROR: FAILED TO GET INTERFACE DESCRIPTOR.", PRINT_ERROR_MAJOR)
            #    Descriptor = FT_InterfaceDescriptor()
            #else:
            #    Descriptor = _GetInterfaceDescriptorHelper(Descriptor)
        ...
        case _GetDescriptorHelperClass.ENDPOINT: # This does not work :(, but it is not mentioned in the programmers guide.
            ...
            if(Status != FT_OK):
                _Print(FT_STATUS_STR[Status] + " | FT_GetDescriptor(), ERROR: FAILED TO GET ENDPOINT DESCRIPTOR.", PRINT_ERROR_MAJOR)
                Descriptor = FT_EndpointDescriptor()
            else:
                Descriptor = _GetEndpointDescriptorHelper(Descriptor)
        case _: # Default case.
            _Print(FT_STATUS_STR[Status] + " | FT_GetDescriptor(), WARNING: GIVEN INVALID DESCRIPTOR TYPE.", PRINT_ERROR_MINOR)
...
def FT_ControlTransfer(Device: FT_Device, SetupPacket: FT_SetupPacket, Buffer: FT_Buffer, BufferLength: int) -> int | int:
    ...
    if(Buffer._RawAddress == "FT_OTHER_ERROR"):
        _Print(FT_STATUS_STR[Status] + " | FT_ControlTransfer(), ERROR: GIVEN INVALID FT_Buffer.", PRINT_ERROR_MAJOR)
    ...
    if(Status != FT_OK):
        _Print(FT_STATUS_STR[Status] + " | FT_ControlTransfer(), ERROR: Failed to transmit data.", PRINT_ERROR_MAJOR)
    return Status, LengthTransferred.value
...
def FT_GetVIDPID(Device: FT_Device) -> int | int | int:
    ...
    if(Status != FT_OK):
        _Print(FT_STATUS_STR[Status] + " | FT_GetVIDPID(), ERROR: Failed to get VID & PID.", PRINT_ERROR_MAJOR)
...
def FT_EnableGPIO(Device: FT_Device, EnableMask: int, DirectionMask: int) -> int:
    ...
    if(Status != FT_OK):
        _Print(FT_STATUS_STR[Status] + " | FT_EnableGPIO(), ERROR: Failed to enable GPIO.", PRINT_ERROR_MAJOR)
    return Status
...
def FT_WriteGPIO(Device: FT_Device, SelectMask: int, Data: int) -> int:
    ...
    if(Status != FT_OK):
        _Print(FT_STATUS_STR[Status] + " | FT_WriteGPIO(), ERROR: Failed to write to GPIO.", PRINT_ERROR_MAJOR)
    return Status
...
def FT_ReadGPIO(Device: FT_Device) -> int | int:
    ...
    if(Status != FT_OK):
        _Print(FT_STATUS_STR[Status] + " | FT_WriteGPIO(), ERROR: Failed to read GPIO.", PRINT_ERROR_MAJOR)
...
def _FT_SetNotificationCallbackHelper(Context, CallbackType, CallbackInfoPointer):
    ...
    if(CallbackType == E_FT_NOTIFICATION_CALLBACK_TYPE_DATA):
        ...
        _Print("INTERNAL LIBRARY ERROR: CALLBACK FUNCTION GOT INVALID CALLBACK TYPE.", PRINT_ERROR_MAJOR)
    return None
...
def FT_SetNotificationCallback(Device: FT_Device, CallbackFunction: typing.Callable[[int, int, int], None]) -> int:
    ...
    if(Status != FT_OK):
        _Print(FT_STATUS_STR[Status] + " | FT_SetNotificationCallback(), ERROR: Failed to set callback function.", PRINT_ERROR_MAJOR)
    return Status
...
def FT_ClearNotificationCallback(Device: FT_Device) -> int:
    if Platform != "windows":
        _Print("FT_ClearNotificationCallback() HAS 'X' FOR LINUX", PRINT_ERROR_CRITICAL)
        return FT_OTHER_ERROR
    Status = _DLL.FT_ClearNotificationCallback(Device._Handle)
    if(Status != FT_OK):
        _Print(FT_STATUS_STR[Status] + " | FT_ClearNotificationCallback(), ERROR: Failed to clear callback function.", PRINT_ERROR_MAJOR)
    return Status
...
def FT_GetChipConfiguration(Device: FT_Device) -> int | FT_60XCONFIGURATION:
    ...
    if(Status != FT_OK):
        _Print(FT_STATUS_STR[Status] + " | FT_GetChipConfiguration(), ERROR: Failed to get chip configuration.", PRINT_ERROR_MAJOR)
...
def FT_SetChipConfiguration(Device: FT_Device, Configuration: FT_60XCONFIGURATION) -> int:
    ...
    if(Status != FT_OK):
        _Print(FT_STATUS_STR[Status] + " | FT_SetChipConfiguration(), ERROR: Failed to set chip configuration.", PRINT_ERROR_MAJOR)
        return Status, Configuration
    return Status

def FT_IsDevicePath(Device: FT_Device, DevicePath: str) -> int: # HAVE NOT CONFIRMED THIS WORKS YET.
    if(Platform != "windows"):
        _Print("FT_IsDevicePath() DOES NOT EXIST IN LINUX OR MACOS", PRINT_ERROR_CRITICAL)
        return FT_OTHER_ERROR
    return _DLL.FT_IsDevicePath(Device._Handle, DevicePath.encode("utf-8"))
...
def FT_GetDriverVersion(Device: FT_Device) -> int | int:
    ...
    if(Status != FT_OK):
        _Print(FT_STATUS_STR[Status] + " | FT_GetDriverVersion(), ERROR: Failed to get driver version.", PRINT_ERROR_MAJOR)
...
def FT_GetLibraryVersion() -> int | int:
    ...
    if(Status != FT_OK):
        _Print(FT_STATUS_STR[Status] + " | FT_GetLibraryVersion(), ERROR: Failed to get library version.", PRINT_ERROR_MAJOR)
...
def FT_CycleDevicePort(Device: FT_Device) -> int:
    ...
    if(Status != FT_OK):
        _Print(FT_STATUS_STR[Status] + " | FT_CycleDevicePort(), ERROR: Failed to cycle device port.", PRINT_ERROR_MAJOR)
    return Status
...
def FT_ReadPipe(Device: FT_Device, Pipe: FT_Pipe, BufferLength: int, Overlapped_TimeoutMs) -> int | FT_Buffer | int:
    ...
    if(Platform == "linux"): # Overlapped is instead a timeout in ms.
        ...
        _Print("FT_ReadPipe(), invalid Overlapped type given, expecting FT_Overlapped or NULL", PRINT_ERROR_MAJOR)
...
def FT_ReadPipeEx(Device: FT_Device, Pipe_FIFOindex, BufferLength: int, Overlapped_TimeoutMs) -> int | FT_Buffer | int:
    ...
    if(Platform == "linux"): # Overlapped is instead a timeout in ms.
        ...
        _Print("FT_ReadPipeEx(), invalid Overlapped type given, expecting FT_Overlapped or NULL", PRINT_ERROR_MAJOR)
    return Status, Buffer, BytesTransferred.value
...
def FT_ReadPipeAsync(Device: FT_Device, FIFO_Index: int, BufferLength: int, Overlapped) -> int | FT_Buffer | int:
    ...
    if(Platform != "linux") and (Platform != "darwin"):
        _Print("FT_ReadPipeAsync() DOES NOT EXIST OUTSIDE LINUX", PRINT_ERROR_CRITICAL)
...
def FT_WritePipe(Device: FT_Device, Pipe: FT_Pipe, Buffer: FT_Buffer, BufferLength: int, Overlapped_TimeoutMs) -> int | int:
    ...
    if(isinstance(Buffer._RawAddress, FT_Buffer)):
        _Print("FT_WritePipe(), was not given expected FT_Buffer.", PRINT_ERROR_MAJOR)
        return Status, 0
    BytesTransferred = ctypes.c_ulong(0)
    if(Platform == "linux"): # Overlapped is instead a timeout in ms.
        ...
        _Print("FT_WritePipe(), invalid Overlapped type given, expecting FT_Overlapped or NULL", PRINT_ERROR_MAJOR)
...
def FT_WritePipeEx(Device: FT_Device, Pipe_FIFOindex, Buffer: FT_Buffer, BufferLength: int, Overlapped_TimeoutMs) -> int | int:
    ...
    if(isinstance(Buffer._RawAddress, ctypes.c_char_p)):
        _Print("FT_WritePipeEx(), was not given expected FT_Buffer.", PRINT_ERROR_MAJOR)
        return Status, 0
    BytesTransferred = ctypes.c_ulong(0)
    if(Platform == "linux"): # Overlapped is instead a timeout in ms.
        Status = _DLL.FT_ReadPipeEx(Device._Handle,
        ...
        _Print("FT_WritePipeEx(), invalid Overlapped type given, expecting FT_Overlapped or NULL", PRINT_ERROR_MAJOR)
    return Status, BytesTransferred.value
...
def FT_WritePipeAsync(Device: FT_Device, FIFO_Index, Buffer: FT_Buffer, BufferLength: int, Overlapped) -> int | int:
    ...
    BytesTransferred = ctypes.c_ulong(0)
    if(Platform != "linux") and (Platform != "darwin"):
        _Print("FT_WritePipeAsync() DOES NOT EXIST OUTSIDE LINUX", PRINT_ERROR_CRITICAL)
        return FT_OTHER_ERROR, 0
...
def FT_SetStreamPipe(Device: FT_Device, AllWritePipes: bool, AllReadPipes: bool, Pipe: FT_Pipe | int, StreamSize: int) -> int:
    Status = FT_OTHER_ERROR
    if(isinstance(Pipe, FT_Pipe)):
        if(AllWritePipes or AllReadPipes):
            _Print("FT_SetStreamPipe(), PipeID will be ignored because AllXPipes arguments are not both False.", PRINT_ERROR_MINOR)
        ...
        _Print("FT_SetStreamPipe(), Was not given expected FT_Pipe or NULL.", PRINT_ERROR_MAJOR)
    return Status

def FT_ClearStreamPipe(Device: FT_Device, AllWritePipes: bool, AllReadPipes: bool, Pipe: FT_Pipe | int) -> int:
    Status = FT_OTHER_ERROR
    if(isinstance(Pipe, FT_Pipe)):
        if(AllWritePipes or AllReadPipes):
            _Print("FT_ClearStreamPipe(), PipeID will be ignored because AllXPipes arguments are not both False.", PRINT_ERROR_MINOR)
        ...
        _Print("FT_ClearStreamPipe(), Was not given expected FT_Pipe or NULL.", PRINT_ERROR_MAJOR)
    return Status

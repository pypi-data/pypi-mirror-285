...
def render_compare(data: List[TableDescription], table_names:List[str],name:str):
    ...
    combined_data = {
        ...
        'Distinct Values (Percentage)': [f"{x:.2f}%" for x in distinct_perc_list],
        'Values Existing in All Tables (Count)': shared_values_count_list,
        'Values Existing in All Tables (Percentage)': [f"{x:.2f}%" for x in shared_values_perc_list],

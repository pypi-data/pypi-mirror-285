...
class HTMLTable(BaseRenderer):
    def __init__(self, data:Any=None, id:str=None, name:str=None, headers:List[str]=None,**kwargs):
        super().__init__(id=id, name=name, content={'data': data, 'headers': headers}, **kwargs)

    def render(self):
        return template("table.html").render(**self.content)

        return template("table.html").render(data, headers,name)

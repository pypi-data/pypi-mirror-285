...
<table id="T_19b39">
  ...
  <tbody>
    <tr>
      <td id="T_19b39_row0_col0" class="data row0 col0" ><a href='examples/myfile.py'
style='text-decoration:none;color:inherit'>myfile</a>.<a href='examples/myfile.py'
style='text-decoration:none;color:inherit'>MyBook</a>:<a href='examples/myfile.py'
style='text-decoration:none;color:inherit'>7</a></td>
      <td id="T_19b39_row0_col1" class="data row0 col1" >class <a href='examples/myfile.py'
style='text-decoration:none;color:#cccccc;background-color:#505050'>MyBook</a>:</td>
    </tr>
    <tr>
      <td id="T_19b39_row1_col0" class="data row1 col0" ><a href='examples/myfile.py'
style='text-decoration:none;color:inherit'>myfile</a>.<a href='examples/myfile.py'
style='text-decoration:none;color:inherit'>print_my_book</a>:<a href='examples/myfile.py'
style='text-decoration:none;color:inherit'>24</a></td>
      <td id="T_19b39_row1_col1" class="data row1 col1" >def print_my_book(book: <a
href='examples/myfile.py'
style='text-decoration:none;color:#cccccc;background-color:#505050'>MyBook</a>) -> None:</td>
    </tr>
    <tr>
      <td id="T_19b39_row2_col0" class="data row2 col0" ><a href='examples/myfile.py'
style='text-decoration:none;color:inherit'>myfile</a>.<a href='examples/myfile.py'
style='text-decoration:none;color:inherit'>print_my_book</a>:<a href='examples/myfile.py'
style='text-decoration:none;color:inherit'>30</a></td>
      <td id="T_19b39_row2_col1" class="data row2 col1" >    book : <a href='examples/myfile.py'
style='text-decoration:none;color:#cccccc;background-color:#505050'>MyBook</a></td>
    </tr>
  </tbody>
</table>
...
## Examples
### tx.module()
The previous demonstration introduced the core function `tx.module()`. In fact, the return type of
`tx.module()` is a subclass of the abstract class `PyText`, who supports various text manipulation
methods:
```py
>>> isinstance(m, tx.PyText)
True
```
Sometimes, your python module may contain not just one file but multiple files and folders, but
don’t worry since `tx.module()` provides support for complex file hierarchies. The return type will
be either `PyDir` or `PyFile` (both are subclasses of `PyText`) depending on the path type.

In conclusion, suppose you've got a python package, you can simply give the package dirpath to
`tx.module()`, and do things like before:
...
### tx.PyText.findall()
As mentioned before, user can use `.findall()` to find all non-overlapping matches of some pattern
in a python module.
```py
>>> m.findall("optional", styler=False)
examples/myfile.py:13: '    story : str, <optional>'
```
The optional argument `styler=` determines whether to use a pandas `Styler` object to beautify the
representation. If you are running python in the console, please always set `styler=False`. You can
also disable the stylers in `display_params`, so that you don't need to repeat `styler=False` every
time in the following examples:
```py
>>> from textpy import display_params
>>> display_params.enable_styler = False
```
Method `.findall()` also has some optional parameters including `whole_word=`, `case_sensitive=`,
`regex=` to customize the match pattern:
```py
...
### tx.PyText.replace()
Use `.replace()` to find all non-overlapping matches of some pattern, and replace them with another
string:
```py
...
```
At this point, the replacement has not yet taken effect on the files. Use `.confirm()` to confirm
the changes and make them done:
```py
>>> replacer.confirm()

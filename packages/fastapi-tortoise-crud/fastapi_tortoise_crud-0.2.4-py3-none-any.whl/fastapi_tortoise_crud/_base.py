...
class CrudGenerator(APIRouter, ABC):
    def __init__(self, model: Union[BaseModel, Any],
                 ...
                 overwrite_dependencies: bool = True,
        ...
        self.schema_list = schema_list if isinstance(schema_list, type) and issubclass(schema_list or object,
                                                                                       PydanticModel) else self.schema_read
        self.schema_update = schema_update if isinstance(schema_update, type) and issubclass(schema_update or object,
                                                                                             PydanticModel) else model.schema_update()
        self.schema_create = schema_create if isinstance(schema_create, type) and issubclass(schema_create or object,
                                                                                             PydanticModel) else model.schema_create()
        self.schema_delete = schema_delete if isinstance(schema_delete, type) and issubclass(schema_delete or object,

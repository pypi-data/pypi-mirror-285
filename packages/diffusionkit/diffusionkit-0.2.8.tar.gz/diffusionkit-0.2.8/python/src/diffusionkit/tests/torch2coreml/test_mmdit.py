...
def get_test_inputs(cfg: mmdit.MMDiTConfig) -> Dict[str, torch.Tensor]:
    ...
    assert (TEST_LATENT_WIDTH + TEST_LATENT_HEIGHT) < cfg.max_latent_resolution
...
def convert_mmdit_to_mlpackage(
    ...
    """Converts a MMDiT model to CoreML"""
    ...
    with argmaxtools_test_utils._get_test_cache_dir(persistent_cache_dir=output_dir) as TEST_CACHE_DIR:
        suite = unittest.TestSuite()
        mmdit_test_model = TestSD3MMDiT("test_torch2coreml_correctness_and_speedup")
        mmdit_test_model.model_version = "2b"
        suite.addTest(mmdit_test_model)
...
if __name__ == "__main__":
    ...
    with argmaxtools_test_utils._get_test_cache_dir(args.o) as TEST_CACHE_DIR:
        suite = unittest.TestSuite()
        mmdit_test_model = TestSD3MMDiT("test_torch2coreml_correctness_and_speedup")
        mmdit_test_model.model_version = args.model_version
        suite.addTest(mmdit_test_model)

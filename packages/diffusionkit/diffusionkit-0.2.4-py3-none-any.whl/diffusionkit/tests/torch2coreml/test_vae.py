...
def get_test_inputs(config: vae.VAEDecoderConfig) -> Dict[str, torch.Tensor]:
    ...
    if (TEST_LATENT_HEIGHT + TEST_LATENT_WIDTH) != config_expected_latent_resolution:
        logger.warning(
            f"TEST_LATENT_SIZE ({(TEST_LATENT_HEIGHT + TEST_LATENT_WIDTH)}) does not match the implied "

...
def get_spatial_reference_api(layer, gis):
    """
    Returns the spatial reference of a given layer asking to the api
    
    Parameters:
        - layer: Layer Item of the structure looking to be read
        - gis: GIS session with access to the layer.
    """
    ...
    response_api = requests.get(ask_url)
...
def get_layer_crs_aux(layer, gis=None):
    """
    ...
    """
    if not "extent" in layer.properties and not gis:
        return get_spatial_reference_api(layer, gis)
    elif not "extent" in layer.properties:
        raise Exception("Empty Layer Must Provide GIS Session To Read CRS")
    else:
...
def get_layer_crs(gis, layer_id, number=None):
    ...
    return get_layer_crs_aux(layer, gis)


def get_layer_extent_aux(layer, out_crs=None, gis=None):
    """

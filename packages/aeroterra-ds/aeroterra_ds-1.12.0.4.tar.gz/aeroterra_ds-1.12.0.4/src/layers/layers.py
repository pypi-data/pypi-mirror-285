...
def change_gdf_to_layer_crs(gdf, layer, gis=None):
    """
    ...
    Parameters:
        ...
        - gis: GIS session with access to the layer. If not provided,
            the process might fail for an empty layer
    
    Returns the sucess status of each add.
    """
    layer_crs = get_layer_crs_aux(layer, gis)
    gdf_crs = gdf.crs
    if gdf_crs is None:
...
def clone_layer(og_gis, og_id, new_gis, new_name=None, copy_data=True, publish_batch=BATCH_AMOUNT):
    ...
    if copy_data:
        layer_content = read_full_layer_aux_sdf(old_layer, gis=og_gis)
    else:
        layer_content = create_empty_gdf_aux(old_layer, gis=og_gis)
...
def create_layer(gdf, gis, title=None, folder=None, publish_batch=BATCH_AMOUNT):
    ...
    add_to_layer_aux(layer.layers[0], gdf[end:], publish_batch, gis)
...
def add_to_layer_aux(layer, gdf, publish_batch=BATCH_AMOUNT, gis=None):
    """
    ...
    gdf = change_gdf_to_layer_crs(gdf, layer, gis)
...
def add_to_layer(gdf, gis, layer_id, number=None, publish_batch=BATCH_AMOUNT):
    ...
    return add_to_layer_aux(layer, gdf, publish_batch, gis)


def update_layer_aux(layer, gdf, columns=None, gis=None):
    """
...
def update_layer(gdf, gis, layer_id, number=None, columns=None):
    ...
    return update_layer_aux(layer, gdf, columns, gis)
...
def create_empty_gdf_aux(layer, crs=None, gis=None):
    """
    ...
    if crs is None:
        crs = get_layer_crs_aux(layer, gis)
...
def create_empty_gdf(gis, layer_id, number=None, crs=None):
    ...
    return create_empty_gdf_aux(layer, crs, gis)


def read_full_layer_aux_sdf(layer, out_crs=None, gis=None):
    """
    ...
    total_data = None
    if out_crs is None:
        out_crs = get_layer_crs_aux(layer, gis)
    ...
    if total_data is None:
        return create_empty_gdf_aux(layer, crs=out_crs, gis=gis)

    return total_data


def read_full_layer_aux(layer, out_crs=None, gis=None):
    """
    Returns the full data stored in an asked layer.
    ...
    if out_crs is None:
        out_crs = get_layer_crs_aux(layer, gis)
    total_data = read_full_layer_aux_sdf(layer, out_crs=out_crs, gis=gis)
...
def read_full_layer(gis, layer_id, number=None, out_crs=None):
    ...
    return read_full_layer_aux(layer, out_crs, gis=gis)


def read_layer_gdf_aux(layer, query="1=1", geometry_filter=None, geometry_crs=4326, out_fields=None, out_crs=None, geometry_post=True, gis=None):
    """
    Returns the full data stored in an asked layer.
    ...
    if total_data is None:
        total_data = create_empty_gdf_aux(layer, crs=out_crs, gis=gis)
...
def read_layer_gdf(gis, layer_id, number=None, query="1=1", geometry_filter=None, geometry_crs=4326, out_fields=None, out_crs=None, geometry_post=True):
    ...
    return read_layer_gdf_aux(layer, query, geometry_filter, geometry_crs, out_fields, out_crs, geometry_post, gis=gis)
...
def rewrite_layer_aux(layer, gdf, publish_batch=BATCH_AMOUNT, gis=None):
    """
...
def rewrite_layer(gis, layer_id, gdf, number=None, publish_batch=BATCH_AMOUNT):
    ...
    return rewrite_layer_aux(layer, gdf, publish_batch, gis)
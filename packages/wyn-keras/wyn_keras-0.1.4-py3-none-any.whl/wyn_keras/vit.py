...
class CustomViT:
    """
    Custom Vision Transformer (ViT) class for image classification.
    ...
    def plot_patches(self, images: np.ndarray):
        ...
        image = images[0]
        patches = self.Patches(self.patch_size)(tf.convert_to_tensor([image]))
        patches = patches.numpy().reshape(-1, self.patch_size, self.patch_size, 3)
    ...
    def plot_attention(self, model: keras.Model, image: np.ndarray):
        """
        Plots the attention map overlaying on the image.

        Args:
            model: Trained ViT model.
            image: Input image of shape (w, h, depth).
        """
        # Preprocess the image
        input_image = tf.image.resize(image, (self.image_size, self.image_size))
        input_image = tf.expand_dims(input_image, axis=0)

        # Get the attention scores from the last transformer layer
        attention_model = keras.Model(
            inputs=model.input,
            outputs=model.layers[-4].output  # Adjust layer index if necessary
        )
        attention_scores = attention_model(input_image)

        # Calculate attention map
        attention_map = tf.reduce_mean(attention_scores, axis=1)
        attention_map = tf.reshape(attention_map, (self.image_size, self.image_size))

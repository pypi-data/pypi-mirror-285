...
import pylogram
from pylogram.methods.utilities.idle import idle
...
class Run:
    def run(
        ...
        :meth:`~pylogram.Client.start`, :meth:`~pylogram.idle` and :meth:`~pylogram.Client.stop` in sequence.
        ...
        operation. This is almost the same as :py:obj:`asyncio.run` except for the fact that Pylogram's ``run`` uses the
        current event loop instead of a new one.

        If you want to run multiple clients at once, see :meth:`pylogram.compose`.

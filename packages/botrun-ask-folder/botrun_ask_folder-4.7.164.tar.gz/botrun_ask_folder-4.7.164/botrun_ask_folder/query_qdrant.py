...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s',
                    filename='./users/botrun_ask_folder/query.log')
...
def query_qdrant_knowledge_base(qdrant_host,
    ...
    logger.info(f"Querying Qdrant for user input: {user_input}")
    ...
    logger.info(f"Received {len(search_result)} results from Qdrant")
    ...
    open("./users/botrun_ask_folder/str_knowledge_base2.txt", "w").write(str_knowledge_base)
...
def completion_call(model, message):
    logger.info(f"Entering completion_call with model: {model}, message: {message}")
    try:
        messages = [{"content": message, "role": "user"}]
        logger.info(f"Messages prepared: {messages}")
        generator = filtered_stream_with_hidden_content(model, messages)
        reference_content = ""
        logger.info("Starting iteration over generator")
        # 迭代生成器直到倒數第二個值
        for chunk in generator:
            if isinstance(chunk, str):  # 確保chunk是字符串
                ...
                if chunk.startswith(SPECIAL_STRING):
                    ...
                    logger.info(f"Found special string. Reference content: {reference_content}")
                    break
                yield chunk
            else:
                logger.debug("Chunk is not a string, breaking loop")
                break  # 如果chunk不是字符串，跳出循環

        try:
            logger.info(f"Reference content: {reference_content}")
            lst_refs = json.loads(reference_content)['references']
            logger.info(f"References parsed: {lst_refs}")
            generated_ref_page = generate_pdf_page_to_image(lst_refs)
            logger.info(f"Generated reference page: {generated_ref_page}")
            yield generate_ref_section(lst_refs, generated_ref_page)
        except json.JSONDecodeError as e:
            logger.error(f"Invalid JSON: {e}")
    except Exception as e:
        logger.error(f"Exception in completion_call: {e}")
        # print stacktrace
        import traceback
        traceback.print_exc()
...
def filtered_stream_with_hidden_content(model, messages):
    logger.info(f"Entering filtered_stream_with_hidden_content with model: {model}, messages: {messages}")
    ...
    for chunk in response:
        ...
        buffer += content
        logger.info(f"Buffer after adding content: {buffer}")

        if not found_special_string:
            # 檢查是否包含特殊字符串
            split = buffer.split(SPECIAL_STRING, 1)
            logger.info(f"Split result: {split}")
            ...
            if len(split) > 1:
                ...
                buffer = ""
                logger.info(f"Found special string. Hidden content: {hidden_content}")
            else:
                ...
                if safe_to_yield:
                    ...
                    logger.info(f"Yielded safe content: {to_yield}")
        else:
            # 特殊字符串之後的所有內容都添加到hidden_content
            hidden_content += content
            logger.info(f"Hidden content after adding content: {hidden_content}")

    # 如果整個響應結束都沒有找到特殊字符串，yield剩餘的buffer
    if buffer and not found_special_string:
        yield buffer
        logger.info(f"Yielded remaining buffer: {buffer}")

    # 最後yield隱藏內容
    yield SPECIAL_STRING + hidden_content.strip()
    logger.info(f"Yielded final hidden content: {SPECIAL_STRING + hidden_content.strip()}")

    # 添加日志记录
    logger.info(f"Filtered stream with hidden content: {hidden_content}")
...
def query_qdrant_and_llm_print(qdrant_host, qdrant_port, collection_name, user_input,
    ...
    logger.info(f"Starting query_qdrant_and_llm for user input: {user_input}")
    for fragment in query_qdrant_and_llm(qdrant_host, qdrant_port, collection_name, user_input,
        ...
        print(fragment, end="")
        sys.stdout.flush()
    logger.info("Finished query_qdrant_and_llm")

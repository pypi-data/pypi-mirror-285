...
import ccxt_zagrava.async_support as ccxt_zagrava  # noqa: E402
import ccxt_zagrava as ccxt_zagrava_sync  # noqa: E402
import ccxt_zagrava.pro as ccxt_zagravapro  # noqa: E402
...
path = os.path.dirname(ccxt_zagrava.__file__)
if 'site-packages' in os.path.dirname(ccxt_zagrava.__file__):
    raise Exception("You are running tests_async.py/test.py against a globally-installed version of the library! It was previously installed into your site-packages folder by pip or pip3. To ensure testing against the local folder uninstall it first with pip uninstall ccxt_zagrava or pip3 uninstall ccxt_zagrava")
...
def init_exchange(exchangeId, args, is_ws=False):
    if is_synchronous:
        return getattr(ccxt_zagrava_sync, exchangeId)(args)
    if (is_ws):
        return getattr(ccxt_zagravapro, exchangeId)(args)
    return getattr(ccxt_zagrava, exchangeId)(args)
...
async def get_test_files(properties, ws=False):
    ...
    for i in range(0, len(finalPropList)):
        ...
        module_string = 'ccxt_zagrava.test.exchange.' + prefix + '.test_' + name_snake_case
        if (ws):
            prefix = 'pro'
            dir_to_test = DIR_NAME + '/../' + prefix + '/test/Exchange/'
            module_string = 'ccxt_zagrava.pro.test.Exchange.test_' + name_snake_case
...
def set_fetch_response(exchange: ccxt_zagrava.Exchange, data):
    if (is_synchronous):

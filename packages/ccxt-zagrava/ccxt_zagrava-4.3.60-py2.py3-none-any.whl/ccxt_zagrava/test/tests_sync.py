...
class testMainClass(baseMainTestClass):
    ...
    def start_test(self, exchange, symbol):
        ...
        try:
            ...
            #     # we test proxies functionality just for one random exchange on each build, because proxy functionality is not exchange-specific, instead it's all done from base methods, so just one working sample would mean it works for all ccxt_zagrava exchanges
    ...
    def test_kucoin(self):
        ...
        assert spot_id == 'ccxt_zagrava', 'kucoin - id: ' + spot_id + ' not in options'

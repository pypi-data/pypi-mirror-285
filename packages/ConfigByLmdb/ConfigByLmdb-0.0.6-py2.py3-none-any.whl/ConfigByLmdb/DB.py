...
class ConfigDB():
    ...
    def write(self, key:str , value:str | dict, db_name = None) -> bool:
        ...
        try:
            ...
            with self.env.begin(db=db, write=True) as txn:
                ...
                if type(value) == dict:
                    try:
                        value = json.dumps(value).encode(encoding=self.code)
                    except Exception:
    ...
    def batch_write(self, k_v_list:list[dict[str,str|int|float|dict|None]], db_name = None) -> bool:
        """批量添加键值对
            ...
            with self.env.begin(db=db, write=True) as txn:
                for kv in k_v_list:
                    for k,v in kv.items():
                        ...
                        if type(v) == dict:
                            try:
                                value = json.dumps(v).encode(encoding=self.code)
                            except Exception:
    ...
    def matching(self,key:str|int|None|float, start:int = 0, limit:int = 100, db_name:str = None) ->list:
        """字符匹配(可内部键值的深度匹配)

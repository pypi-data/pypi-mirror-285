...
class Resource:
    ...
    def _replace_output_placeholders(
        ...
        if "{ext()}" in output_file:
            extensions = MorphYaml.analyze_output_extensions(self.path)
...
#   - output_paths: ["data/outputs/example_python_cell/result_{unix()}{ext()}"]""",
    ...
    def find_resource_by_alias(
        ...
        if resource:
            ...
            Resource(
                ...
                path=OsUtils.get_abs_path(resource["path"], project_root),
    ...
    def find_resource_by_path(
        ...
        # First, search in the SQLite database
        resource = db_manager.get_resource_by_path(abs_path)
        if resource:
            if not resource.get("alias") or not resource.get("path"):
                return None
        ...
        for alias, resource in morph_yaml.resources.items():
            resource_path = OsUtils.get_abs_path(resource["path"], project_root)
            if resource_path == abs_path:
                # Sync to SQLite
                replaced = db_manager.replace_resource_record(alias, abs_path, resource)
                return Resource(
                    alias=replaced["alias"],
                    path=OsUtils.get_abs_path(replaced["path"], project_root),
                    connection=replaced.get("connection"),
    ...
    def find_or_create_resource_by_alias(
        ...
        output_paths = MorphYaml._generate_default_output_path(alias, project_root)

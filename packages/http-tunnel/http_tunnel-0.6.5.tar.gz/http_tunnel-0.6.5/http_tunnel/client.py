...
def get_equeue(equeue: queue.Queue, aes: Crypto_AES, done: threading.Event):
    ...
    time.sleep(0.001)  # to avoid websocket-client send 1005, 1006?
...
def handle_ws(
    ...
    try:
        _client.connect(
            ...
            host=settings.forward_host,
            # not sure but if timeout is provided while using ssl,
            # when under load, the client will FIN the connection
            timeout=20.0
        )
    except Exception as identifier:

...
import shutil
from blok.cli import CLI
from blok.types import ValidPath
...
class CopyPathProject(BaseModel):
    ...
    base_dir: str = Field(default_factory=lambda: os.path.join(os.getcwd(), ".blok"))
    ...
    async def ainititialize(self) -> CLI:
        ...
        if not os.path.exists(compose_file):
            raise Exception(
                "No docker-compose.yml found in the template. It appears that the template is not a valid blok template."
            )
    ...
    async def atear_down(self, cli: CLI) -> None:
        ...
        temporary files, or to remove the project from the .blok
        directory.

...
def load_and_call_get_blocks(module_name, rekuest_path, magic_path):
    try:
        ...
        if hasattr(rekuest_module, "get_bloks"):
            ...
            assert isinstance(bloks, list), "get_bloks must return a dictionary"
            return bloks
        else:
...
def check_and_import_magic(magic_path: str):
    ...
    for _, module_name, _ in pkgutil.iter_modules():
        try:
            ...
            print(f"Failed to call init_blok for installed package {module_name}: {e}")
            traceback.print_exc()

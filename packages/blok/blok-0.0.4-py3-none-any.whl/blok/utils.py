...
def check_protocol_compliance(instance: t.Type, protocol: t.Type) -> bool:
    protocol_methods = {name for name in dir(protocol) if callable(getattr(protocol, name, None)) and not name.startswith("__")}

    instance_methods = {name for name in dir(instance) if callable(getattr(instance, name, None))}

    missing_methods = protocol_methods - instance_methods

    if missing_methods:
        raise ProtocolNotCompliantError(f"Instance of {type(instance).__name__} does not implement " + f"the following methods required by {protocol.__name__}" + ":\n\t-" + "\n\t -".join(missing_methods))
...
def get_prepended_values(kwargs: t.Dict[str, t.Any], blok_name: str):
    prepended = {key.split("_", 1)[1]: value for key, value in kwargs.items() if key.startswith(blok_name)}

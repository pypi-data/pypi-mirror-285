...
from sifflet_sdk.client.model.workspace_apply_object_response_dto import WorkspaceApplyObjectResponseDto
...
@click.pass_context
def apply(ctx: Context, file_name: str, dry_run: bool, force_delete: bool, fail_on_error: bool, verbose: bool):
    """
...
def print_response(response: WorkspaceApplyResponseDto, verbose: bool) -> None:
    ...
    if verbose:
        ...
        response_dict = {**response.to_dict(), "changes": [change.to_dict() for change in filtered_changes]}
...
def count_changes(changes: List[WorkspaceApplyObjectResponseDto], change_type: str, sub_status: str, kind: str) -> int:
    return sum(
        1

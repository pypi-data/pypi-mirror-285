...
from pandas import DataFrame, DatetimeIndex, PeriodIndex, Period, read_excel, Series, Timestamp, period_range
...
def read_rba_ocr(monthly: bool = True, **kwargs: Any) -> Series:
    ...
    depending on the value of the monthly parameter."""

    rba, rba_meta = read_rba_table("A2", **kwargs)
    ocr = rba["ARBAMPCNCRT"]
    ...
    if not monthly:
        return ocr
    ocr = ocr[ocr.index >= "1990-08-02"].astype(float).sort_index()
    ocr.index = PeriodIndex(ocr.index, freq="M")
    ocr = ocr.sort_index()
    ocr = ocr[~ocr.index.duplicated(keep='last')]
    new_index = period_range(start=ocr.index.min(), end=ocr.index.max(), freq="M")
    ocr = ocr.reindex(new_index, method="ffill")
    ocr.name = "Official Cash Rate"
    return ocr

...
class Zerodha(Broker):
    ...
    def historical_data(
        ...
        if tradingsymbol:
            ...
            instrument_token = self.symbol_data[f"{exchange}:{tradingsymbol}"].get(
                "instrument_token"
            )
            ...
            t = [
                ...
                if exchange == a["exchange"]
                and str(token) == str(a.get("exchange_token"))
            ]

            if len(t) != 1:
                raise InvalidArgumentException(f"Invalid token {token}")
            instrument_token = t[0].get("instrument_token")

...
class IIFL(Broker):
    ...
    def profile(self):
        api_response = self.wrapper.profile(client_id=self.client_code)
    ...
    def positions(self):
        api_response = self.wrapper.net_positions(self.client_code)["NetPositionDetail"]
    ...
    def get_exchange(self, exchange):
        ...
        if exchange not in exchange_code_map:
            raise KeyError("INVALID_EXCHANGE: Exchange not in ['NSE','BSE','MCX']")
        return exchange_code_map[exchange]

    def get_exchange_type(self, exch_type):
        exchange_type_code_map = {"CASH": "C", "DERIVATIVE": "D", "CURRENCY": "U"}
    ...
    def get_symbol(self, exchange, symbol):
        ...
        scripcode = int(
            scrip_data.loc[scrip_data["Name"] == symbol].loc[
                scrip_data["Exch"] == exchange
            ]["Scripcode"]
        )

        return scripcode

    def ltp(self, exchange, symbol, exch_type="C"):

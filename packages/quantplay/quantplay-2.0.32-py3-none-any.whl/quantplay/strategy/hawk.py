...
class Hawk(QuantplayAlgorithm):
    ...
    def get_trades(self, market_data):
        ...
        trades = trades[trades.symbol == "NIFTY BANK"]

        trades.loc[:, "transaction_type"] = "SELL"
        trades.loc[:, "quantity"] = np.where(trades.symbol == "NIFTY 50", 100, 100)
        trades.loc[:, "stoploss"] = np.where(trades.symbol == "NIFTY 50", 1, 1.2)

...
# Attempting to import the variable here so that we
# hard crash at import time if the variable is not
# available when we manually upgrade bigquery from
...
try:
    ...
    import google.cloud.bigquery._versions_helpers

    _is_read_session_optional = google.cloud.bigquery._versions_helpers.BQ_STORAGE_VERSIONS.is_read_session_optional
...
def _download_table_bqstorage_stream_no_deadlock(
    ...
    if _is_read_session_optional is None:
        raise Exception("Monkeypatched function called without successfully importing depending variable")
...
try:
    ...
    if env_var_bool("CHALK_FALLBACK_TO_ORIGINAL_TABLE_DOWNLOAD_FUNC"):
        _logger.debug("Not monkeypatching BQ table download function")
    else:
        ...
        if not hasattr(google.cloud.bigquery._pandas_helpers, _download_func_name):
            raise Exception(f"Expected to find function {google.cloud.bigquery._pandas_helpers}.{_download_func_name}")
        setattr(

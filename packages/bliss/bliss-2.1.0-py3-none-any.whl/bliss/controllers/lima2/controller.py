...
class RoiStatController(IntegratingCounterController):
    ...
    def get_values(self, from_index, *counters: list):
        ...
        values = self._dev.current_pipeline.pop_roi_statistics(10)
        ...
        if not values:
            ...
            for c in counters:
                res.append(
                    values[0][name2idx[c.roi.name] :: len(self._rois)][c.stat.value]
                )
...
class IntegratingController(IntegratingCounterController):
    def __init__(self, device, master_controller):
        super().__init__(
            device.name, master_controller=master_controller, register_counters=False
        )
    ...
    def get_values(self, from_index, *counters):
        values = self._dev.current_pipeline.pop_fill_factors(10)
        _logger.debug(f"pop_fill_factors returned {len(values)} values")

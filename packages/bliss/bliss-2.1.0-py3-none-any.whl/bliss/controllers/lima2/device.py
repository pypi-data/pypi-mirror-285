...
class Lima2(CounterContainer, Settings):
    ...
    def __init__(self, config):
        ...
        self._det = Detector(tango_ctrl_dev, *tango_recv_devs)
        self._det_info = None
        self._version = None
        ...
        state_history = [self._det.state]
        self._transition_evt = gevent.event.Event()

        def on_state_change(state):
            _logger.debug(f"transition from {state_history[-1]} to {state}")
            state_history.append(state)
            self._transition_evt.set()

        self._det.register_transition_logger(on_state_change)

        params_default = self._det.params_default
        ...
        plugins_enum = {}
        for recv in self._det.recv_processings:
            procs = {}
            for cls, schema in recv.items():
                try:
                    import importlib

                    plugin = cls.removeprefix("LimaProcessing").lower()
                    module = importlib.import_module(
                        __package__ + ".processings" + f".{plugin}"
                    )

                    if plugin not in self._processing_plugins:
                        procs[plugin] = module.Processing(self, schema)
                        plugins_enum[plugin] = cls
                except ImportError:
                    _logger.warning(f"could not find a plugin for processing {plugin}")
            self._processing_plugins.update(procs)

        # Register the enum at the module namespace scope
        global ProcPlugin
        if ProcPlugin is None:
            ProcPlugin = enum.Enum("ProcPlugin", plugins_enum)

        # Read settings after ProcPlugin is initializaed
        ...
        class Acquisition(Settings):
            ...
            def expo_time(self, value: numbers.Number):
                range = self._device.det_capabilities.get(
                    "expo_time_range", [1000, 100000000]
                )
    ...
    def counters(self):
        return counter_namespace(self._detector.counters + self._processing.counters)
    ...
    @proc_plugin.setter
    # def proc_plugin(self, plugin: str | enum.Enum): # Python 3.10 alternatives
    def proc_plugin(self, plugin):
        if isinstance(plugin, str):
            self._proc_plugin = ProcPlugin[plugin]
            self._processing = self._processing_plugins[plugin]
        else:
    ...
    # @logger
    # def _get_pipeline(self, uuid):
    #     _logger.debug(f"UUID {uuid}")
    #     return self._det.get_pipeline(uuid)

...
class Lima2AcquisitionMaster(AcquisitionMaster):
    ...
    def prepare(self):
        ...
        for p in [self.ctrl_params["ctrl"], *self.ctrl_params["recvs"]]:
            ...
            p["acq"]["latency_time"] = int(self.acq_params.get("sleep_time", 0) * 1e6)
        ...
        channels_metadata.update({"raw_frame": channels_metadata["input_frame"]})
        ...
        def on_error(evt):
            ...
            if not evt.err and p.uuid == self.acq_uuid and self._reading_task:
                _logger.error(f"processing {p.uuid} failed with {p.last_error}")
                # self.stop()
                self._reading_task.kill(
                    RuntimeError(f"processing {p.uuid} failed with {p.last_error}")
                )
    ...
    def _emit_status(self, status):
        progress_keys = ["nb_frames_acquired", "nb_frames_xferred", "state"]
        ...
        if self.channels and status["nb_frames_xferred"] >= 0:
            payload = {
                ...
                # "last_index_saved": nb_frames_saved,
        ...
        while True:
            ...
            try:
                ...
                if not self.prepare_once:
                    status["nb_frames_xferred"] = self.__sequence_index * self.npoints
                else:
                    status["nb_frames_xferred"] = self._dev.nb_frames_xferred
            except Exception:
                _logger.warning(f"{self.name} is in FAULT state")
            ...
            if nb_frames > last_nb_frames:
                status["last_image_acquired"] = nb_frames
            ...
            if status["state"] == State.FAULT:
                raise RuntimeError(f"Detector {self.name} is in Fault state")
            ...
            if self._dev._det.state in [State.RUNNING, State.STARTING]:

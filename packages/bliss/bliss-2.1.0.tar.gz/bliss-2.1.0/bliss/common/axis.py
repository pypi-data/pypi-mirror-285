...
class GroupMove:
    ...
    def _find_sub_axes(self, axis_pos_dict, found_axes):
        """
        axis_pos_dict: dict of {axes: target_pos} passed to the motion cmd
    ...
    def _resolve_motions_tree(self, axis_pos_dict, relative, polling_time):
        ...
        found_axes = []
        self._find_sub_axes(axis_pos_dict, found_axes)
...
class Axis(Scannable, HasMetadataForDataset):
    ...
    def __info__(self):
        ...
        try:
            ...
            info_string += "     ERROR: unable to get position\n"
        ...
        try:
            ...
            if self._disabled:
                info_string += "            Axis disabled because of a problem during initialization\n"
        ...
        if self.controller is not None:
            info_string += "\n"
            info_string += "CONTROLLER:\n"
            info_string += f"     {bliss_obj_ref(self.__controller)}\n"
            info_string += (
                f"     type {BOLD(f'{self.name}.controller')} for more information\n"
            )

...
class MockDatabase():
    ...
    def get(self, path: str, shallow=False):
        ...
        record = self._get_from_store(keys)
        if shallow:
    ...
    def set(self, path: str, record: dict):
        keys = self._get_keys(path)
        record = serialise_input(record)
        record = dict_to_default_dict(record)
        self._set_in_store(keys, record)
    ...
    def where(self, path, **kwargs):
        keys = self._get_keys(path)
        records = self._get_from_store(keys)
    ...
    def remove(self, path: str):
        keys = self._get_keys(path)
        parent = self._get_from_store(keys[:-1])
        parent.pop(keys[-1])
    ...
    def _set_in_store(self, keys, value):
        ...
        if keys:
            self._get_from_store(keys[:-1])[keys[-1]] = value
        else:
            self.store = value

    def _update_in_store(self, keys, value):
        current_value = self._get_from_store(keys[:-1]) or {}

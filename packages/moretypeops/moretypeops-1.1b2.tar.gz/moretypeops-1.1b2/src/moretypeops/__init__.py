...
class Float:
    ...
    def __imod__(self, value: float) -> Float:
        ...
        return self
    def __itruedivmod__(self, value: float) -> Tuple[Float, Float]:
        result = self.value.__divmod__(value)
...
class Str(str):
    ...
    def __sub__(self, other: list | tuple | str):
        if isinstance(other, list) or isinstance(other, tuple):
            if len(other) != 2:
                raise NotImplemented
            return self.value.replace(str(other[0]), "", other[1])
        return self.value.replace(str(other), "")
    ...
    def __iadd__(self, other):
        ...
        return self
    def __isub__(self, other: str | list | tuple):
...
class Dict(dict):
    ...
    def __str__(self):                        return str(dict(self.value))

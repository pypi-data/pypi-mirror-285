...
#     # base, in big endian
#     __base = tf.convert_to_tensor([base ** __e for __e in range(depth)[::-1]], dtype=data.dtype)
#     # reshape: input rank + 1
#     __base = tf.reshape(__base, shape=__shape)
#     # select each bit from the original data
#     __digits = tf.bitwise.bitwise_and(tf.expand_dims(data, -1), __base)
#     # format

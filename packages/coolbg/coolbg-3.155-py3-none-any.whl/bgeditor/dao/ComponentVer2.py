...
def create_video(list_comp_data, path_video, job_id, mf_server):
    ...
    for comp in arr_comps:
        if is_music and comp.duration>intro_time and not comp.is_intro and not comp.is_outro:
    ...
    for comp in arr_comps:
        ...
        if comp.type == "video":
            ...
            is_cont = False
            if comp.is_intro:
                is_intro_music_bg=True
                intro_path = os.path.join(get_dir('coolbg_ffmpeg'), str(uuid.uuid4()) + "-intro.mp4")
                rs = comp.get_clip()
                if is_intro_music_bg and audio_compilation_path:
                    length_of_intro=math.floor(rs.duration)
                    intro_audio_part1, audio_compilation_path = split_audio(audio_compilation_path, length_of_intro)
                    audio_intro = AudioFileClip(intro_audio_part1)
                    rs.set_audio(audio_intro)
                    rs.subclip(0,length_of_intro).write_videofile(intro_path, bitrate=base_bitrate, fps=base_fps, codec='libx264', audio_bitrate='128000')
                else:
                    if not rs.audio:
                        rs.close()

...
from .._config import IS_AUTO_SYNC, TODO_ABS_FILE_PATH, CURRENT_TIME
from ..log._helper import append_log_item, get_pretty_log_lines, get_log_file_name
    ...
    if len(items) == 0:
        show_lines(
            ...
            *get_pretty_todo_item_lines(
                get_todo_items(
                    file_name=TODO_ABS_FILE_PATH, completed=False
                )
            ),
        )
        return
...
def start_todo(*args, **kwargs):
    ...
    show_lines(
        ...
        *get_pretty_log_lines(file_name=get_log_file_name(CURRENT_TIME)),
        "",
        *get_pretty_todo_item_lines(
            get_todo_items(file_name=TODO_ABS_FILE_PATH)
        )
    )

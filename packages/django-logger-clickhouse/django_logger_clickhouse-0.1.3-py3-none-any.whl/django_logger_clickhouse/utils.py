    ...
    while attempt < max_attempts:
        try:
            ...
            if attempt < max_attempts:
                time.sleep(5)  # Пауза в 5 секунд между попытками
            else:
                print(f"Clickhouse-django-logger error: Failed to ensure ClickHouse DB and table exist after {max_attempts} attempts: {e}")
                return

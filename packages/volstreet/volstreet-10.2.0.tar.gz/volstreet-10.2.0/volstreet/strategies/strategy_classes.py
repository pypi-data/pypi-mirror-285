    ...
    def logic(
        ...
        position_manager = ThetaXDeltaPosition(
            ...
            exit_triggers={
                "end_time": False,
                "max_entries": False,
                "trend_delta_threshold": False,
            },
            notifier_url=self.webhook_url,
            order_tag=self.strategy_tag,
        )
        recording_task = partial(record_position_status, position_manager, file_name)
        recording_task = timed_executor(55)(recording_task)
        ...
        while current_time().time() < time(*exit_time) and not any(
            position_manager.exit_triggers.values()
        ):
            sleep_until_next_action(1, exit_time, tasks_to_perform=[recording_task])

            if (
                position_manager.current_state != "neutral"
                and abs(position_manager.aggregate_greeks().delta)
                <= 0.075 * quantity_in_shares
            ):
                position_manager.exit_triggers["trend_delta_threshold"] = True
                break

...
_TRANS_TREND.extend(["ETA", "TCX", "CP"])
_TV_STRING = "T;V"
...
class Fluid:
    ...
    def set_state(self, values, given="TP", wanted=_THERMO_STRING, composition=None):
        ...
        if self.fluidmodel.props == "REFPROP":
            self._set_state_refprop(values, given, wanted, composition)
    ...
    def _set_state_refprop(self, values, given, wanted, composition):
        ...
        if all_values["T"] > 0:  # on error it is -8888
            ...
            if wanted == _TRANS_STRING:
                ...
                for tr_want  in _TRANS_TREND[-3:]:
                    ...
                    trans_dict[tr_want],err = self.fluidmodel.rp_instance.TREND_EOS(*values)

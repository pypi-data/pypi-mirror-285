...
def read_stream_from_kafka(spark, topic, schema) -> DataFrame:
    ...
    df = spark \
        ...
        .load()
    df = df.select(col("value").cast("string").alias("json_string"), unix_millis(col("timestamp")).alias("timestamp"))
    df = df.select(from_json(col("json_string"), schema).alias("data"), col("timestamp")).select("data.*", "timestamp")
    return df

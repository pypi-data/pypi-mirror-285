...
from sink_sdk.types.model_referenced_in_parent_and_child import ModelReferencedInParentAndChild
...
class TestAsyncChild:
    ...
    async def test_streaming_response_retrieve(self, async_client: AsyncSink) -> None:
        async with async_client.model_referenced_in_parent_and_child.child.with_streaming_response.retrieve() as response:
            assert not response.is_closed

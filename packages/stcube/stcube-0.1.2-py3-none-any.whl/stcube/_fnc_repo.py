    ...
    * Can only manage small MODS (cannot manage Library)
    ...
    def loading(self):
        ...
        if not os.path.exists(INDEX_PATH):
            with open_unknown_encoding_file(INDEX_PATH, 'w') as f:
    ...
    def visit(self):
        with open_unknown_encoding_file(INDEX_PATH, 'r') as f:
            urls = [url.strip() for url in f.readlines() if url.strip()]

        if not urls:
            print("no url in index" if setting.language == 'en' else "索引文件中没有url")
            return

        names = [get_url_name(url) for url in urls]

        print("Index urls:" if setting.language == 'en' else "索引文件中的url:")
        for i, url in enumerate(urls):
            print(f"  {i+1}: {url}")

        index = input("input index: " if setting.language == 'en' else "输入索引: ")
        if not index.isdigit() or int(index) > len(urls):
            print("invalid index" if setting.language == 'en' else "无效的索引")
            return

        url = urls[int(index)-1]
        print(f"Visit {url}" if setting.language == 'en' else f"访问 {url}")
        self.local_repo = STCubeModuleRepo(url, specific_dir=LOCAL_REPO)

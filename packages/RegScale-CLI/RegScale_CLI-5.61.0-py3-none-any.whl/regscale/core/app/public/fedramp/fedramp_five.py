...
def handle_matching_objectives(
    ...
    for objective in matching_objectives:
        ...
        existing_options = ImplementationOption.get_all_by_parent(
            parent_id=objective.securityControlId, plan_id=parent_id
        )
        part_statement = f"{part.get('value', '')}"
        option = get_or_create_option(
            part_name=f"{control.controlId}:{part_name}",
            part_value=part_statement,
            control=control,
        ...
        if has_existing_obj:
            continue
        duplicate = True if option and option.id in option_ids_used else False

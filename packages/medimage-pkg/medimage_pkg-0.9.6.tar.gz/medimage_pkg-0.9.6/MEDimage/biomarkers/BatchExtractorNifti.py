...
class BatchExtractorNifti(object):
    """
    ...
    def __associate_roi_to_image(self, path_roi, image_file, medscan, nifti):
        ...
        file = image_file.parent / (_id + '(GTV).ROI.nii.gz')
        ...
        if file.name.startswith(_id) and 'ROI' in file.name.split("."):
            ...
            #roi_data = medscan.data.ROI.convert_to_LPS(data=roi_data)
            indexes_1 = np.where(roi_data.flatten() == 1)[0]
            indexes_2 = np.where(roi_data.flatten() == 2)[0]
            if len(indexes_1) > 0:
                medscan.data.ROI.update_indexes(key=roi_index, indexes=indexes_1)
                medscan.data.ROI.update_name_set(key=roi_index, name_set=name_set)
                medscan.data.ROI.update_roi_name(key=roi_index, roi_name='GTVp')
                roi_index += 1
            if len(indexes_2) > 0:
                medscan.data.ROI.update_indexes(key=roi_index, indexes=indexes_2)
                medscan.data.ROI.update_name_set(key=roi_index, name_set=name_set)
                medscan.data.ROI.update_roi_name(key=roi_index, roi_name='GTVn')
                roi_index += 1
        return medscan

    def __associate_spatialRef(self, nifti_file: Union[Path, str], medscan):
    ...
    def __process_one_nifti(self, path_roi, nifti_file: Union[Path, str]) -> MEDimage.MEDscan:
        ...
        # UPDATE spatialRef
        medscan = self.__associate_spatialRef(nifti_file, medscan)
        # GET ROI
        medscan = self.__associate_roi_to_image(path_roi, nifti_file, medscan, nib.load(nifti_file))
    ...
    def __compute_radiomics_one_patient(
        ...
        try:
            medscan = self.__process_one_nifti(self._path_read, self._path_read / name_patient)
    ...
    def __batch_all_patients(self, im_params: Dict) -> None:
        ...
        for r in range(0, n_roi_types):
            ...
            tabel_roi['under'] = '_'
            tabel_roi['dot'] = '.'
            tabel_roi['roi_label'] = 'GTV'
            tabel_roi['oparenthesis'] = '('
            tabel_roi['cparenthesis'] = ')'
            tabel_roi['ext'] = '.nii.gz'
            ...
            tabel_roi = tabel_roi.drop(columns=['under', 'under', 'dot', 'oparenthesis', 'cparenthesis', 'roi_label', 'ext'])

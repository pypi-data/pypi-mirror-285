...
class UserDatabase(SQLAlchemyUserDatabase):
    ...
    async def create(self, create_dict: Dict[str, Any]) -> UP:
        ...
        await smart_run(self.session.commit)
        await smart_run(self.session.refresh, user)
        return user
...
class QueryManager:
    ...
    def __init__(self, db: AsyncSession | Session, datamodel: Any):
        self.db = db
        ...
        *,
        ...
        if filter_classes:
            for col, filter_class, value in filter_classes:
                await smart_run(self.filter_class, col, filter_class, value)
    ...
    async def delete(self, item: Model):
        ...
        """
        await smart_run(self.db.delete, item)
    ...
    async def commit(self):
        ...
        """
        try:
            await smart_run(self.db.commit)
        except IntegrityError as e:
            await smart_run(self.db.rollback)
    ...
    async def _apply_filter(
        ...
        if inspect.iscoroutinefunction(cls.apply):
            self.stmt = await cls.apply(self.stmt, col, value)
        else:
            self.stmt = cls.apply(self.stmt, col, value)
...
class DatabaseSessionManager:
    ...
    async def close(self):
        ...
        """
        if self._engine:
            await smart_run(self._engine.dispose)
            self._engine = None
            self._sessionmaker = None

        if self._engine_binds:
            for engine in self._engine_binds.values():
                await smart_run(engine.dispose)
        ...
        ***EVEN IF THE SESSION IS SYNC, ASYNC WITH ... AS ... IS STILL NEEDED.***
        ...
        if bind:
            ...
            try:
                ...
                await smart_run(session.rollback)
                raise
            finally:
                await smart_run(session.close)
        else:

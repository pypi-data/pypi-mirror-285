...
from netbox_cesnet_services_plugin.filtersets import BGPConnectionFilterSet, LLPDNeighborFilterSet, LLPDNeighborLeafFilterSet
from netbox_cesnet_services_plugin.models import BGPConnection, LLDPNeighbor, LLDPNeighborLeaf
from netbox_cesnet_services_plugin.tables import BGPConnectionTable, LLDPNeighborTable, LLDPNeighborLeafTable
...
class LLDPNeighborPluginTemplateExtension(PluginTemplateExtension):
    ...
    def get_page(self):
        ...
        lldp_neighbor_table = LLDPNeighborTable(lldp_neighbors, exclude=self.exclude)

        return self.render(
            "netbox_cesnet_services_plugin/lldpneighbor/lldpneighbor_include.html",
            extra_context={"lldpneighbors": lldp_neighbors, "related_session_table": lldp_neighbor_table},
        )
...
class BGPConnectionPluginTemplateExtension(PluginTemplateExtension):
    ...
    def get_page(self):
        ...
        bgp_connection_table = BGPConnectionTable(bgp_connections, exclude=self.exclude)

        return self.render(
            "netbox_cesnet_services_plugin/bgpconnection/bgpconnection_include.html",
            extra_context={"bgpconnections": bgp_connections, "related_session_table": bgp_connection_table},
        )
...
def create_bgpconnection_tabview(model):
    ...
    class View(generic.ObjectChildrenView):
        ...
        if model == Tenant:
            ...
            def get_table(self, data, request, bulk_actions=True):
                table = super().get_table(data, request, bulk_actions)
                table.exclude = "bgp_prefix_tenant"
                return table
            ...
            def get_table(self, data, request, bulk_actions=True):
                table = super().get_table(data, request, bulk_actions)
                table.exclude = "next_hop_interface"
                return table

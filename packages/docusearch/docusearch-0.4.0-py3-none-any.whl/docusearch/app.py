...
def query_chunk(chunk_num, chunk, question, api_key, model="gpt-4", short_response=False):
    openai.api_key = api_key
    ...
    prompt = f"Document chunk {chunk_num}:\n{chunk}\n\nQuestion: {question}\n\nProvide a concise answer and cite sentences from the document." if short_response else f"Document chunk {chunk_num}:\n{chunk}\n\nQuestion: {question}\n\nProvide a detailed answer and cite sentences from the document."
...
def get_answers_with_citations(question, document_chunks, api_key, model="gpt-4", short_response=False):
    all_evidence = []
    with ThreadPoolExecutor() as executor:
        futures = [executor.submit(query_chunk, i+1, chunk, question, api_key, model, short_response) for i, chunk in enumerate(document_chunks)]
...
def process_best_matching_document(query_text, api_key, folder_path, model="gpt-4", short_response=False):
    ...
    index_creation_time = time.time() - index_creation_start_time
    logging.info(f"FAISS index creation time: {index_creation_time:.2f} seconds")
    ...
    faiss_query_time = time.time() - faiss_query_start_time
    logging.info(f"FAISS index query time: {faiss_query_time:.2f} seconds")
    ...
    evidence_list = get_answers_with_citations(query_text, document_chunks, api_key, model, short_response)
...
def process_query(query_text, api_key, folder_path, model="gpt-4", short_response=False):
    ...
    best_metadata, answer, unsupported_files = process_best_matching_document(query_text, api_key, folder_path, model, short_response)

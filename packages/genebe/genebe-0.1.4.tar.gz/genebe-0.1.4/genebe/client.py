...
def _parse_string_variant(variant):
    parts = variant.split("-")
    if len(parts) != 4:
        # Handle invalid input
        warning_message = f"Invalid input: {variant}"
        print(warning_message)
        return {}  # Return empty dictionary for invalid input
    else:
        return {"chr": parts[0], "pos": int(parts[1]), "ref": parts[2], "alt": parts[3]}
...
def lift_over_variants(
    ...
    vars = [_parse_string_variant(variant) for variant in variants]

    print(f"this is my payload: {json.dumps(vars)}")

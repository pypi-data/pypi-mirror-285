...
def _parse_string_variants(variants: List[str]) -> List[Dict[str, str]]:
    ...
    Returns:
        List[Dict[str, str]]: A list of dictionaries representing valid variants.
    ...
    # Handle invalid input by checking if any column is missing
    invalid_variants = df.isnull().any(axis=1)

    # Print warnings for invalid inputs
    for variant in df[invalid_variants]["variant"]:
        print(f"Invalid input: {variant}")

    # Convert the 'pos' column to integers
    df["pos"] = pd.to_numeric(df["pos"], errors="coerce")

    # Handle rows with non-numeric 'pos'
    invalid_pos = df["pos"].isnull()
    for variant in df[invalid_pos]["variant"]:
        print(f"Invalid input: {variant}")

    # Create a list of dictionaries for valid variants
    valid_variants = df.dropna().to_dict(orient="records")

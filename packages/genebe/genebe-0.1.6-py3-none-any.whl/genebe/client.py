...
def _parse_string_variants(variants: List[str]) -> List[Dict[str, str]]:
    """
    Parses a list of string variants (chr-pos-ref-alt) and returns a list of dictionaries representing valid variants.
    Invalid inputs are represented as empty dictionaries in the resulting list.
    I go through pandas df as it's faster than applying a function to a list.

    Args:
        variants (List[str]): A list of string variants.

    Returns:
        List[Dict[str, str]]: A list of dictionaries representing valid variants, with invalid inputs as empty dictionaries.

    Raises:
        None

    Examples:
        >>> variants = ["chr1-100-A-T", "chr2-200-C-G"]
        >>> _parse_string_variants(variants)
        [{'chr': 'chr1', 'pos': '100', 'ref': 'A', 'alt': 'T'}, {'chr': 'chr2', 'pos': '200', 'ref': 'C', 'alt': 'G'}]
    """
    # Create a DataFrame from the list of variants
    df = pd.DataFrame(variants, columns=["variant"])

    # Split the 'variant' column into separate columns
    split_variants = df["variant"].str.split("-", n=3, expand=True)

    # Ensure we have exactly four columns, filling missing parts with None
    split_variants = split_variants.reindex(columns=range(4), fill_value=None)

    # Rename the resulting columns
    split_variants.columns = ["chr", "pos", "ref", "alt"]

    # Merge the split columns back to the original dataframe
    df = pd.concat([df, split_variants], axis=1)
    ...
    # Print warnings for invalid inputs
    for variant in df[invalid_rows]["variant"]:
        print(f"Invalid input: {variant}")

    # Create a list of dictionaries for all variants, replacing invalid ones with empty dictionaries
    result = []
    for index, row in df.iterrows():
        if invalid_rows[index]:
            result.append({})
        else:
            result.append(
                {
...
def lift_over_variants(
    ...
    vars = _parse_string_variants(variants)

...
class gen:
    def license_key(slef):
        return '-'.join(
            ''.join(secrets.choice(string.ascii_letters + string.digits) for _ in range(secrets.randbelow(4) + 3)) for _
            in
            range(6))[:-1]

    def license_key_v2(self, min_part_length=3, max_part_length=6, num_parts=6, separator='-', total_length=None,

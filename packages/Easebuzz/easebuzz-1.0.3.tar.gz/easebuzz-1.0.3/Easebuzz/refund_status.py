"""Module for checking refund_status against a transaction
"""
...
def refund_status(params, merchant_key, salt, env):
    """
    refund_status method refund amount and call display the refund page.

    Args:
        params (dict):  A dictionary containing refund_status-related parameters.
    ...
    """
    try:
        result = _refund_status(params, merchant_key, salt, env)
        return result

    except Exception as _:
        traceback.print_exc()
        logger.error("#######Error on refund_status:refund_status#######")
        return ({"status": False, "reason": 'Exception occurred'})
    

def _refund_status(params, merchant_key, salt, env):
    """
    ...
    refund_status_url = utility.get_url(env)

    # process to start refund
    refund_status_result = _refund_payment_status(posted_array, salt, refund_status_url)

    return refund_status_result
...
def _type_validation(params):
    ...
    if not(isinstance(params['easebuzz_id'], str)):
...
def _refund_payment_status(params_array, salt_key, url):
    """
    ...
    # requests call for retrieve transaction
    request_result = requests.post(url + 'refund/v1/retrieve', params_array)
...
def _get_hash_key(posted, salt_key):
    ...
    hash format (hash sequence) :hash_string

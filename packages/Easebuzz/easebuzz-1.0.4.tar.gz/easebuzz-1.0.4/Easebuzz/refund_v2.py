"""Module for raising refund against a transaction
"""
...
def _empty_validation(params, salt):
    ...
    if not params['merchant_refund_id']:
...
def _type_validation(params):
    ...
    if not(isinstance(params['merchant_refund_id'], str)):
...
def _refund_payment(params_array, salt_key, url):
    ...
    request_result = requests.post(url + 'transaction/v2/refund', params_array)

...
WORKDIR = os.path.join(os.path.expanduser("~"), ".aimmocore")
...
def extract_mongodb(filepath, destination):
    ...
    for item in os.listdir(extracted_dir):
        ...
        if os.path.isdir(s):
            if os.path.exists(d):
                shutil.rmtree(d)  # Remove existing directory
            shutil.move(s, d)
        else:
            if os.path.exists(d):
                os.remove(d)  # Remove existing file
            shutil.move(s, d)
...
def install_mongodb(destination: str):
...
def main():
    install_dir = os.path.join(WORKDIR, "mongodb_installation")

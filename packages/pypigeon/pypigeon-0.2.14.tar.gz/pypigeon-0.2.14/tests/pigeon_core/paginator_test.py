...
import pytest
from pypigeon.pigeon_core import paginator
...
def test_paginate(httpx_mock, client):
    ...
    httpx_mock.add_response(
        url="https://pigeon.test/api/v1/collections?page_size=50",
        json={
            "collections": [
                make_collection("a"),
                make_collection("b"),
            ...
            "pagination": {
                "items_per_page": 5,
                "total_items": 7,
                "next_page_url": "https://pigeon.test/api/v1/collections?token=xyz",
            },
        },
    )
    httpx_mock.add_response(
        url="https://pigeon.test/api/v1/collections?token=xyz",
        json={
            "collections": [
                make_collection("f"),
                make_collection("g"),

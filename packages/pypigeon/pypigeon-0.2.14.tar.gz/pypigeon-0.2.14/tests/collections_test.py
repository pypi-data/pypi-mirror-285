...
import pandas as pd
import pytest
from pypigeon.collections import PigeonCollection
from pypigeon.pigeon_core.models import Collection
...
def collection(client):
    ...
    return PigeonCollection(m, client)


def test_list_folder(httpx_mock, collection):
    # list top level
    httpx_mock.add_response(
        url="https://pigeon.test/api/v1/collections/xyz/LIVE/items?in_folder=ROOT",
        json={
            "items": [
                {
                    "id": "f",
...
def test_get_and_list_folder(httpx_mock, collection):
    ...
    httpx_mock.add_response(
        url=(
            "https://pigeon.test/api/v1/collections/xyz/LIVE"
    ...
    httpx_mock.add_response(
        url="https://pigeon.test/api/v1/collections/xyz/LIVE/items?in_path=baz",
        json={
            "items": [
                {
                    "id": "1",
...
def test_walk_folder(httpx_mock, collection):
    ...
    assert [i.name for i in collection.walk()] == ["foo", "bar", "baz", "one", "two"]
...
def test_write_table(httpx_mock, collection):
    httpx_mock.add_response(
        method="POST",
        url="https://pigeon.test/api/v1/collections/xyz/LIVE/items",
        status_code=201,
        json={
            "id": "b",
            "name": "bar",

...
def perform_update(filename: click.Path):
    ...
    secret_update_result = create_or_update_github_secrets(
        repo=repo, env_data=secrets
    )
    not_secret_update_result = create_or_update_github_variables(
        repo=repo, env_data=not_secrets
    )

    results = {
        "SECRETS": [secret.name for secret in secret_update_result],
        "VARIABLES": [variable.name for variable in not_secret_update_result]
    }
...
def create_or_update_github_secrets(repo, env_data):
...
def create_or_update_github_variables(repo: Repository, env_data):
    vars = [var for var in repo.get_variables()]
    ...
    for env_var_name, env_var_value in env_data.items():
        ...
        if env_var_name in var_names:
            ...
            results.append(repo.create_variable(env_var_name, env_var_value))

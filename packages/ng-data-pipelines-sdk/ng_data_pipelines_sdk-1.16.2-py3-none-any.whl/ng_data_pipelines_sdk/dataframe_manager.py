...
class DataFrameManager:
    ...
    def apply_transform(
        ...
        if transform_params.fn_kind == FnKind.SINGLE:
            for df_name, df in dfs_to_transform.items():
                try:
                    transformed_df = transform_fn(df, **fn_kwargs)
                    if verbosity >= 1:
                        df.show(1, truncate=truncate)

                    input_dfs_dict[df_name] = transformed_df
                except Exception as e:
                    logger.error(
                        f"Error applying transformation to DataFrame '{df_name}'"
                    )
                    df.show(3, truncate=truncate)
                    raise ValueError(
                        f"Error applying transformation to DataFrame '{df_name}': {e}"
                    ) from e
            ...
            try:
                ...
                logger.error(
                    "Error applying batch transformation to DataFrames. Showing first 3 rows of each input DataFrame:"
                )
                for df_name, df in dfs_to_transform.items():
                    logger.error(f"DataFrame '{df_name}':")
                    df.show(3, truncate=truncate)
                raise ValueError(
                    f"Error applying batch transformation to DataFrames: {e}"
                ) from e
    ...
    def apply_all_transforms(
        ...
        for transform_id, transform_params in transform_params_dict.items():
            ...
            input_dfs_dict = self.apply_transform(
                input_dfs_dict, transform_params, verbosity, truncate
            )
        return input_dfs_dict
        ...
        try:
            ...
            if step_params.transform_params:
                dfs_to_output_dict = self.apply_all_transforms(
                    ...
                    transform_params_dict=step_params.transform_params,
                    verbosity=verbosity,
                    truncate=truncate,
                )
                print("")

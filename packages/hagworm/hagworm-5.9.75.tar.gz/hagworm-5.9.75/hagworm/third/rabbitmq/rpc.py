...
class RpcServer:
    ...
    async def _consume_message(self, message: aio_pika.IncomingMessage):
        ...
        with catch_error():
            ...
            _func_type = _message.get(r'type')
            _func_name = _message.get(r'func')
            ...
            try:
                ...
                if _func_type == FunctionType.Unary.value and inspect.iscoroutinefunction(_function):
                    ...
                    raise Exception(f'function not supported: {_message}')

            except Exception as err:

                await self._reply_error(message, _func_name, str(err))

                if _func_type == FunctionType.Stream.value:
                    await self._reply_eof(message)
    ...
    async def _consume_message(self, message: aio_pika.IncomingMessage):
        ...
        with catch_error():
            ...
            if correlation_id in self._unary_response:
                ...
                raise Exception(f'unexpected message: {correlation_id=} {message_body=}')
    ...
    async def call_stream(
        ...
        try:
            ...
            async with async_timeout.timeout(request_timeout):
                ...
                while (_response := await queue.get()) != STREAM_EOF:

                    if r'error' in _response:
                        raise RpcServerError(_response[r'error'])

                    yield _response[r'data']

...
def bubble_sort(lst: list, round: bool = False) -> list:
    """
    A simple bubble sort function ~(￣▽￣)~*\n

    :param lst: The list you need to sort
    :param round: Rounding floating-point numbers
    ...
    for _i in range(len(lst)):
        if not (isinstance(lst[_i], int) or isinstance(lst[_i], float)):
            return lst
...
def replace_str(string: str, __c: str, __nc: str, num=0) -> str:
    ...
    if len(__c) == 1 and num == 0:
        ...
        new_string = str("".join(lst_string))
        try:
            del _i, _j, temp, temp2, temp_bool
        except UnboundLocalError:
            pass
        gc.collect()
        return new_string
...
def reverse_str(string: str) -> str:
    """
    A very very easy function to reverse str
    :param string: The string you want to reverse
    :return: the reversed str
    """
    return str("".join(list(reversed(list(string)))))

...
def replace_str(string: str, __c: str, __nc: str, num=0) -> str:
    """
    Change the character in the string to a new character, but unlike "str.replace()", num specifies the number of original strs that that need to change (not the maximum number of changes)
            ...
            if temp == __c:
                ...
                for _j in range(len(__nc)):
                    if len(__nc) > len(__c):
                        ...
                        lst_string[_i + _j] = temp2[_j]
                        if _j == len(__nc) - 1:
                            lst_string.pop(_i + _j + 1)
        new_string = str("".join(lst_string))
        del _i, _j, temp, temp2
        gc.collect()
        return new_string

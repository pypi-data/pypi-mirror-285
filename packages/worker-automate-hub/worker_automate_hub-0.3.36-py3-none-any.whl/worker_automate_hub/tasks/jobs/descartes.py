...
async def descartes(task):
    ...
    try:
        ...
        try:
            ...
            field_modelo_faturamento = pyautogui.locateOnScreen(ASSETS_BASE_PATH + "field_modelo_faturamento.png", confidence=0.85, grayscale=False )
        except:
            field_modelo_faturamento = None

        if field_modelo_faturamento is not None:
            pyautogui.click(field_modelo_faturamento.left + 100, field_modelo_faturamento.top + 5)
            time.sleep(3)
            
            field_to_select_modelo = None
            ...
            for _ in range(13):
                ...
                if field_to_select_modelo is not None:
                    break
                time.sleep(1)

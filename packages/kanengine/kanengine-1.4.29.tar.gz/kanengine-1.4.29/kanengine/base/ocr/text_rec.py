        ...
        if raw:
            ...
            for i in range(length):
                if t[i] != 0 and (not (i > 0 and t[i - 1] == t[i])):
                    # print(self.alphabet)
                    # print("Minimum value of t:", min(t))
                    # print("Maximum value of t:", max(t))
                    char_list.append(self.alphabet[t[i] - 1])
                    # char_list.append(self.alphabet)
            return ''.join(char_list)


class TextRecognizer:
    def __init__(self, weight_path: str, dict_path: str):
        self.sess = onnxruntime.InferenceSession(weight_path)
        self.alphabet = list(
            map(lambda x: x.decode('utf-8').strip("\n").strip("\r\n"), open(dict_path, 'rb').readlines()))

    ...
    def process_data(self, df, weights):
        ...
        self.df = (
            self.df.assign(
                ...
                Lipinski=pd.to_numeric(self.df['Lipinski'], errors='coerce').apply(lambda x: 0.526 if x < 2 else 0),
                Pfizer=pd.to_numeric(self.df['Pfizer'], errors='coerce').apply(lambda x: 0.52 if x < 2 else 0),

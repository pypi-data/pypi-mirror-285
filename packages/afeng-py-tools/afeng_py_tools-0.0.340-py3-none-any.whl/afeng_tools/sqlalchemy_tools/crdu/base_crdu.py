...
@auto_commit_db()
def create_commit_query(model_class, callback: Callable, *criterion,
                        db: Session = None) -> Any:
    """
    创建提交的query
...
@auto_commit_db()
def save_batch_by_where(model_list: list, criterion_fun: Callable,
                        db: Session = None,
...
def query_all(model_class, *criterion, db: Session = None,
    ...
    db_query = create_query(model_class, *criterion, db=db).order_by(_order_by)

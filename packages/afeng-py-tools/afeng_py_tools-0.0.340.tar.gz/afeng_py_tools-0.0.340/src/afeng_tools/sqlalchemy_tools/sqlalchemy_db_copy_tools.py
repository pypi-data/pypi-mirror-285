...
def create_db_instance(database_uri: str) -> SqlalchemyDb:
    return SqlalchemyDb(DatabaseInfoItem(database_uri=database_uri))


def run_db_query(db_instance: SqlalchemyDb, query_callback: Callable[[Session, ...], Any], **kwargs) -> Any:
...
class DatabaseCopyTool:
    ...
    def __init__(self, source_database_uri: str, target_database_uri: str):
        self.source_db_instance = create_db_instance(source_database_uri)
        self.target_db_instance = create_db_instance(target_database_uri)

    def query_source_db_data(self, query_callback: Callable[[Session], Any], **kwargs) -> Any:
        """查询源数据库数据"""
        return run_db_query(db_tool=self.source_db_instance, query_callback=query_callback, **kwargs)

    def query_target_db_data(self, query_callback: Callable[[Session], Any], **kwargs) -> Any:
        """查询目标数据库数据"""
        return run_db_query(db_tool=self.target_db_instance, query_callback=query_callback, **kwargs)
    ...
    def save_target_data(self, model, *criterion, exist_update: bool = True, is_commit: bool = True):
        ...
        def fun_wrap(session: Session):
            ...
            if old_data:
                if exist_update:
                    db_query.update(sqlalchemy_model_tools.to_dict(model))
                    if is_commit:
                        session.commit()
...
if __name__ == '__main__':
    ...
    for old_group in old_group_list:
        new_group = sqlalchemy_model_tools.copy_model_data(old_group, 'new_po.GroupInfoPo')

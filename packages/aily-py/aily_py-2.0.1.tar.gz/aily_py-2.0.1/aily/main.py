...
class Aily:
    """
    Aily class represents the main class for the Aily application.
    
    Observable:
        wakeup (Subject): Subject for wakeup event.
        on_function_call (Subject): Subject for function call event.
        on_custom_invoke (Subject): Subject for custom invoke event.
        on_custom_invoke_vision (Subject): Subject for custom invoke vision event.
        on_record_begin (Subject): Subject for record begin event.
        on_record_end (Subject): Subject for record end event.
        on_play_begin (Subject): Subject for play begin event.
        on_play_end (Subject): Subject for play end event.
        on_recognition (Subject): Subject for recognition event.
        on_direction (Subject): Subject for direction event.
        on_invoke_start (Subject): Subject for invoke start event.
        on_invoke_end (Subject): Subject for invoke end event.
        on_invalid_msg (Subject): Subject for invalid message event.
        on_conversation_records (Subject): Subject for conversation records event.
        on_error (Subject): Subject for error event.
        aily_event (Subject): Subject for Aily event.

    Attributes:
        port: Serial port device path configuration
        baudrate: Baud rate configuration
        serial_timeout: Serial read timeout configuration
        conversation_mode: Conversation mode configuration, values: `single/multi/manual`, default: `single`
        llm_key: Large model key configuration
        llm_model: Model used configuration
        llm_server: Large model service address configuration
        llm_temperature: Temperature configuration
        llm_pre_prompt: System prompt word configuration
        llm_max_token_length: Maximum token length configuration
        llm_vision_model: Vision model configuration
        llm_vision_key: Vision model key configuration
        llm_vision_server: Vision model service address configuration
        custom_invoke: Whether to customize model invocation, values: `True/False`, default: `False`
        custom_invoke_vision: Whether to customize vision model invocation, values: `True/False`, default: `False`
        use_aily_tts: Whether to use aily tts module, values: `True/False`, default: `False`
        tts_port: Aily tts serial port configuration
        tts_baudrate: Aily tts baud rate configuration
        loop_play_wait_words: Whether to loop play the waiting sound, values: `True/False`, default: `False`
        auto_play_wait_words: Whether to automatically play the waiting sound, values: `True/False`, default: `True`
        conversation_expired_at: Conversation record expiration time configuration in seconds, default: `300`
    ...
    Raises:
        RuntimeError: If LLM_KEY or LLM_URL is not set.

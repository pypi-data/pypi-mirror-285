...
class GoogleContactsPush(GoogleContacts):
    ...
    def initialize_service(self, email: str):
        ...
        token_data = self.user_externals_local.get_auth_details(
            username=email, system_id=GoogleContactConstants.GOOGLE_SYSTEM_ID,
    ...
    def get_contact_ids_with_google_people_api_resource_name(
            self, *, limit: int = 1, data_source_types_ids_list: list = None,  data_source_instances_ids_list: list = None,
            order_by: str = None) -> list[int]:
        self.logger.start('get_contact_ids_with_google_people_api_resource_name', object={'limit': limit})
        contacts_ids_list: list = []
        connection = Connector.connect('contact')
        params = []
        select_query = ''
        ...
        for result in results:
            contacts_ids_list.append(result[0])
        self.logger.end('get_contact_ids_with_google_people_api_resource_name',
                        object={'contacts_ids_list': contacts_ids_list})
        return contacts_ids_list
    ...
    def create_non_existing_google_contact_groups(self, *, contact_id: int) -> dict:
        ...
        for group in google_groups_lists:
            groups_titles_to_resource_name[group.get('name')] = group.get('resourceName')
    ...
    def update_contact_by_google_contact_dict(self, *, resource_name: str, google_contact_dict: dict, fields_to_update: list) -> dict:
        self.logger.start('update_contact_by_google_contact_dict', object={'resource_name': resource_name})
        if not google_contact_dict:
            self.logger.end('google_contact_dict is empty', object={'google_contact_dict': google_contact_dict})
            updated_contact = {}
            return updated_contact
        update_fields = ','.join(fields_to_update)
        try:
            # Retrieve the existing contact to get its etag
            existing_contact = self.service.people().get(
            ...
            self.logger.info('Contact updated successfully', object={'resource_name': resource_name})
        except Exception as exception:
            self.logger.exception('Exception in update_contact_by_google_contact_dict', object={'exception': exception})
            raise exception
        self.logger.end('update_contact_by_google_contact_dict')
        return updated_contact

    def update_google_contact_fields(self, *, username: str, contact_id: int, fields_to_update: list) -> dict:
        ...
        try:
            ...
            self.logger.exception('Exception in update_existing_contact', object={'exception': exception})
            raise exception
    ...
    def get_resource_name_by_contact_id(self, contact_id: int) -> str:
        resource_name = self.select_one_value_by_column_and_value(
            ...
            column_name='entity_id', column_value=contact_id
        )

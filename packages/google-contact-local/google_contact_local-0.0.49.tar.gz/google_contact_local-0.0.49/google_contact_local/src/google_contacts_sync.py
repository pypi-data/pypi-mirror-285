...
class GoogleContactsSync(GoogleContactsPush):
    ...
    def _insert_contact_details_to_db(self, contact_dict: dict, user_external_id: int,
        ...
        try:
            ...
            self._sync_organization_profile(
                contact_dict=contact_dict)
    ...
    def _sync_organization(self, contact_dict: dict):
        ...
        if not update_status_and_information_list:
            ...
            for update_status_and_information in update_status_and_information_list:
                ...
                for organizations_job_titles_dict in organizations_job_titles_dicts_list:
                    if organizations_job_titles_dict.get("name") == organization_name and organizations_job_titles_dict.get("title") == job_title_str:
                        # Update contact_dict["organizations_job_titles_dicts_list"]
                        organizations_job_titles_dict["organization_id"] = update_status_and_information.get(
                            "organization_id")
                        organizations_job_titles_dict["organization_ml_id"] = update_status_and_information.get(
                            "organization_ml_id")
                        organizations_job_titles_dict["job_title_id"] = update_status_and_information.get(
                            "job_title_id")
                        organizations_job_titles_dict["job_title_ml_id"] = update_status_and_information.get(
                            "job_title_ml_id")
    ...
    def _sync_organization_profile(self, contact_dict: dict, ) -> list:
        organizations_job_titles_dicts_list: list[dict] = contact_dict.get(
            "organizations_job_titles_dicts_list")
        main_profile_id = contact_dict.get("main_profile_id")
        if main_profile_id is None:
            ...
            for organization_job_title_dict in organizations_job_titles_dicts_list:
                ...
                organization_profile_id = self.organization_profile.upsert_mapping(
                    organization_id=organization_id, profile_id=main_profile_id, data_dict=data_dict)

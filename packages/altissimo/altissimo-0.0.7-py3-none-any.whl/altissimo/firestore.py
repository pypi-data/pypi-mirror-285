...
class FirestoreCollection:
    ...
    def __init__(self, fs, collection_name):
        """Initialize Collection class."""
        self.collection = fs.db.collection(collection_name)
    ...
    def list_documents_paginated(self, pagination) -> PaginatedList:
        """Return an embedded list of documents from this collection with pagination."""
        ref = self.collection.order_by(pagination.order_by)
        ...
        docs: list = list(ref.stream())
        next_cursor = ""
        if len(docs) == pagination.limit + 1:
            next_cursor = docs.pop().id
        return PaginatedList(items=docs, next_cursor=next_cursor)
    get_docs_paginated = list_documents_paginated
    get_documents_paginated = list_documents_paginated
    ...
    def list_dicts_paginated(self, pagination) -> PaginatedList:
        """Return an embedded list of document dicts from this collection with pagination."""
        response = self.list_documents_paginated(pagination)
        items = []
        for item in response.items:
            items.append(item.to_dict())
        return PaginatedList(items=items, next_cursor=response.next_cursor)
    get_documents_paginated = list_documents_paginated
...
class FirestoreUpdate:
        ...
        for key, item in data.items():
            ...
            if current[key] != item:
                if self.diff:
                    c = current[key]
                    output = []
                    for k in sorted(set(list(c) + list(item))):
                        o = c.get(k)
                        v = item.get(k)
                        if v != o:
                            output.append(f"  {k}: {o} -> {v}")
                    if output:
    ...
    def prepare_data(self, current, data):
        ...
        collection = self.c.collection_name
    ...
    def update(self, data):
        ...
        # convert all data keys to strings
...
class Firestore:
    ...
    # Pagination
    #
    def pagination(self, limit=100, cursor="", order_by="id"):

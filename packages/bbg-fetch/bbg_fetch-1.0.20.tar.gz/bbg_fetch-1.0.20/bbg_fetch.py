...
def fetch_fundamentals(tickers: List[str],
    ...
    df = df.reindex(index=tickers).reindex(columns=[x.lower() for x in fields])  # bdp changes columns to lower
    return df
...
def fetch_balance_data(tickers: List[str] = ['ABI BB Equity', 'T US Equity', 'JPM US Equity'],
    ...
    issue_data = issue_data.rename({x: x.upper() for x in issue_data.columns}, axis=1)  #  bdp return fields in small caps
...
def run_unit_test(unit_test: UnitTests):
    ...
    if unit_test == UnitTests.CHECK:
        this = blp.bdp(tickers=['AAPL US Equity', 'BAC US Equity'], flds=['Security_Name', 'GICS_Sector_Name', 'CRNCY'])
        print(this)

    elif unit_test == UnitTests.FUNDAMENTALS:
        df = fetch_fundamentals(tickers=['AAPL US Equity', 'BAC US Equity', 'MVOL LN Equity'],
                                fields=['Security_Name', 'GICS_Sector_Name', 'INDUSTRY_SECTOR', 'INDUSTRY_GROUP', 'CRNCY'])
        print(df)
...
if __name__ == '__main__':
    ...
    unit_test = UnitTests.FUNDAMENTALS

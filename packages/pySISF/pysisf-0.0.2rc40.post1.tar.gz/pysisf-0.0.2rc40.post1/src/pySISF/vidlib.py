...
def encode_stack(input_stack, method=EncoderType.X264, debug=False):
    ...
    nodes = ffmpeg.input(
        "pipe:", format="rawvideo", pix_fmt="gray", s=f"{h}x{w}", framerate="24/1"
    )
    ...
    if not len(out):
        raise ValueError(
            "No output receieved from ffmpeg. Is your chunk size sufficient?"
        )
...
def decode_stack(input_blob, dims=(128, 128), method="libx264", debug=False):
    nodes = ffmpeg.input("pipe:", framerate="24/1").output(
        "pipe:", format="rawvideo", pix_fmt="gray", framerate="24/1"
    )

    process = nodes.run_async(
        cmd=fmpeg_exe, pipe_stdout=True, pipe_stdin=True, pipe_stderr=(not debug)
    )

...
class TestAsyncAPI:
    ...
    async def test_method_list(self, async_client: AsyncOpenplugin) -> None:
        api = await async_client.api.list()
        assert_matches_type(object, api, path=["response"])

    @parametrize
    async def test_raw_response_list(self, async_client: AsyncOpenplugin) -> None:
        response = await async_client.api.with_raw_response.list()
    ...
    async def test_streaming_response_list(self, async_client: AsyncOpenplugin) -> None:
        async with async_client.api.with_streaming_response.list() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

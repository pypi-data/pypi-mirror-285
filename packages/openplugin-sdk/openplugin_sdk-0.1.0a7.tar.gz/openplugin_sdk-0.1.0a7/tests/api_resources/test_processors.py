...
class TestProcessors:
    ...
    def test_method_list(self, client: Openplugin) -> None:
        processor = client.processors.list()
        assert_matches_type(ProcessorListResponse, processor, path=["response"])

    @parametrize
    def test_raw_response_list(self, client: Openplugin) -> None:
        response = client.processors.with_raw_response.list()
    ...
    def test_streaming_response_list(self, client: Openplugin) -> None:
        with client.processors.with_streaming_response.list() as response:
...
class TestAsyncProcessors:
    ...
    async def test_method_list(self, async_client: AsyncOpenplugin) -> None:
        processor = await async_client.processors.list()
        assert_matches_type(ProcessorListResponse, processor, path=["response"])

    @parametrize
    async def test_raw_response_list(self, async_client: AsyncOpenplugin) -> None:
        response = await async_client.processors.with_raw_response.list()
    ...
    async def test_streaming_response_list(self, async_client: AsyncOpenplugin) -> None:
        async with async_client.processors.with_streaming_response.list() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

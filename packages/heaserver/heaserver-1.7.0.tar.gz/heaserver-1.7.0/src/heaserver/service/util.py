...
from collections.abc import Sequence, AsyncGenerator, Awaitable, Coroutine, Callable, Generator, Iterable
...
def raise_multiple(errors: Sequence[Exception]):
    """
    Raises multiple exceptions at once, nesting them.

    :param errors: the exceptions to raise.
    """
    if not errors:
        return
    errors_ = deque(errors)
    try:
        raise errors_.popleft()
    finally:

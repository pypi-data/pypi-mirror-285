...
class Player(QMainWindow):
    ...
    def __create_ui(self):
        ...
        self.abort_current_annotation_action = QAction(
            ...
            self,
            triggered=self.timelineWidget.abort_current_annotation,
            enabled=False,
        )
        self.abort_current_annotation_action.setShortcuts(
            [Qt.Key.Key_Escape]
        )
        edit_menu.addAction(self.abort_current_annotation_action)

        self.select_next_timeline_action = QAction(
            "Select Next TimeLine",
            self,
            triggered=self.timelineWidget.select_next_timeline,
            enabled=True,
        )
        self.select_next_timeline_action.setShortcuts(
            [Qt.Key.Key_Down]
        )
        edit_menu.addAction(self.select_next_timeline_action)

        self.select_previous_timeline_action = QAction(
            "Select Previous TimeLine",
            self,
            triggered=self.timelineWidget.select_previous_timeline,
            enabled=True,
        )
        self.select_previous_timeline_action.setShortcuts(
            [Qt.Key.Key_Up]
        )
        edit_menu.addAction(self.select_previous_timeline_action)

        self.delete_annotation_action = QAction(
            "Delete Annotation",
            self,
            triggered=self.timelineWidget.delete_annotation,
            enabled=True,
        )

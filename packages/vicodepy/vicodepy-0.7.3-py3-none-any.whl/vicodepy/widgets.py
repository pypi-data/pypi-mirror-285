...
class TimeLineWidget(QWidget):
    ...
    def select_timeline(self, line):
        line.select = True
        self.selected_timelineLine = line
        line.update()
    ...
    def handle_annotation(self):
        ...
        if not self.selected_timelineLine:
            ...
            if self.currentAnnotation is None:
                ...
                self.player.abort_current_annotation_action.setEnabled(True)
                if annotation:
                ...
                self.player.abort_current_annotation_action.setEnabled(False)
                self.update()
...
class Annotation(QGraphicsRectItem):
    ...
    def get_time_from_bounding_interval(self, time) -> int:
        if self.lower_bound and time < self.lower_bound:
            ...
            self.timelineWidget.player.mediaplayer.pause()
        return time

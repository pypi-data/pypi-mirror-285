...
class EmbeddedPanoptoExtractor(LinkExtractor):
    ...
    def _is_tag_valid(self, tag: Tag) -> bool:
        ...
        if self._is_iframe_embedded_panopto_video(src_raw):
            return True
        elif self._is_iframe_lti_resource(src_raw):
            lti_url = URL(src.params["url"])
            return self._is_panopto_lti_resource(lti_url)
        else:
            return False
    ...
    @staticmethod
    def _is_iframe_lti_resource(raw_src: str):
    ...
    @staticmethod
    def _is_panopto_lti_resource(lti_url: URL):

...
from .types import Shape, Nodes, Tuple, Child
Tag = int
...
class Block:
    def __init__(self, nodes, cells, child, points, divs):
        import shps.child
        self.nodes  = nodes
        self.cells  = cells
        outline = shps.child.IsoparametricMap(plane.Q9, nodes=points)
        ...
        def find_node(coord):
            for tag, xyz in self.nodes.items():
                if np.linalg.norm(np.array(xyz) - np.array(coord)) <=1e-8:
                    return tag

        exterior_coords = [outline.coord(x) for x in nat_exterior]
        segments = []
        for i in range(1,len(exterior_coords)):
            segments.append((find_node(exterior_coords[i-1]),

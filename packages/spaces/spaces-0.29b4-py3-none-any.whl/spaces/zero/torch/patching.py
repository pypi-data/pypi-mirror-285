...
def _move(nvidia_uuid: str):
    ...
    # TODO: Do not display in logs (maybe only display a custom summary with nParams / Speed)
    progress = tqdm(total=total, unit='B', unit_scale=True, title="ZeroGPU: spawn") if tqdm is not None else nullcontext()
...
def move(nvidia_uuid: str):
    with ThreadPoolExecutor(1) as e:
        e.submit(copy_context().run, _move, nvidia_uuid).result()

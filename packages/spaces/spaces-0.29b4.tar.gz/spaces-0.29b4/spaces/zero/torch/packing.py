...
def pack_tensors(
    ...
    for tensor in tensors:
        size = tensor.untyped_storage().size() # pyright: ignore [reportAttributeAccessIssue]
    ...
    get_meta = {tensor: tensor.to('meta') for tensor in tensors}
    ...
    # TODO: ZeroGPUTensorPack.__init__
    fd = os.open(pack.path(), os.O_CREAT | os.O_WRONLY | os.O_DIRECT)
    try:
        total_size = sum([aligned_size for batch in batches for *_, aligned_size in batch])
        total_size += sum([aligned_size for *_, aligned_size in big_tensors])
        if total_size > 0:
            os.posix_fallocate(fd, 0, total_size)
            for batch in batches:
                for tensor, *_ in batch:
                    write(fd, tensor)
            for tensor, *_ in big_tensors:
                write(fd, tensor)
        return pack
    finally:
        os.close(fd)
    ...
    with ThreadPoolExecutor(2) as e:
        ...
        try:
            for f in as_completed([
                e.submit(disk_to_pin, fd),
                e.submit(pin_to_cuda),
            ]):
                f.result()
        finally:
            os.close(fd)

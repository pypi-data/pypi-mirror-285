...
# {
    ...
    5. post_process(self, result, coeff_names, sample_size, hess_inv=None, verbose=1);
    ...
    def setup(self, X, y, varnames=None, alts=None, isvars=None, num_classes=2,
            ...
            panels=None, verbose=1, method="bfgs", scipy_optimisation=False,
            validation=False, mnl_init=True):
    # {
        print(f'The regularisation penalty is {self.reg_penalty}')
        ...
        super(LatentClassModel, self).setup(X, y, varnames, alts, isvars,
                ...
                gtol, return_grad, return_hess, verbose, method, scipy_optimisation)
    # }
    ...
    # {
        ref = self.varnames if coeff_names is None else coeff_names
        tmp_varnames = ref.copy()
        for i, varname in enumerate(tmp_varnames): # {
            if varname.startswith('lambda.'):
                tmp_varnames[i] = varname[7:] # remove lambda so can get indices correctly
        # }
        ...
        for varname in self.member_params_spec[class_num]: # {
            for i, tmp_varname in enumerate(tmp_varnames): # {
                if varname in tmp_varname and varname != '_inter':
                    X_class_idx = np.append(X_class_idx, i)
            # }
        # }
    ...
    # {
        if self.validation:
            return
        # else:
        super(LatentClassModel, self).post_process(result, coeff_names, sample_size, verbose=verbose)
    # }
    ...
    # { 
        ...
        # {
            ...
            # {
                if 'inter' in var and coeff_names is not None and 'inter' in vars:
                # {
        ...
        for idx_pos, _ in enumerate(self.varnames):
        # {

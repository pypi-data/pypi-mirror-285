...
boxc_l = ['L1', 'L2']
infinity = float("inf")
valid_criterions = {'bic', 'aic', 'mae', 'loglik'}
...
# {
   ...
   '''
    ...
    def __init__(self, criterions, df, varnames, df_test=None, distr=None, avail=None, test_avail=None,
        ...
        ftol=1e-5, gtol=1e-5, gtol_membership_func=1e-5, ftol_lccm=1e-4, ftol_lccmm=1e-4,
        ...
        intercept_opts=None, base_alt=None, val_share=0.25):
    # {
...
# {
    ...
    10. add_bcfeature(self, solution, prob);
    ...
    ''' Function.                                                  '''
    ''' ---------------------------------------------------------- '''
    def __init__(self, param:Parameters, idnum=0):
    # {
        ...
        self.min_classes = 1  # Min number of latent classes / int
        self.max_classes = 5  # Max number of latent classes / int
        self.generate_plots = False

        self.converged, self.not_converged, self.exception = 0, 0, 0
        self.idnum = idnum
    ...
    # {
        ...
        member_vars = ['_inter'] + self.param.avail_isvars
    ...
    # {
        ...
        member_params_spec_new = [np.sort(np.append(member_params, new_param)) for member_params in member_params_spec]
        solution['member_params_spec'] = member_params_spec_new
        if new_param not in solution['isvars']:
            solution['isvars'] = sorted(set(solution['isvars']).union(new_param))
    # }
    ...
    # {
        ...
        for i, member_params in enumerate(member_params_spec):
            member_params_spec[i] = np.array([var for var in member_params if var != rem_member])

        solution['member_params_spec'] = member_params_spec

        if rem_member not in np.concatenate(member_params_spec):
            solution['isvars'] = [var for var in solution['isvars'] if var != rem_member]
    # }
    ...
    def perturb_add_bcfeature(self, solution, prob):
    # {
        ...
        # {
            ...
            print("bcvars =", solution['bcvars'])

            # OLD CODE
            # This line generates a random sample of binary values
            # Note: Pr(0) = 1-prob and Pr(1) = prob
            #bools = self.param.np.random.choice([0,1], size=len(solution['asvars']), p=[1 - prob, prob])

            # Iterate over pairs of elements from 'asvars' and bools
            #new_bcvar = [asvar for asvar, selected in zip(solution['asvars'], bools) if selected]

            #solution['bcvars'] = sorted(set(solution['bcvars']).union(new_bcvar))
            #solution['bcvars'] = [var for var in solution['bcvars'] if var not in self.param.ps_corvars]
        ...
        # Ensure 'corvars' is initialized
        #solution.setdefault('corvars', [])
        # Remove corvars that are now included in bcvars
    ...
    # {
        ...
        # BOTH STATEMENTS BELOW SHOULD BE IN THE ABOVE LOOP?
        # REVIEW THIS REQUIREMEWNT WITH PRITHVI!
    ...
    # {
        ...
        class_params_spec_new = self.add_class_param(choice, solution)
        solution['class_params_spec'] = class_params_spec_new
    # }
    ...
    def perturb_bcfeature(self, sol, prob):
    # {
        if np.random.rand() <= 0.5:
            return self.perturb_add_bcfeature(sol, prob)
        else:
   ...
   # {
    ...
    # {
        ...
        # {
            ...
            # {
                ...
                #DEBUG: print("EvalSol: bcvars=", sol['bcvars'])
                if converged:
                # {
                    self.converged += 1
                    sol['converged'] = True
                    # REMOVE: sol['coeff_names'] = coeff_names
                    # REMOVE: sol['pvals'], sol['pval_member'] = pval, pval_member
    ...
    # {
        ...
        model.setup(X=X, y=y, varnames=varnames, isvars=isvars, alts=alts, transvars=transvars, ids=ids,
            randvars=randvars, panels=panels, fit_intercept=fit_intercept, correlation=corvars, n_draws=n_draws,
    ...
    # {
        ...
        model.setup(X, y, varnames=varnames, class_params_spec=class_params_spec,
                ...
                n_draws=n_draws, correlation=corvars, maxiter=maxiter, ftol=ftol, ftol_lccmm=ftol_lccmm,
    ...
    # {
        ...
        # {
            ...
            aic, bic, loglik = model.aic, model.bic, model.loglikelihood
       ...
       # except Exception as e:
        # {
          #  print("EXCEPTION TRIGGERED EVAL_MNL")
            #exit()
            #self.exception += 1
            #tuple = self.setup_empty_tuple()
        # }
       # return tuple
    # }
    ...
    # {
        ...
        # {
            ...
            rand_var_names = [i for i, j in (
                            rand_vars.items() if isinstance(rand_vars, dict) else (rand_vars if isinstance(rand_vars, list) else []))]
            cor_var_names = [i for i, j in (cor_vars.items() if isinstance(cor_vars, dict) else (cor_vars if isinstance(cor_vars, list) else []))]
            bc_vars = [i for i in self.define_bc_vars(sol) if i not in self.param.isvarnames]
            all_vars = list(set(as_vars + is_vars + rand_var_names + cor_var_names))  #new addition make sure all the names are in vars
        ...
        #except Exception as e:
        # {
          #  print("EXCEPTION TRIGGERED EVAL_MXL")
          #  exit()
          #  self.exception += 1
          #  tuple = self.setup_empty_tuple()
        # }
    ...
    # {
        ...
        # {
            ...
            model.setup(X_test, y_test, varnames=all_vars,
                            ...
                            member_params_spec=member_params_spec, num_classes=self.param.num_classes,
                            ids=self.param.test_choice_id, panels=self.param.test_ind_id, randvars=rand_vars,
                            n_draws=self.param.n_draws, correlation=corvars, transformation="boxcox",

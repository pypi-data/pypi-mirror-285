...
# {
    ...
    2. setup(self, X, y, ...);
    3. fit(self);
    4. post_process(self, optimization_res, coeff_names, sample_size, hess_inv=None, verbose=1);
    ...
    def __init__(self): # {
        ...
        print(self.reg_penalty) #TODO add in as argument, we want to be able to set this to 0
    # }
    ...
    def setup(self, X, y, varnames=None, alts=None, isvars=None, num_classes=2,
              ...
              maxiter=2000, correlation=None, n_draws=1000, halton=True,
              verbose=1, batch_size=None, halton_opts=None, ftol=1e-5, ftol_lccmm=1e-4,
    ...
    # {
        ...
        # {
            ...
            # {
                ...
                mxl = misc.setup_logit(i, mxl, X, y, varnames, self.class_params_spec, avail, alts, transvars, gtol,
                        mixed=True, panels=panels, randvars=randvars,
                        correlation=correlation, n_draws=n_draws, mnl_init=mnl_init)
        ...
        super(LatentClassMixedModel, self).setup(X, y, varnames, alts, isvars,
                ...
                self.fit_intercept, init_coeff, maxiter, correlation,
                n_draws, halton, self.expectation_maximisation_algorithm, verbose, batch_size,
    ...
    # {
        if not self.validation:
            super(LatentClassMixedModel, self).post_process(optimization_res, coeff_names, sample_size, verbose=verbose)
    # }
    ...
    # {
        ...
        # {
            if self.rvtransidx[ii] and hasattr(self, 'correlation') \
                    and self.correlation \
                        and hasattr(self.correlation,'append') and var in self.correlation:
                num_corr_rvtrans += 1
        # }
        ...
        # {
            ...
            # {
                is_correlated = hasattr(self, 'correlation') and self.correlation and (
                            hasattr(self.correlation, 'append') or var in self.correlation)
    ...
    # {
        class_idx = self.get_class_X_idx(class_num)
    ...
    # {
        ...
        self.set_bw(class_idxs[i])  # sets sd. and corr length
        self.Kchol = self.get_kchol(self.global_varnames[class_idxs[i]])
        rand_idx = [ii for ii, param in enumerate(self.randvars) if param in self.global_varnames[class_idxs[i]]]
        randtrans_idx = [ii for ii, param in enumerate(self.randtransvars) if param in self.global_varnames[class_idxs[i]]]
    ...
    def expectation_maximisation_algorithm(self, tmp_fn, tmp_betas, args,
        ...
        log_lik_old, log_lik_new = 0, 1E10

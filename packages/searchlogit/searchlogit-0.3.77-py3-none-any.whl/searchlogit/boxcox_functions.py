...
# {
    ...
    if x <= 0:
        raise ValueError("Error. x <= 0 and hence log(x) is undefined")
...
# {
    ...
    for ii, lmda in enumerate(lmdas):
        bxcx_X[:, :, :, ii] = transform(X_matrix[:, :, :, ii], lmda)
    return bxcx_X
# }
...
# {
    ...
    for ii, lmda in enumerate(lmdas):
        der_bxcx_X[:, :, ii] = transform_derivative(X_matrix[:, :, ii], lmda)
    return der_bxcx_X
# }

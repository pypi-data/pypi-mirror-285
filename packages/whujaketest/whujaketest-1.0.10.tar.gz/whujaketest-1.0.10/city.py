...
class CityscapesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        original_size = image.shape[:2]  # 保存原始大小
        ...
        return image, label, tuple(map(int, original_size))  # 将原始大小转换为整数元组
...
test_images, test_labels, original_sizes = next(iter(val_dataloader))
...
for i in range(predictions.shape[0]):
    height, width = int(original_sizes[0][i]), int(original_sizes[1][i])  # 确保转换为整数

    restored_prediction = cv2.resize(predictions[i], (width, height), interpolation=cv2.INTER_NEAREST)
    restored_predictions.append(restored_prediction)
restored_predictions = np.array(restored_predictions)
        ...
        for label in np.unique(predictions[i]):
            ...
            for contour in contours:
                ...
                if len(polygon) < 3:
                    continue  # 忽略非多边形
                objects.append({
                    "label": labels[label].name,  # 使用名称而不是ID
                    "polygon": polygon
                })
...
from labels import labels  # 确保有labels.py文件并导入

# 示例：将预测结果保存为JSON文件
test_image_paths = val_image_paths[:16]  # 获取批量中的图像路径
output_dir = 'task3/dataset/predictions'
save_predictions_as_json(test_image_paths, restored_predictions, output_dir)

# 将预测结果转换为类别标签并可视化
for i in range(16):  # 批量大小为4
    ...
    plt.imshow(restored_predictions[i], cmap='gray')
...
for i in range(16):  # 批量大小为4
    ...
    plt.subplot(1, 3, 2)
    plt.title('Ground Truth')
    ground_truth_color = label_to_color_image(test_labels[i].cpu().numpy())
    plt.imshow(ground_truth_color)

    plt.subplot(1, 3, 3)
    plt.title('Prediction')
    prediction_color = label_to_color_image(restored_predictions[i])

...
def construct_performance_df(df, seconds_gran, location_slots, time_slots, distance, height_satellite, 
    ...
    first_obs_time = first_obs_time.astype('datetime64[ns]') # Ensure 'first_obs_time' is of datetime type
    ...
    sat_new = list()
    for i in range(0,len(performance_df)):
        idx_sat = int(np.where(np.isin(sat_unique, sat_old.iloc[i]))[0][0]) 
        if sat_old[i] == 0:

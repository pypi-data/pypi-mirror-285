...
def scan(computer, domain, dns_server, port, debug, debugmax):
	...
	try:
		...
		answer = resolver.resolve(computer, "A", tcp=True)
		if len(answer) == 0:
			answer = resolver.resolve(computer, "A", tcp=False)
			if len(answer) == 0:
				screenLock.acquire()
		...
		if debug:
			screenLock.acquire()
			print ('DNS resolution for ', str(computer) , ' : ',  str(answer))
			screenLock.release()
	...
	# If it fails
		...
		if debug:
			screenLock.acquire()
			print ('DNS resolution for ', str(computer) , ' : ',  str(answer))
			print ('Scanning ', computer , 'on port',  port)

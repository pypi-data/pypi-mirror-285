...
def get_cpu_load():
    ...
    if platform.uname().system == "Darwin":
        ...
        with open('/proc/loadavg') as f:
            ...
            cpu_load_dict["1min"] = round(float(cpu_load_result[0]), 2)
            cpu_load_dict["5min"] = round(float(cpu_load_result[1]), 2)
            cpu_load_dict["15min"] = round(float(cpu_load_result[2]), 2)

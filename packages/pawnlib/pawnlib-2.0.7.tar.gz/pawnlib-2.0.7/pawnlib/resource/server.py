...
class DiskPerformanceTester:
    def __init__(self, file_path, file_size_mb, iterations=5, block_size_kb=1024, num_threads=1, io_pattern="sequential", decimal_places=2):
    ...
    def cleanup(self, file_path):
        try:
            if os.path.exists(file_path):
                os.remove(file_path)
                self.console.log(f"Deleted file {file_path}")
        except OSError as e:
    ...
    def run_parallel_tests(self):
        ...
        # self.average_write_speed = statistics.mean(self.write_speeds) if self.write_speeds else 0
        # self.average_read_speed = statistics.mean(self.read_speeds) if self.read_speeds else 0
        #

...
def connection_refused():
	...
	if mem.var['stable']:
		...
		time.sleep(30)
		return 'retry'
	print('%s Target has rate limiting in place, please use --stable switch')
	return 'kill'
...
def error_handler(response, factors):
	...
	if type(response) != str and response.status_code in (400, 413, 418, 429, 503):
		if response.status_code == 503:
			mem.var['kill'] = True
			print('%s Target is unable to process requests, try --stable switch')
			return 'kill'
		elif response.status_code in (429, 418):
			print('%s Target has a rate limit in place, try --stable switch')
			return 'kill'
		else:
			if factors['same_code'] != response.status_code:
				...
				if mem.var['bad_req_count'] > 20:
					mem.var['kill'] = True
					print('%s Server received a bad request. Try decreasing the chunk size with -c option')
					return 'kill'
			else:
				return 'ok'
	else:
		if 'Timeout' in response:
			if mem.var['timeout'] > 20:
				...
				print('%s Target might have a rate limit in place, try --stable switch')
				return 'kill'
			else:
				print('%s Connection timed out, increased timeout by 5 seconds')
				mem.var['timeout'] += 5
				return 'retry'
			...
			if '\'' in response:
				print('%s Encountered an error: %s' % (response.split('\'')[1]))
			return 'kill'
	return 'ok'
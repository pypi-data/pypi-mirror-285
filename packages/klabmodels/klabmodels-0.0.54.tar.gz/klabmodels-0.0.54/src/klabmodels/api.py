   ...
   if evaluation:
      try:
         cv_evaluation = CVEvaluation(candidate=candidate_pk, 
                   ...
                   company_fit_grade=evaluation.get('company_fit_grade'),
                   vacancy_fit_grade=evaluation.get('vacancy_fit_grade'),
      ...
      logging.error(f"No evaluation to save for candidate {candidate_pk}")
...
def read_cv_evaluation_by_candidate(candidate_pk: str):
   try:
      cv_evaluation = CVEvaluation.find(CVEvaluation.candidate==candidate_pk).first()
      logging.info(f"CV Evaluation {cv_evaluation.pk} found")
      return cv_evaluation
   except Exception:

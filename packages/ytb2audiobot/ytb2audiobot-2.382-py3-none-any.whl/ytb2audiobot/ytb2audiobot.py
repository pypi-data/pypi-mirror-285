...
from ytb2audiobot.utils import read_file
...
timerlogger_handler = logging.FileHandler(config.TIMERS_FILE_PATH.as_posix(), mode='a', encoding='utf-8')
timerlogger_handler.setFormatter(logging.Formatter("%(asctime)s : %(message)s"))
...
async def processing_download(command_context: dict):
    ...
    if dev_mode:
        ...
        timerlogger.info(f'{predict_time} : {stopwatch_time} : {int(stopwatch_time - predict_time)}')
...
async def timers_show_handler(message: Message) -> None:
    if not config.TIMERS_FILE_PATH.exists():
        await message.answer(text='No timers file')

    with config.TIMERS_FILE_PATH.open(mode='r') as file:
        text = file.read()

    if len(text) > config.MAX_TELEGRAM_BOT_TEXT_SIZE:
        rows = text.split('\n')
        rows = rows[::-1]
        text = '\n'.join(rows)
        text = text[:config.MAX_TELEGRAM_BOT_TEXT_SIZE - 8]
        rows = text.split('\n')
        rows = rows[::-1]
        text = '\n'.join(rows)
        text = f'...\n{text}'

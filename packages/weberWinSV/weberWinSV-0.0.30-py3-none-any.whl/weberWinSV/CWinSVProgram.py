...
class CWinSVProgram:
    ...
    def CheckAndRunOneCmd(self, iLoopCnt):
        ...
        if self.CheckStopFileExists():
            ...
            if iLoopCnt > 0 and self.CheckPidFileExists():
                # WeiYF.20170928 非首次启动才检查pid文件
                pid = self.ReadPidFromFile()
                if psutil.pid_exists(pid):
                    self.TouchPidFile()
                    if self.bVerbose:
            ...
            self.fdOut = open(self.sLogOutFile, 'a+', encoding='utf8')
            self.fdOut.write("\n[%s]%s{%s}...\n" % (GetCurrentTime(), '*'*40, self.sProgramId))
            self.fdOut.flush()
            if self.bErr2Out:
                self.fdErr = self.fdOut
            else:
                self.fdErr = open(self.sLogErrFile, 'a+', encoding='utf8')

...
def outage_query(sample_size, time_window):
    ...
    power_outage_io = Outage_event.wea.outagedf

    content = power_outage_io.read().decode('utf-8')
    lines = content.splitlines()
    data = [line.split(',') for line in lines[:]]
    power_outage_data = pd.DataFrame(data,
                                     columns=["Start", "End", "Region", "Reason", "Area", "Loss", "People", "Year",
                                              "Duration", "StartHour", "StartMonth", "Duration"])

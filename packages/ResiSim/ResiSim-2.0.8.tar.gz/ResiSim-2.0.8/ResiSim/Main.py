...
def weather_query(city, time_span, sce):
    ...
    if sce != 'All':
        ...
        return _epw, _df, plotter.wea.heat_wave_events, plotter.wea.Spic, plotter.wea.Sdeb, plotter.wea.Sint
    else:
        _epw = plotter.wea.raw_io
        _df = plotter.wea.data_dict['{}_df'.format(sce)]
        return _epw.getvalue(), _df, plotter.wea.heat_wave_events, plotter.wea.Spic, plotter.wea.Sdeb, plotter.wea.Sint
...
def outage_mcmc(outagedf, sample_size, time_window):
    power_outage_data = outagedf

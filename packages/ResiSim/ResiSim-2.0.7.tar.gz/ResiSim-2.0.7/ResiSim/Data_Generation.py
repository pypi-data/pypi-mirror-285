...
class weather():
    ...
    def get_outage_data(self):
        file_id = search_file_by_name(self.service, 'PowerOutage.csv', "114XBgd8BSwIWH1EspVZmj6eGVLz7YJtK")
        url = f'https://drive.google.com/uc?export=download&id={file_id}'
        response = requests.get(url)
        with open('power_outage.csv', 'wb') as f:
            f.write(response.content)
        self.outagedf = pd.read_csv('power_outage.csv')

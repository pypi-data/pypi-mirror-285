...
class _MaterPreProcess:
    ...
    def _to_mater(self, variable: str, k=None):
        ...
        **Parameters:**
        ...
        os.makedirs(
            os.path.join(self.dir_path, self.scenario_name, variable),
            exist_ok=True,
        )
    ...
    def get(self, variable: str, t: int = None, scenarios: list = None):
        ...
        if t == None:
            return pd.read_parquet(os.path.join(self.dir_path, self.scenario_name, variable))["value"].unstack("time")
        else:
    ...
    def _inputs_setup(self, input_file: str):
        ...
        for input in input_dict:
            ...
            if input == "vectors":
                ...
                os.makedirs(
                    os.path.join(self.dir_path, self.scenario_name, input),
                    exist_ok=True,
                )

                # save the inputs into parquet format
                df2.stack(dropna=False).to_frame("value").to_parquet(
                    os.path.join(self.dir_path, self.scenario_name, input, input + ".mater"),
                    engine="pyarrow",
                    compression="snappy",
                )

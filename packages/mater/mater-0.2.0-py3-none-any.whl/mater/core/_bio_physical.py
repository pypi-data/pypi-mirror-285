...
from mater.core._utils import _log_normal
from mater.core._pre_processing import _MaterPreProcess
...
class _MaterProcessBioPhysical(_MaterPreProcess):
    ...
    def _self_disposal_flow_computation(self, k: int):
        ...
        .. math::
            A^d_{l,o,t}(k-1) = L_{l,o,t} \odot A^c_{l,o}(k-1)
            :label: self_disposal_flow_calc

        Where:

        - :math:`A^d_{l,o,t}(k-1)` represents the `self_disposal_flow` for `production_date` :math:`k-1`.
        - :math:`L_{l,o,t}` is the log-normal distribution output :eq:`log_normal_calc`, reflecting variability in cohort lifetimes.
        - :math:`A^c_{l,o}(k-1)` is the `control_flow` output :eq:`control_flow_calc` at `time` :math:`k-1`.
        ...
        self_disposal_flow_distribution = pd.concat(
            ...
            keys=[k - 1],
            names=["production_date"],
        ).reorder_levels(["location", "object", "production_date"])
    ...
    def _traded_control_flow_computation(self, k: int):
        ...
        .. math::
            A^{c^T}_{l_p,o}(k) = A^c_{l,o}(k) \, T^c_{l,o,l_p}(k) \ (= \sum_{l}A^c_{l,o}(k) \, T^c_{l,o,l_p}(k) \ \text{a matrix product})
            :label: traded_control_flow_computation

        Where:

        - :math:`A^{c^T}_{l_p,o}(k)` is the `control_flow_trade` output at `time` :math:`k`.
        - :math:`A^c_{l,o}(k)` represents the `control_flow` output :eq:`control_flow_calc` at `time` :math:`k`.
        ...
        traded_control_flow = (
            ...
            .dropna(axis=0, how="all")
            .groupby(level=["production_location", "object"])
            .sum()
            .rename_axis(index={"production_location": "location"})
        )
        ...
        self.outputs["traded_control_flow"].loc[traded_control_flow.index] = traded_control_flow.copy()
    ...
    def _endogenously_controlled_process_computation(self, k: int):
        ...
        .. math::
            P^{endo^{ref}}_{l,p}(k) = \sum_{e}[A^{c^T}_{l,o}(k) \, s^c_{l,o,p}(k) + \sum_{d}[A^r_{l,o,d}(k)] \, s^r_{l,o,p}(k)]
        ...
        - :math:`A^{c^T}_{l,o}(k)` is the `traded_control_flow` output :eq:`traded_control_flow_computation` at `time` :math:`k`.
        - :math:`s^c_{l,o,p}(k)` is the :ref:`control_flow_shares <input>` input at `time` :math:`k`.
        - :math:`A^r_{l,o,d}(k)` represents the `recycling_flow` output :eq:`recycling_flow_calc` at `time` :math:`k`.
        ...
        self.endogenously_controlled_reference_process = (
            (
                (self.outputs["traded_control_flow"].replace(0, np.nan).mul(self.inputs["control_flow_shares"][k]))
                .add(
    ...
    def _controlled_process_computation(self, k: int):
        ...
        self.controlled_process = (
            (self.endogenously_controlled_process.mul(self.inputs["process_shares"][k]).dropna(axis=0, how="all"))
            .reorder_levels(["location", "object", "process"])
            .add(self.exogenously_controlled_process[k], fill_value=0)
        )
    ...
    def _free_process_computation(self, k: int):
        ...
        .. math::
            P^f_{l,o,p}(k) = \sum_{d}[S^u_{l,o,d}(k-1)] \odot u^{\text{ref}}_{l,o,p}(k-1)
            :label: free_process_computation

        Where:

        - :math:`P^f_{l,o,p}(k)` represents the `free_process` output for "free" `objects` at `time` :math:`k`.
        - :math:`S^u_{l,o,d}(k-1)` represents the `in_use_stock` output :eq:`in_use_stock_computation` for "free" `objects` at `time` :math:`k-1`.
        ...
        self.free_process = (
            ...
            .loc[:, self.vector["free_object"], :]
            .mul(self.outputs["reference_intensity_of_use"][k - 1])
            .groupby(level=["location", "object", "process"])
            .sum()
        )
    ...
    def _process_computation(self, k: int):
        ...
        .. math::
            P_{l,p}(k) = \sum_{e}P^{\text{des}}_{l,o,p}(k)
        ...
        - indices are the :ref:`variables dimensions <dimensions>`.
        """
        self.outputs["process"] = self.controlled_process.add(self.free_process, fill_value=0)
    ...
    def _control_flow_composition_computation(self, k: int):
        ...
        .. math::
            A^{c^{comp}}_{l_p,p,o_c}(k) = \sum_{e}[\sum_{l}[s^c_{l_p,o,p}(k) \odot T^{c^{-1}}_{l_p,o,l}(k) \odot E^{comp}_{l,o,o_c}(k) \odot P_{l_p,p}(k)]]
            :label: control_flow_composition

        Where:

        - :math:`A^{c^{comp}}_{l_p,p,o_c}(k)` is the `control_flow_composition` output at `time` :math:`k`.
        ...
        - indices are the :ref:`variables dimensions <dimensions>`.
        """
        # flows du to control object composition
        traded_control_flow_composition = (
            self.inputs["control_flow_shares"][k]
            .rename_axis(index={"location": "production_location"})
            .mul(self.control_flow_trade_inverse_matrix[k])
            .mul(self.inputs["object_composition"][k])
            .mul(self.process.rename_axis(index={"location": "production_location"}))
            .dropna(axis=0, how="all")
            .groupby(level=["production_location", "process", "object_composition"])
            .sum()
            .rename_axis(
                index={
        ...
        control_flow_composition_without_trade = (
            (
                self.inputs["control_flow_shares"][k]
                .mul(self.inputs["object_composition"][k])
                .mul(self.process)  # fill_value=1
            )
            .dropna(axis=0, how="all")
            .groupby(level=["location", "process", "object_composition"])
            .sum()
            .rename_axis(index={"object_composition": "object"})
        )  # only needed if the the control flow trade matrix is not complete.
        self.control_flow_composition = control_flow_composition_without_trade.mul(
            traded_control_flow_composition, fill_value=1
        ).div(
            traded_control_flow_composition, fill_value=1
        )  # same
        self.control_flow_composition.loc[traded_control_flow_composition.index] = (
            traded_control_flow_composition.copy()
        )  # same
    ...
    def _secondary_production_flow_computation(self, k: int):
        ...
        .. math::
            R^{comp}_{l,o,o_c}(k) = \frac{\sum_{d}[A^r_{l,o,d}(k) \odot E^{comp}_{l,o,o_c,d}]}{\sum_{d}[A^r_{l,o,d}(k)]}
        ...
        - :math:`A^r_{l,o,d}(k)` represents the `recycling_flow` output :eq:`recycling_flow_calc` at `time` :math:`k`.
        - :math:`E^{comp}_{l,o,o_c,d}` is the :ref:`object_composition <input>` input with the `time` dimension :math:`t` renamed in the `production_date` dimension :math:`d`.
        - :math:`\odot` the `Hadamard product <https://en.wikipedia.org/wiki/Hadamard_product_(matrices)#:~:text=In%20mathematics%2C%20the%20Hadamard%20product,of%20the%20multiplied%20corresponding%20objects.>`_ of two matrices or vectors. In the case where the dimensions of both matrices or vectors are different, the missing dimensions are added with replicate values before performing the product.
        - indices are the :ref:`variables dimensions <dimensions>`.
        ...
        .. math::
            A^{r^{comp}}_{l,p,o_d}(k) = \sum_{e_c}[\sum_{e}[s^r_{l,o,p}(k) \odot R^{comp}_{l,o,o_c}(k) \odot P_{l,p}(k)] \odot r_{l,o_c,o_d}(k)]
            :label: secondary_production

        Where:

        - :math:`A^{r^{comp}}_{l,p,o_d}(k)` is the `secondary_production` output at `time` :math:`k`.
        - :math:`s^r_{l,o,p}(k)` is the :ref:`recycling_flow_shares <input>` input at `time` :math:`k`.
        ...
        recycling_composition = (
            (self.outputs["recycling_flow"].unstack("production_date").rename_axis(columns={"production_date": "time"}))
        ...
        self.outputs["secondary_production"] = (
            (
                (self.inputs["recycling_flow_shares"][k].mul(recycling_composition).mul(self.process))
                .groupby(level=["location", "process", "object_composition"])
                .sum()
                .rename_axis(index={"object_composition": "object"})
            )
            .mul(self.inputs["recycling_rate"][k])
            .groupby(level=["location", "process", "object_downgrading"])
            .sum()
    ...
    def _thermodynamic_process_flow_computation(self, k: int):
        ...
        .. math::
            A^{thP}_{l,o_s,o}(k) = \sum_{p}[[P^{th}_{l,p,o}(k) \odot P_{l,p}(k) - A^{c^{comp}}_{l,p,o}(k) + A^{r^{comp}}_{l,p,o}(k)] \odot s^p_{l,o_s,p}(k)]
            :label: thermodynamic_process_flow_computation

        Where:

        - :math:`A^{thP}_{l,o_s,o}(k)` is the `thermodynamic_process_flow` output at `time` :math:`k`.
        - :math:`P^{th}_{l,p,o}(k)` is the :ref:`thermodynamic_process <input>` input at `time` :math:`k`.
        - :math:`P_{l,p}(k)` is the `process` output :eq:`total_process_computation` at `time` :math:`k`.
        - :math:`A^{c^{comp}}_{l_p,p,o_c}(k)` is the `control_flow_composition` output :eq:`control_flow_composition` at `time` :math:`k`.
        - :math:`A^{r^{comp}}_{l,p,o}(k)` is the `secondary_production` output :eq:`secondary_production` at `time` :math:`k`.
        ...
        self.thermodynamic_process_flow = (
            (
                process_flow.sub(self.control_flow_composition, fill_value=0).add(
                    self.outputs["secondary_production"], fill_value=0
                )
            )
            .mul(self.inputs["process_shares"][k].rename_axis(index={"object": "object_Su"}))
            .groupby(level=["location", "object_Su", "object"])
            .sum()
        )
    ...
    def _extraneous_flow_computation(self, k: int):
        ...
        .. math::
            A^e_{l,o_s,o_e}(k) = A^{thP}_{l,o_s,o_e}(k) + \sum_{e}[E^{eff}_{l,o_s,o,o_e}(k) \odot A^{thP}_{l,o_s,o}(k)]
            :label: extraneous_flow_computation

        Where:

        - :math:`A^e_{l,o_s,o_e}(k)` is the `extraneous_flow` output at `time` :math:`k`.
        - :math:`E^{eff}_{l,o_s,o,o_e}(k)` represents the :ref:`object efficiency <input>` input at `time` :math:`k`.
        - :math:`A^{thP}_{l,o_s,o_e}(k)` is the `thermodynamic_process_flow` output :eq:`thermodynamic_process_flow_computation` at `time` :math:`k` with the `object` dimension :math:`e` replaced by the `object_efficincy` dimension :math:`o_e`.
        - :math:`A^{thP}_{l,o_s,o}(k)` is the `thermodynamic_process_flow` output :eq:`thermodynamic_process_flow_computation` at `time` :math:`k`.
        - :math:`\odot` the `Hadamard product <https://en.wikipedia.org/wiki/Hadamard_product_(matrices)#:~:text=In%20mathematics%2C%20the%20Hadamard%20product,of%20the%20multiplied%20corresponding%20objects.>`_ of two matrices or vectors. In the case where the dimensions of both matrices or vectors are different, the missing dimensions are added with replicate values before performing the product.
        - indices are the :ref:`variables dimensions <dimensions>`.
        """
        # extraneous flow
    ...
    def _old_stock_computation(self, k: int):
        ...
        .. math::
            S^o_{l,o,d}(k) = S^o_{l,o,d}(k-1) + A^d_{l,o,d}(k) - A^r_{l,o,d}(k)
            :label: old_stock_computation

        Where:

        - :math:`S^o_{l,o,d}(k-1)` is the `old_stock` output from the previous time step.
        - :math:`A^d_{l,o,d}(k)` is the `self_disposal_flow` output :eq:`self_disposal_flow_calc` at `time` :math:`k`.
        - :math:`A^r_{l,o,d}(k)` is the `recycling_flow` output :eq:`recycling_flow_calc` at `time` :math:`k`.
        - indices are the :ref:`variables dimensions <dimensions>`.
        """
    ...
    def _in_use_stock_computation(self, k: int):
        ...
        .. math::
            S^u_{l,o,d}(k) = S^u_{l,o,d}(k-1) - A^d_{l,o,d}(k) + \sum_{e_s}[A^e_{l,o_s,o}(k,k)] + A^c_{l,o}(k,k)
        ...
        - :math:`A^d_{l,o,d}(k)` is the `self_disposal_flow` output :eq:`self_disposal_flow_calc` at `time` :math:`k`.
        - :math:`A^e_{l,o_s,o}(k,k)` is the `extraneous_flow` output :eq:`extraneous_flow_computation` at `time` :math:`k` and `production_date` :math:`k` with the `object_efficiency` dimension :math:`o_e` replaced by the `object` dimension :math:`e`.
        - :math:`A^c_{l,o}(k,k)` represents the `control_flow` output :eq:`control_flow_calc` at `time` :math:`k` and `production_date` :math:`k`.
        - indices are the :ref:`variables dimensions <dimensions>`.
        """
        ...
        control_flow = pd.concat({k: self.outputs["control_flow"]}, names=["production_date"]).reorder_levels(
            ["location", "object", "production_date"]
        )
        # in-use stock

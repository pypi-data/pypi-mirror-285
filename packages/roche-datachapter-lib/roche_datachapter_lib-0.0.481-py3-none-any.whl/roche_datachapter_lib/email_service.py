...
class EmailDestination:
    ...
    def __init__(self, to: list[str], cc: list[str] = None, bcc: list[str] = None):
...
class AttachmentFile:
    ...
    def __init__(self, file_path: str, file_type: AttachmentFileType = None):
        if path.exists(file_path):
            ...
            self.file_name: str = path.basename(file_path)
        else:
            raise EmailServiceError(
                f"AttachmentFile '{file_path}' does not exist")
        if file_type is None:
            self._infer_file_type()
        if not isinstance(file_type, AttachmentFileType):
            raise EmailServiceError(
...
class EmailService():
    ...
    def send_email(self, destination: EmailDestination, subject: str, body: str, attachments: list[AttachmentFile] = None):
        """Crea un registro de correo electrónico para ser enviado por Appsheet"""
        self._validate_email_destination(destination)
        appsheet_post_data = [{
            "destinatario/s": destination.get_to() if destination.get_to() else '',
            "asunto": str(subject),
            "cuerpo": str(body),
            "cc": destination.get_cc() if destination.get_cc() else '',
            "bcc": destination.get_bcc() if destination.get_bcc() else ''
        }]
        if attachments is not None:
            ...
            for attachment in attachments:
                ...
                remove(attachment.get_file_path())

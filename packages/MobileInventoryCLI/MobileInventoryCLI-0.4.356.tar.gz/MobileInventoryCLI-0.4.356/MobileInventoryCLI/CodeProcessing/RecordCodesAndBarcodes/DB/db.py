...
class DefaultListEditor:
    def __init__(self):
        self.helpTextMenu=f'''
Default List Editor
#Note Search will search the SystemPreference.pid as well as the data contained within SystemPreference.value_4_Json2DictString, which is a JsonDict("DefaultTags"=[tags1,tags,2,tags3,etc])
andt,add new default list - a comma separated list of text to add to SystemPreferences.name=="DefaultLists"
sssdl,search select set default list - search for default lists in system preferences, select it by number, set to true, and all others to false, for use by udt
sdl,search default list - search for default list by text or pid
ssedl,search select edit default list - search for default list to edit/add/remove
reset_all_default_lists,reset all default lists - clear all default lists from SystemPreferences 
list all default lists,ls,list default lists,lsadl - list all default selections
        '''
        while True:
            print(self.helpTextMenu)
            break
...
from chik._tests.environments.wallet import WalletTestFramework
from chik._tests.wallet.conftest import *  # noqa
from chik.cmds.cmd_classes import ChikCommand, Context, NeedsWalletRPC, chik_command, option
from chik.types.blockchain_format.sized_bytes import bytes32


def check_click_parsing(cmd: ChikCommand, *args: str) -> None:
    @click.group()
    def _cmd() -> None:
        pass
...
def test_cmd_bases() -> None:
    ...
    @chik_command(cmd, "temp_cmd", "blah")
    class TempCMD:
        def run(self) -> None:
            print("syncronous")

    @chik_command(cmd, "temp_cmd_async", "blah")
    class TempCMDAsync:
...
def test_option_loading() -> None:
    ...
    @chik_command(cmd, "temp_cmd_2", "blah")
    class TempCMD2:
...
def test_context_requirement() -> None:
    ...
    with pytest.raises(ValueError, match="context"):
        ...
        @chik_command(cmd, "shouldnt_work", "blah")
        class BadCMD:
            context: int
...
def test_typing() -> None:
    ...
    # Test optional
    @chik_command(cmd, "temp_cmd_optional", "blah")
    ...
    with pytest.raises(TypeError):
        ...
        @chik_command(cmd, "temp_cmd_optional_bad", "blah")
    ...
    @chik_command(cmd, "temp_cmd_optional_fine", "blah")
    ...
    # Test multiple
    @chik_command(cmd, "temp_cmd_sequence", "blah")
    ...
    with pytest.raises(TypeError):
        ...
        @chik_command(cmd, "temp_cmd_sequence_bad", "blah")
    ...
    with pytest.raises(TypeError):
        ...
        @chik_command(cmd, "temp_cmd_bad_type", "blah")
    ...
    with pytest.raises(TypeError):
        ...
        @chik_command(cmd, "temp_cmd_bad_default", "blah")
    ...
    # Test bytes parsing
    @chik_command(cmd, "temp_cmd_bad_bytes", "blah")
    class TempCMDBadBytes:
        blob: bytes = option("--blob", required=True)

        def run(self) -> None: ...

    @chik_command(cmd, "temp_cmd_bad_bytes32", "blah")

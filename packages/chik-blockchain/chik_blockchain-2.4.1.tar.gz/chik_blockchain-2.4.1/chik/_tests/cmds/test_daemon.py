...
from chik.cmds.start_funcs import create_start_daemon_connection
...
async def test_daemon(
    ...
    mocker.patch("chik.cmds.start_funcs.connect_to_daemon_and_validate", side_effect=connect_to_daemon_and_validate)
    mocker.patch("chik.cmds.start_funcs.Keychain", new=DummyKeychain)
    mocker.patch("chik.cmds.start_funcs.get_current_passphrase", side_effect=get_current_passphrase)
...
def test_start_daemon(tmp_path: Path, empty_keyring: Any, mocker: MockerFixture) -> None:
    ...
    mocker.patch(
        "chik.cmds.start_funcs.create_start_daemon_connection", side_effect=create_start_daemon_connection_dummy
    )

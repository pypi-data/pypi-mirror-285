    ...
    with desktop_app():
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                raise Exception(f"Error authenticating with desktop app: {response.text}")
        except Exception as err:
            pm.warning(f"Error authenticating with desktop app: {err}. The job will still be sent, but you'll need to manually authenticate the app.")

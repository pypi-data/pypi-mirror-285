...
def pumping_power(pres_entry, pres_exit, t):
    """
    Pump is operating with an efficiency of 75%
    """
    return ((pres_exit - pres_entry) * get_avg_sp_vol_l(t))/0.75
...
def turbine_power_highly_inefficient(t5):
    """
    ...
    turb_exit_actual_enthalpy = get_enthalpy_t(t5, q=0.9759)
    turb_exit_actual_entropy = get_entropy_t(t5, q=0.9759)
    turb_exit_temp_prop = get_properties_t(t5)

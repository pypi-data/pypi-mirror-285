...
def main():
    ...
    with dpg.window(
            ...
            no_close = True,
            no_resize = True,
            min_size = (WIDTHCONST, (image.height + 40) // 2 - 20),
            pos = (0, 0)
        ):
        dpg.add_slider_float(label="X Pos.", tag = "X", default_value=image.width // 4, max_value=image.width // 2, callback = update_rect_x)
        dpg.add_slider_float(label="Y Pos.", tag = "Y", default_value=image.height // 4, max_value=image.height // 2, callback = update_rect_y)
    ...
    with dpg.window(
            ...
            no_resize = True,
            min_size = (WIDTHCONST, (image.height + 40) // 2 - 20),
            pos = (0, (image.height + 40) // 2 - 20)
        ):
        dpg.add_slider_float(label="Width", tag = "W", default_value=image.width // 2, max_value=0.75 * image.width, callback = update_rect_w)
        dpg.add_slider_float(label="Height", tag = "H", default_value=image.height // 2, max_value=0.75 * image.height, callback = update_rect_h)
    ...
    dpg.create_viewport(title = "TIFF Cropper -- Hudson Liu", width = image.width + WIDTHCONST + 15, height = image.height + 30)
    dpg.setup_dearpygui()
    dpg.show_viewport()
    dpg.start_dearpygui()
    dpg.destroy_context()
    ...
    for f in glob.glob("*.tif"):
        ...
        if not im.shape[:2] == (image.height, image.width):

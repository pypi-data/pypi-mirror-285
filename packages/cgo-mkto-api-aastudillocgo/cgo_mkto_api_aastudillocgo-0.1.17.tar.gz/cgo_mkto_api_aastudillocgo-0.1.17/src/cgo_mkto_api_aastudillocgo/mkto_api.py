...
def get_deleted_leads(config, since_datetime):
  ...
  types_url = f"{endpointurl}/v1/activities/types.json?access_token={access_token}"

  print(types_url)

  paging_token_url = f"{endpointurl}/v1/activities/deletedleads.json?nextPageToken={next_page_token}&batchSize=300&access_token={access_token}"
  ...
  try:
    ...
    paging_token_url = f"{endpointurl}/v1/activities/deletedleads.json?nextPageToken={json_result['nextPageToken']}&batchSize=300&access_token={access_token}"

    print(paging_token_url)

    esult = requests.get(paging_token_url)

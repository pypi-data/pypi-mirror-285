...
from typing import Any, Dict, Literal, NamedTuple, Optional, Tuple, TypedDict
...
LoggerConfig = TypedDict(
    "LoggerConfig",
    {
        "agent.logger.level": str,
        "agent.logger.path": str,
        "agent.logger.stdout": bool,
        "agent.logger.stderr": bool,
        "agent.logger.progname": str,
    },
)


def configure_agent_logger(config: LoggerConfig):
    """
...
SecurityLoggerConfig = TypedDict(
    "SecurityLoggerConfig",
    {
        "protect.enable": bool,
        "agent.security_logger.path": str,
        "agent.security_logger.level": str,
    },
)


def setup_security_logger(config: SecurityLoggerConfig):
...
Severity = Literal["ALERT", "CRITICAL", "ERROR", "WARNING", "NOTICE", "INFO", "DEBUG"]

SyslogConfig = TypedDict(
    "SyslogConfig",
    {
        "protect.enable": bool,
        "agent.security_logger.syslog.enable": bool,
        "agent.security_logger.syslog.protocol": Literal["UDP", "TCP", "TCP_TLS"],
        "agent.security_logger.syslog.server_host": str,
        "agent.security_logger.syslog.ip": str,
        "agent.security_logger.syslog.port": int,
        "agent.security_logger.syslog.facility": int,
        "agent.security_logger.syslog.severity_exploited": Severity,
        "agent.security_logger.syslog.severity_blocked": Severity,
        "agent.security_logger.syslog.severity_blocked_perimeter": Severity,
        "agent.security_logger.syslog.severity_probed": Severity,
        "agent.security_logger.syslog.severity_suspicious": Severity,
    },
)


def configure_syslog_logger(config: SyslogConfig) -> bool:
    """
...
def _syslog_handler_config(config: SyslogConfig) -> Dict[str, Any]:
...
def _logger_path(config):
    if not config:
        return None

    if config.get(CONFIG_KEY_STDOUT):
        path = sys.stdout
    elif config.get(CONFIG_KEY_STDERR):
        path = sys.stderr
    else:
        path = config.get(CONFIG_KEY_LOGGER_PATH)
    return path

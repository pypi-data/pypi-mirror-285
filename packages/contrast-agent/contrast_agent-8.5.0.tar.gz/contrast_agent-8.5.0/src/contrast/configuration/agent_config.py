...
ConfigWatcher = Callable[["AgentConfig"], Any]
...
class Watcher:
    ...
    def __call__(self, config: AgentConfig):
        ...
        self.f(config_spy)
        if not config_spy._self_used_keys:
            logger.debug(
                "WARNING: Watcher does not use any config values. It will not be called when the config changes.",
                f=self.f,
            )

        for key in config_spy._self_used_keys:
            config.watch_map[key].add(self)


class ConfigSpy(dict):
    def __init__(self, wrapped: AgentConfig):
        super().__init__(wrapped.dict())
        self._self_used_keys = set()

    def get(self, key: str) -> Optional[ConfigOption]:
        self._self_used_keys.add(key)

...
from contrast.agent.inventory.cloud_providers import report_resource_id
...
def _log_environment(settings: Settings):
    ...
    log_level = settings.config.get("agent.logger.level").value().upper()
...
def initialize():
    ...
    with module.init_lock:
        ...
        setup_security_logger(module.settings.config.dict())
        ...
        if module.settings.config.get_value("server.discover_cloud_resource"):
            report_resource_id(module.reporting_client)

...
class AppBuildCommand(CliCommand):
    ...
    def _mender_package_app(self, args, meta, image_file):
        ...
        if 'simulation' in target_devices:
            out_file = os.path.join(args.project_dir, 'build', slug +  '-' + meta['app-version'] + '-simulation.auterionos')
        else:
            out_file = os.path.join(args.project_dir, 'build', slug  + '-' + meta['app-version'] + '.auterionos')

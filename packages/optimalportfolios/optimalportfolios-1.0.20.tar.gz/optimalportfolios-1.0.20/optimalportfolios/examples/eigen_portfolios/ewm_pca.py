...
class EWMPCA(object):
    def __init__(self, alpha, W_initial=None, tol=1e-12, max_iter_count=None):
        self.ewmcov = EWMCov(alpha)

        # sorted_indices are being chosed using the initial
        if W_initial is not None:
            eigen_values, eigen_vectors = np.linalg.eigh(W_initial)
            self.sorted_indices = np.argsort(eigen_values)[::-1]
        else:
    ...
    def add(self, x):
        x = np.reshape(x, (-1, 1))
        self.ewmcov.add(x)
        output = ogita_aishima(
                A=self.ewmcov.cov,
                X_hat=self.W,
                tol=self.__tol,
                max_iter_count=self.__max_iter_count,
                sort_by_eigenvalues=False,
            return_extra=True  # me
            )

        W = output['result']
        eigen_value = output['lmbda']
        if self.sorted_indices is not None:
            eigen_value = eigen_value[self.sorted_indices]
            W = W[:, self.sorted_indices]

        self.W = output['result']
        self.eigen_value = output['lmbda']
        return eigen_value, W
    ...
    def add_all(self, xs, verbose=True):
        if self.W is None:
            ...
            self.W = sorted_eig(sample_cov)[1]
        zs = []

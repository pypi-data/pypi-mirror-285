...
class ApplianceManager(ABC, ClassLogger):
    ...
    def __init__(
        ...
        # when hosting inference, we'll set this to True to slightly tweak the
        # init and load phase of execution coordinator.
    ...
    mgmt_mount_dirs = property(lambda self: self.cluster_config.mount_dirs)
    mgmt_python_paths = property(lambda self: self.cluster_config.python_paths)
    ...
    def initialize_servers(self):
        ...
        with self.tracker_execute.entry("execute_init_request"):
            self.grpc_client.init_servers(
                InitRequest(inference=self._inference_mode)
            )

    def initialize_base_session(
        self, compile_response: CompileResponse, drop_cmd_state: bool
    ):
        """Perform initial connection handshake for all execute modes.
        ...
        with self.tracker_execute.entry("execute_load_request"):
            load_request = LoadRequest(
                ...
                inference=self._inference_mode,
    ...
    def initialize_session(
        ...
        """
        self.initialize_base_session(compile_response, drop_cmd_state)
    ...
    def initialize_inference(
        self,
        run_request: RunInferenceRequest,
        compile_response: CompileResponse,
        drop_cmd_state: bool,
    ):
        """Perform initial connection handshake for inference mode.
        ...
        with self.tracker_execute.entry("execute_run_info"):
            self.grpc_client.run_inference(run_request)
    ...
    def build_worker_image(self, should_skip=False, timeout: Real = 60 * 60):
        ...
        with ClusterManagementClient(**self._mgmt_client_args) as mgmt_client:
            ...
            # base image, pip configs and package dependencies, we will reuse the same job.
    ...
    def send_weights_from_checkpoints(
        ...
        initial_weights = self._weight_iterator(checkpoint_files)
        ...
        with short_temp_dir(), limit_mp_threads(), pool_cls(
            ...
            try:
                for pkl_val in tqdm(
                    ...
                    file=sys.stdout,
                    disable=(
                        True if send_all else None
                    ),  # Disable on non-TTY/all
                ):

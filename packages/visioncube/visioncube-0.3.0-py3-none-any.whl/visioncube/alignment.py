...
class AbstractMultiAlignment(object):
    """Abstract alignment
    """

    def __call__(self, image: np.ndarray) -> List[AlignmentOutput]:
        raise NotImplementedError()
    

class TemplateMatching(AbstractMultiAlignment):
    """Template multi matching class
    """
    ...
    def __init__(
        ...
        self.template = self._pre_process(template)
    ...
    def _thred(self, res, image):
        output_list = []
        mask = np.where(res>self.thred, res, 0)
        h, w = self.template.shape[:2]
        while np.any(mask):
            _, max_value, _, max_loc = cv.minMaxLoc(mask)
            ...
            y0 = min(max(0, y0), image.shape[0])
            x1 = min(max(0, x1), image.shape[1])
            x2 = min(max(0, x2), image.shape[1])
            x0 = min(max(0, x0), image.shape[1])

            matched = image[y1:y2, x1:x2, ...]
            
            output_list.append(AlignmentOutput(
    ...
    def __call__(self, image: np.ndarray):
        ...
        if self.thred:
            output_list = self._thred(res, image)
        else:
            raise ValueError('Args:thred cannot be None.')

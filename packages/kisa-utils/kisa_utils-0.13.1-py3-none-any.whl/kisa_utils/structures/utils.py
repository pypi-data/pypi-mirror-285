...
class Value:
    def __init__(self, valueType:Any, validator:Callable,/):
        ...
        if not validator:
            raise TypeError(f'invalid `validator` given')

        if not inspect.isfunction(validator):
            raise TypeError('Invalid `validator` given. Expected function|None')
        
        if 1 != len(inspect.signature(validator).parameters):
            raise TypeError(f'validator should take only 1 argument')

...
class PlacementFetcher:
  ...
  def _join_conversion_split(
    ...
    for row in placements:
      ...
      final_report_values.append(
        row.data + [conversion_name, conversions, all_conversions]
      )
  ...
  def _extract_conversion_split_metrics(
    ...
    conversion_row = placements_by_conversion_name.loc[is_conversion_row]
    if not (conversions := sum(conversion_row['conversions'].values)):
      conversions = 0.0
    if not (all_conversions := sum(conversion_row['all_conversions'].values)):
      all_conversions = 0.0
  ...
  def _aggregate_placements(
    ...
    if perform_relative_aggregations:
      for key, [numerator, denominator] in relative_aggregations_dict.items():
        if set([numerator, denominator]).issubset(

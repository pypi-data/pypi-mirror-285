        ...
        for perturbvar in input_xr.data_vars:
            if perturbvar in ['temperature',
                ...
                for lev in input_xr['level']:
                    ...
                    for time in input_xr['time']:
                        selected_data = input_xr[perturbvar].sel(level=lev,time=time)
                        modified_data = selected_data*rand
                        input_xr[perturbvar].loc[dict(level=lev,time=time)] = gaussian_filter(modified_data,sigma=sigma)
            elif perturbvar in ['2m_temperature',
                                'mean_sea_level_pressure',
                                '10m_v_component_of_wind',
                                '10m_u_component_of_wind',
                ...
                rand = np.random.uniform(perturbmin,perturbmax,(len(input_xr['lat']),len(input_xr['lon'])))
                for time in input_xr['time']:
                    selected_data = input_xr[perturbvar].sel(time=time)
                    modified_data = selected_data*rand
                    input_xr[perturbvar].loc[dict(time=time)] = gaussian_filter(modified_data,sigma=sigma)
        return input_xr

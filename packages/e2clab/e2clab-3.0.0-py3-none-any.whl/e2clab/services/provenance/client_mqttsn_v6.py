...
class Callback:
    ...
    def message_arrived(self, topic_name, payload, qos, retained, msgid):
        log.debug(
            f"Publish Arrived: {topic_name}, {payload}, " f"{qos}, {retained}, {msgid}"
        )
        return True
...
class Client:
    ...
    def start(self):
        ...
        mreq = struct.pack("4sl", socket.inet_aton(self.host_), socket.INADDR_ANY)

        self.sock.setsockopt(socket.IPPROTO_IPV6, socket.IP_ADD_MEMBERSHIP, mreq)

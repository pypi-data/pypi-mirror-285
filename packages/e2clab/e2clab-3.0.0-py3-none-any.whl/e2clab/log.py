...
class E2clabLogger(logging.LoggerAdapter):
    ...
    def process(self, msg, kwargs):
        ...
        if extra_tags is not None:
            prefix = ",".join([LOG_E2CLAB_PREFIX, *self.extra["tags"]])
        else:
            prefix = LOG_E2CLAB_PREFIX
        return f"[{prefix}] {msg}", kwargs


def getLogger(name, tags=None) -> E2clabLogger:
    """
...
def config_file_logger(
    ...
    log_file = file_path / LOG_INFO_FILENAME
    log_file = log_file.resolve().as_posix()
    file_handler_info = logging.FileHandler(log_file, LOG_WRITING_MODE)
    file_handler_info.setLevel(level=level)
    file_handler_info.setFormatter(file_formatter)

    error_file = file_path / LOG_ERR_FILENAME
    error_file = error_file.resolve().as_posix()
    file_handler_error = logging.FileHandler(error_file, LOG_WRITING_MODE)

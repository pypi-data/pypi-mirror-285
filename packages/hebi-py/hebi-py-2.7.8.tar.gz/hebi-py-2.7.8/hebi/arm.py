...
class ArmPlugin:
  ...
  __slots__ = ('_enabled', '_enabled_ratio', '_ramp_time')

  def __init__(self, enabled: bool, ramp_time: float):
...
class DynamicCompEffortPlugin(ArmPlugin):
  __slots__ = ('dyn_efforts')

  def __init__(self, enabled: bool = True, ramp_time: float = 0.0):
    super().__init__(enabled, ramp_time)

  def on_associated(self, arm: Arm):
    self.dyn_efforts = np.empty(arm.size, dtype=np.float64)
  ...
  def update(self, arm: Arm, dt: float):
    ...
    if arm.trajectory is not None:
      ...
      arm.robot_model.get_dynamic_comp_efforts(fbk_p,
                                               ...
                                               a,
                                               jacobians=arm._com_jacobians,
                                               output=self.dyn_efforts)
      arm.pending_command.effort += self.dyn_efforts * self._enabled_ratio
...
class EffortOffset(ArmPlugin):
  ...
  def __init__(self, offset: 'npt.NDArray[np.float32]', enabled: bool = True, ramp_time: float = 0.0):
    super().__init__(enabled, ramp_time)
    self._offset = offset
...
class ImpedanceController(ArmPlugin):
  ...
  def __init__(self, gains_in_end_effector_frame: bool = False, enabled: bool = True, ramp_time: float = 0.0):
    super().__init__(enabled, ramp_time)
  ...
  def set_i_clamp(self, x: float, y: float, z: float, roll: float, pitch: float, yaw: float):
    """Sets the clamping values for the impedance controller integrator.
    ...
    self._i_clamp[1] = y
    self._i_clamp[2] = z
    self._i_clamp[3] = roll
    self._i_clamp[4] = pitch
    self._i_clamp[5] = yaw
  ...
  def update(self, arm: Arm, dt: float):
    ...
    if not np.isnan(self._i_clamp).any():
      i_wrench = np.max(i_wrench, -1 * self._i_clamp)
      i_wrench = np.min(i_wrench, self._i_clamp)
...
class DoubledJointMirror(ArmPlugin):
  ...
  def __init__(self, index: int, group: 'Group', enabled: bool = True, ramp_time: float = 0.0):
    super().__init__(enabled, ramp_time)
...
def create_from_config(config: 'HebiConfig'):
  base_dir = os.getcwd()
  if config.hrdf:
    hrdf_file = os.path.join(base_dir, config.hrdf)
  ...
  if config.plugins:
    for plugin in config.plugins:
      if plugin['type'] == 'GravityCompensationEffort':
        ...
        arm.add_plugin(GravCompEffortPlugin(enabled, ramp_time, imu_feedback_index, imu_frame_index, imu_rotation_offset))
      elif plugin['type'] == 'DynamicCompensationEffort':
        enabled = plugin.get('enabled', True)
        enabled = plugin.get('enabled', True)
        ramp_time = float(plugin.get('ramp_time', 0.0))
        arm.add_plugin(DynamicCompEffortPlugin(enabled, ramp_time))
      elif plugin['type'] == 'ImpedanceController':
        in_ee_frame = plugin['gains_in_end_effector_frame'] == 'true'
        enabled = plugin.get('enabled', True)
        ramp_time = float(plugin.get('ramp_time', 0.0))
        p = ImpedanceController(in_ee_frame, enabled, ramp_time)
        ...
        p = DoubledJointMirror(idx, grp, enabled, ramp_time)
        arm.add_plugin(p)

  if config.gains and 'default' in config.gains:
    gains_file = os.path.join(base_dir, config.gains['default'])

...
class Minimol: 
    ...
    def __init__(self):
        # handle the paths
        with pkg_resources.path('minimol.ckpts.minimol_v1', 'state_dict.pth') as ckpt_p:
            ckpt_path = str(ckpt_p)
        with pkg_resources.path('minimol.ckpts.minimol_v1', 'config.yaml') as config_p:
            config_path = str(config_p)
        with pkg_resources.path('minimol.ckpts.minimol_v1', 'base_shapes.yamlex') as base_shapes_p:
    ...
    def load_config(self, config_name):
        config_dir = os.path.relpath('minimol/ckpts/minimol_v1', start=os.getcwd())

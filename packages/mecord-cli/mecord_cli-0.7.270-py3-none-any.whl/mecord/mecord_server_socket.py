...
class MecordShortConnectThread(Thread):
    ...
    def __init__(self, country, executor, mtype=''):
        ...
        logFilePath = f"{os.path.dirname(os.path.abspath(__file__))}/ShortConnect.log"
        file_handler = RotatingFileHandler(logFilePath, maxBytes=1024*1024*20, backupCount=2)
    ...
    def taskCallback(self, taskUUID, is_ok, msg, result):
        is_ok = xy_pb.TaskNotify(self.service_country, taskUUID, is_ok, msg, result)
        self.logger.info(f"{self.name} === {taskUUID} Notify is_ok={is_ok}")

    def run(self):
        print(f"   {self.name} start")
        num = 0
        what_time = 1 if self.service_country == "sg" else 3
        while self.is_running:
            if os.path.exists(refreshMessage):
                try:
                    os.remove(refreshMessage)
            ...
            if limit >= 1:
                ...
                self.logger.info(f'{self.name} === wait {limit} tasks -t {int(time.time())}, widgets: {self.widgets}')
                try:
                    datas, timeout = xy_pb.GetTask(self.service_country, self.widgets, limit=limit, keep_alive=False)
                    if len(datas) > 0:
                        self.logger.info(f'{self.name} === get {len(datas)} task -t {int(time.time())}')
                        task_list = []
                        for it in datas:
                        ...
                        self.logger.info(f'{self.name} === GetTaskList {str(task_list)} -t {int(time.time())}')
                except Exception as ex:
                    self.logger.info(f"{self.name} === get task exception : {ex}")
            num += 1

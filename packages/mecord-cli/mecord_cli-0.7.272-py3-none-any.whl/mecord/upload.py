...
def upload(src, taskUUID, _country=None, timeout=300):
    ...
    if total_time >= 5000:
        print(f'加速链接上传 {ossurl}\n=== x-oss-request-id:{request_id}  file_read_time: {read_time}ms  file_put_time: {put_time}ms  total_time: {total_time}ms')
    ...
    if res.status_code == 200:
        ...
        if total_time2 >= 5000:
            print(f'直连链接上传 {ossurl}\n=== x-oss-request-id:{request_id2}  file_read_time: {read_time}ms  file_put_time: {put_time2}ms  total_time: {total_time2}ms')
        s.close()

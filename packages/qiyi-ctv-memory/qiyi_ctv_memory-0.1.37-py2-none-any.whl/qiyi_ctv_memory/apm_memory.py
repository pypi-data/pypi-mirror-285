...
def parse_arguments():
    ...
    parser.add_argument('-i', '--ip', dest='ip', help='device ip,if there is only one device please ignore this')
    parser.add_argument('-p', '--pkgname', dest='pkgname',
                           help='package name,default is com.gitvdemo.video,if don`t change package name please '
                                'ignore this')
    parser.add_argument('-a', '--action', dest='action', help='action to do')
    parser.add_argument('--fi', '--fi', dest='fi', help='first input')
    parser.add_argument('--si', '--si', dest='si', help='second input')

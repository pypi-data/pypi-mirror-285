    ...
    if argParams.action:
        ...
        if action == "ss":
            ...
            if os.path.exists(snapshot_path):
                snapshot_summary.snapshot_summary(snapshot_path, output)
        elif action == "sr":
            t = time.time()
            strTime = str(int(t))
            ...
            if os.path.exists(record_path):
                record_summary.record_summary(record_path, output)
        elif action == "rs":
            ...
            if argParams.fi and argParams.si and os.path.exists(argParams.fi) and os.path.exists(argParams.si):
                snapshot_compare.compare(argParams.fi, argParams.si, output)
            else:
                print("参数非法或文件不存在")
        elif action == "cr":
            if argParams.fi and argParams.si and os.path.exists(argParams.fi) and os.path.exists(argParams.si):
                record_compare.compare(argParams.fi, argParams.si, output)
            else:
                print("参数非法或文件不存在")

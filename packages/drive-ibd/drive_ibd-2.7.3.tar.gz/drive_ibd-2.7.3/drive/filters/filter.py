...
class IbdFilter:
    ...
    def preprocess(
        ...
        ids_in_ibd_pd = len(
            np.unique(np.concatenate([self.ibd_pd.idnum1.unique(), self.ibd_pd.idnum2.unique()]))
        )

        logger.info(
            f"{ids_in_ibd_pd} out of the {len(cohort_ids)} ids provided were found within the IBD data file"
        )

...
class YugenClient:
    ...
    def __init__(self, auth_token: Optional[str] = None, config_path="./gru/config.yaml"):
        """
        ...
        for type in self.config.update_types:
            if entity_type.value == type:
                try:
                    ...
                    json_req["type"] = type
                    json_req["airflow_dags_backup_path"] = (
    ...
    def get_feature_health(self, entity_type: EntityType = EntityType.RAW_FEATURE, feature_name: Optional[str] = None) -> dict:
        """
        ...
        try:
            if entity_type.value in self.config.health_types:
                ...
                response = self.request_handler.send_request(method="GET", url_path=url_path)
                
                if response.status_code==200:  
                    logger.info(f"response: {response.json()}")    
                    return response.json()
                else:
                    logger.warning(f"Failed to retrieve feature health: {response.json()}")
                    return {}
            else:

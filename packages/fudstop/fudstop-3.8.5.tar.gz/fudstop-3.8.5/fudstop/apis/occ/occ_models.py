...
class StockInfo:
    def __init__(self, data, ticker):
        ...
        # Advanced metrics
        self.rsi = talib.RSI(self.close_prices)[-1] if self.close_prices else None
        self.macd, self.macd_signal, self.macd_hist = talib.MACD(self.close_prices) if self.close_prices else (None, None, None)
        self.bollinger_upper, self.bollinger_middle, self.bollinger_lower = talib.BBANDS(self.close_prices) if self.close_prices else (None, None, None)
        self.momentum = talib.MOM(self.close_prices)[-1] if self.close_prices else None
        self.adx = talib.ADX(self.high_prices, self.low_prices, self.close_prices)[-1] if self.high_prices and self.low_prices and self.close_prices else None
        self.stochastic_oscillator = talib.STOCH(self.high_prices, self.low_prices, self.close_prices) if self.high_prices and self.low_prices and self.close_prices else (None, None)
        self.williams_r = talib.WILLR(self.high_prices, self.low_prices, self.close_prices)[-1] if self.high_prices and self.low_prices and self.close_prices else None
        self.cci = talib.CCI(self.high_prices, self.low_prices, self.close_prices)[-1] if self.high_prices and self.low_prices and self.close_prices else None
        self.average_daily_volume = sum(self.volumes) / len(self.volumes) if self.volumes else None
        self.volatility_percentile = talib.VAR(self.close_prices)[-1] if self.close_prices else None
        self.sharpe_ratio = self.calculate_sharpe_ratio()
        self.sortino_ratio = self.calculate_sortino_ratio()
        self.treynor_ratio = self.calculate_treynor_ratio()
        self.calmar_ratio = self.calculate_calmar_ratio()
        self.information_ratio = self.calculate_information_ratio()
        self.capture_ratio = self.calculate_capture_ratio()
        self.alpha = self.calculate_alpha()
        self.beta = self.calculate_beta()
        self.r_squared = self.calculate_r_squared()
        self.tracking_error = self.calculate_tracking_error()
        self.upside_capture = self.calculate_upside_capture()
        self.downside_capture = self.calculate_downside_capture()
        ...
        self.data_dict = {
            ...
            'rsi': self.rsi,
            'macd': self.macd,
            'bollinger_bands': (self.bollinger_upper, self.bollinger_middle, self.bollinger_lower),
    ...
    def calculate_sharpe_ratio(self):
        ...
        return np.mean(excess_returns) / np.std(excess_returns) if len(self.returns) > 1 else None

    def calculate_sortino_ratio(self):
        risk_free_rate = 0.01  # Example risk-free rate
        negative_returns = [r for r in self.returns if r < risk_free_rate]
        downside_deviation = np.std(negative_returns)
        excess_returns = self.returns - risk_free_rate
        return np.mean(excess_returns) / downside_deviation if len(negative_returns) > 1 else None
    ...
    def calculate_calmar_ratio(self):
        ...
        return np.mean(self.returns) / max_drawdown if max_drawdown else None

    def calculate_information_ratio(self):
        benchmark_returns = np.mean(self.returns)  # Assuming benchmark returns are the same as stock returns
        tracking_error = self.calculate_tracking_error()
        return (np.mean(self.returns) - benchmark_returns) / tracking_error if tracking_error else None
    ...
    def calculate_capture_ratio(self):
        ...
        return upside_capture / downside_capture if downside_capture else None

    def calculate_alpha(self):
        risk_free_rate = 0.01  # Example risk-free rate
        beta = self.calculate_beta()
        expected_return = risk_free_rate + beta * (np.mean(self.returns) - risk_free_rate)
        return np.mean(self.returns) - expected_return if beta else None

    def calculate_beta(self):
        market_returns = self.returns  # Assuming market returns are the same as stock returns
        cov_matrix = np.cov(self.returns, market_returns)
        return cov_matrix[0, 1] / cov_matrix[1, 1] if len(cov_matrix) > 1 else None

    def calculate_r_squared(self):
        market_returns = self.returns  # Assuming market returns are the same as stock returns
        correlation_matrix = np.corrcoef(self.returns, market_returns)
        return correlation_matrix[0, 1] ** 2 if len(correlation_matrix) > 1 else None

    def calculate_tracking_error(self):
        benchmark_returns = np.mean(self.returns)  # Assuming benchmark returns are the same as stock returns
        differences = self.returns - benchmark_returns
        return np.std(differences) if len(self.returns) > 1 else None

    def calculate_upside_capture(self):
        market_returns = self.returns  # Assuming market returns are the same as stock returns
        up_market = [r for r in market_returns if r > 0]
        stock_up = [self.returns[i] for i in range(len(self.returns)) if market_returns[i] > 0]
        return np.mean(stock_up) / np.mean(up_market) if len(up_market) > 0 else None

    def calculate_downside_capture(self):
        market_returns = self.returns  # Assuming market returns are the same as stock returns
        down_market = [r for r in market_returns if r < 0]
        stock_down = [self.returns[i] for i in range(len(self.returns)) if market_returns[i] < 0]
        return np.mean(stock_down) / np.mean(down_market) if len(down_market) > 0 else None

    def calculate_max_drawdown(self):
        cumulative_returns = np.cumprod(1 + self.returns) - 1
        peak = np.maximum.accumulate(cumulative_returns)
        drawdown = (cumulative_returns - peak) / peak
        return np.min(drawdown) if len(drawdown) > 0 else None
class ExpiryDates:
    def __init__(self, data):
...
class OptionsMonitor:
    def __init__(self, data):
        ...
        self.put_asksize = [float(i.get('put_asksize', 0.0)) for i in data]
        ...
        self.call_bidtime = [float(i.get('call_bidtime', 0.0)) for i in data]
        ...
        self.put_bidtime = [float(i.get('put_bidtime', 0.0)) for i in data]
        self.call_vega = [float(i.get('call_vega', 0.0)) for i in data]
        self.put_theta = [float(i.get('put_theta', 0.0)) for i in data]
        self.put_optionsymbol = [float(i.get('put_optionsymbol', 0.0)) for i in data]
        ...
        self.call_asktime = [float(i.get('call_asktime', 0.0)) for i in data]
        ...
        self.call_optionsymbol = [float(i.get('call_optionsymbol', 0.0)) for i in data]
        ...
        self.put_asktime = [float(i.get('put_asktime', 0.0)) for i in data]
    ...
    def calculate_call_profitability_percentage(self):
        profitability = []
        for ask, bid, strike, current_price in zip(self.call_ask, self.call_bid, self.strike, self.current_price):
            premium_received = (ask + bid) / 2
            profit = ((current_price - strike + premium_received) / strike) * 100
            profitability.append(profit)
        return profitability

    def calculate_put_profitability_percentage(self):
        profitability = []
        for ask, bid, strike, current_price in zip(self.put_ask, self.put_bid, self.strike, self.current_price):
            premium_received = (ask + bid) / 2
            profit = ((strike - current_price + premium_received) / strike) * 100
            profitability.append(profit)
        return profitability

    def calculate_advanced_call_profitability(self):
        profitability = []
        for delta, theta, vega, iv, strike, current_price, tte in zip(self.call_delta, self.call_theta, self.call_vega, self.call_iv, self.strike, self.current_price, self.time_to_expiry):
            iv_historical = iv  # Assuming iv_historical is available or equal to current iv
            profit = delta * (current_price - strike) + theta * tte + vega * (iv - iv_historical)
            profitability.append(profit)
        return profitability

    def calculate_advanced_put_profitability(self):
        profitability = []
        for delta, theta, vega, iv, strike, current_price, tte in zip(self.put_delta, self.put_theta, self.put_vega, self.put_iv, self.strike, self.current_price, self.time_to_expiry):
            iv_historical = iv  # Assuming iv_historical is available or equal to current iv
            profit = delta * (strike - current_price) + theta * tte + vega * (iv - iv_historical)
            profitability.append(profit)
        return profitability

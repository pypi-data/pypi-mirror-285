...
def get_client_custom_params(**kwargs):
    ...
    parms = {
        ...
        "proxies": kwargs.get("proxies", {"https://": proxy, "http://": proxy}) if proxy else None,
        "verify": os.environ.get("LANGFUSE_VERIFY_SSL", "true") == "true",
    }
    if (not os.path.exists("/etc/ssl/certs/ca-certificates.crt") and not os.listdir("/usr/local/share/ca-certificates")):
        parms["verify"] = False
    return parms

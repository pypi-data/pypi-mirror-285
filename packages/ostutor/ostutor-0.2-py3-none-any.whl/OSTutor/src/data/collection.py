...
class Collection:
    ...
    def collectMan(self, inst: str, type: str):
        """
        收集指定指令Man文档
        """
        if not os.path.exists('../assets/man1'):
            os.makedirs('../assets/man1')
        if not os.path.exists('../assets/man8'):
            os.makedirs('../assets/man8')
        # 使用 subprocess.Popen 执行 rpm 命令并获取输出
        flag = '1' if type == 'user' else '8'
        ...
        # 如果内容存在NAME标题则为有效内容
        if 'NAME' in  text:
            file = f'{self.getManPath(type) + inst}.txt'

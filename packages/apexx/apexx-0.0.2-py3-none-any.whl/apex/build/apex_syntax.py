...
class Apex:
    ...
    def __get_vars(text: str, scope_tag: str) -> Dict[str, str]:
        ...
        """
        vars = {}
        var_regex = re.compile(r"@apexx:\w+:var")
    ...
    def __get_tag(text: str) -> str:
        tag_regex = re.compile(r"@apexx:\w+:tag")
    ...
    def parse_syntax(self, blueprint_dir: str) -> Tuple[Set[str], Dict[str, str]]:
        """
        Parse apexx annotations from template
        ...
        for filename in os.scandir(blueprint_dir):
            if filename.is_file():
                        ...
                        if len(var_list):
                            for var_key in var_list:
                                if var_key in selected_vars:
                                    line = line.replace(f"@apexx:{var_key}:var", selected_vars[var_key])
                                else:
                ...
                if tag != "":
                    if tag not in selected_tags:
                        continue
                    dir_name = dir_name.replace(f"@apexx:{tag}:tag", "")

                var_list = self.__get_vars(dir_name, "")
                if len(var_list):
                    for var_key in var_list:
                        if var_key in selected_vars:
                            dir_name = dir_name.replace(f"@apexx:{var_key}:var", selected_vars[var_key])
                        else:
                            raise ValueError(f"Variable {var_key} not found in selected variables")

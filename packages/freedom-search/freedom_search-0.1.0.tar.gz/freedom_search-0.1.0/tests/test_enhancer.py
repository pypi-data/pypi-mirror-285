...
class TestInternetSearchEnhancer(unittest.TestCase):
    ...
    @patch('freedom_search.enhancer.PackageManager')
    def test_create_google_search_installed(self, mock_pm):
        mock_pm.return_value.is_installed.return_value = True
        self.enhancer._create_google_search()
        mock_pm.return_value.is_installed.assert_called_once_with("googlesearch")
        mock_pm.return_value.install.assert_not_called()

    @patch('freedom_search.enhancer.PackageManager')
    def test_create_google_search_not_installed(self, mock_pm):
        mock_pm.return_value.is_installed.return_value = False
        self.enhancer._create_google_search()
        mock_pm.return_value.is_installed.assert_called_once_with("googlesearch")
        mock_pm.return_value.install.assert_called_once_with("googlesearch-python")
